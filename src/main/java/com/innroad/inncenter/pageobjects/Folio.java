...
public class Folio implements IFolio {
	...
	static CPReservationPage reservationPage = new CPReservationPage();
...
//		wait.until(ExpectedConditions.elementToBeClickable(moveFolio.GuestInfo));
	...
	public void click_Folio(WebDriver driver, ArrayList<String> test_steps) throws InterruptedException {
		...
		try {
			...
			test_steps.add("Clicking on Folio");
			folioLogger.info("Clicking on Folio");
			String path = "//a[@class='current']";
			Wait.WaitForElement(driver, path);
			Wait.waitForElementToBeVisibile(By.xpath(path), driver);
			assertTrue(driver.findElement(By.xpath(path)).isDisplayed(), "Failed: to Displayed Folio Detail");
			test_steps.add("Folio Detail Tab is Displayed");
			folioLogger.info("Folio Detail Tab is Displayed");
		}
	}
	...
	public void folioPayment(ArrayList<String> test_steps, WebDriver driver,String Amount)
			throws InterruptedException {
		String paybuttonxpath = "//a[contains(text(),'"+Amount+"')]";
		Utility.ScrollToElement(driver.findElement(By.xpath(paybuttonxpath)), driver);
		...
		folioLogger.info("Click On Pay $ "+Amount+" Button");
		test_steps.add("Click On Pay $ "+Amount+" Button");
		
		String closebuttonxpath = "//button[@data-bind = 'click: CloseTakePaymentButtonClick']";
		Utility.ScrollToElement(driver.findElement(By.xpath(closebuttonxpath)), driver);
		...
		folioLogger.info("Click On CLose Button");
		test_steps.add("Click On CLose Button");
	}
	
	public void selectFolioByRoomClassAbb(WebDriver driver, String RoomCLassAbb) throws InterruptedException {
		
		String xpath = "(//span[contains(text(),'Guest Folio For "+RoomCLassAbb+"')])[last()]";
		Wait.WaitForElement(driver, xpath);
		driver.findElement(By.xpath(xpath)).click();
		folioLogger.info("Selected Guest Folio For "+RoomCLassAbb);
		test_steps.add("Selected Guest Folio For "+RoomCLassAbb);
	}
	...
	public void SwipePayment(WebDriver driver, String Amount, String PaymentType, String CardNumber)
		...
		jse.executeScript("arguments[0].click();", element);
			...
			e.printStackTrace();
			element.click();
			folioLogger.info("Pay button click");
		}
		try {
			Wait.WaitForElement(driver, OR.Reservation_PaymentPopup);
			// Wait.explicit_wait_xpath(OR.Reservation_PaymentPopup, driver);
	...
	public List<WebElement> getAmountWebElementList(WebDriver driver) throws InterruptedException {
		String Amountxpath = "(//tr[@data-bind = 'if: $parent.ShouldShowThisItem($data)']//td[11]//span)";
		Wait.WaitForElement(driver, Amountxpath);
		Wait.wait5Second();
		List<WebElement> AmountList = driver.findElements(By.xpath(Amountxpath));
		return  AmountList;
	}
	
	public List<WebElement> getDescriptionLinkWebElementList(WebDriver driver) {
		String LinkXpath = "(//tr[@data-bind = 'if: $parent.ShouldShowThisItem($data)']//td[8])//a";
		Wait.WaitForElement(driver, LinkXpath);
		List<WebElement> linksList = driver.findElements(By.xpath(LinkXpath));
		return linksList;
		
	}
	
	public int getRowCount(WebDriver driver, ArrayList<String> testSteps) {
		String cat_xpath = "//span[@data-bind = 'text:$data.Category']";
		List<WebElement> rowsList = driver.findElements(By.xpath(cat_xpath));
		testSteps.add("row size : " + rowsList.size());
		Utility.app_logs.info("row size : " + rowsList.size());
		return rowsList.size();
	}
	
	public void FolioItemDetailPopup(WebDriver driver, ArrayList<String> testSteps,String Amount, String Description, String Note) {
		// Heading Verification
		String headingxpath = "//h4[contains(text(),'Item Detail')]";
		Wait.WaitForElement(driver, headingxpath);
		String heading =   driver.findElement(By.xpath(headingxpath)).getText();
		testSteps.add("Expected PopUp Title: <b>"+heading);
		testSteps.add("Found PopUp Title: <b>"+heading);
		assertEquals(heading, "Item Detail");
		
		
		int countofRows = getRowCount(driver, testSteps); 
		
		
		// Select category
		String Categoryxpath = "//select[contains(@data-bind , 'options: $parent.CategoriesToShow')]";
		Wait.WaitForElement(driver, Categoryxpath);
		driver.findElement(By.xpath(Categoryxpath)).click();
		testSteps.add("Clicking On Category Dropdown Menu");
		String SelecedCategoryxpath = "//select[contains(@data-bind , 'options: $parent.CategoriesToShow')]//option[contains(text() , 'Room Charge')]";
		Wait.WaitForElement(driver, SelecedCategoryxpath);
		driver.findElement(By.xpath(SelecedCategoryxpath)).click();
		testSteps.add("Clicking On Room Charge Category ");
		
		
		//Add Description
		String decriptioninputxpath = "//input[contains(@data-bind , 'value: $parent.NewFolioDescription')]";
		Wait.WaitForElement(driver, decriptioninputxpath);
		driver.findElement(By.xpath(decriptioninputxpath)).sendKeys(Description);
		testSteps.add("Entering Description:  <b>"+Description);
		
		
		//Add Amount
		String amountinputxpath = "//input[contains(@data-bind , 'value: $parent.NewFolioAmount')]";
		Wait.WaitForElement(driver, amountinputxpath);
		driver.findElement(By.xpath(amountinputxpath)).sendKeys(Amount);
		testSteps.add("Entering Amount:  <b>"+Amount);
		
		//Add Notes
		String Noteinputxpath = "//textarea[contains(@data-bind , 'value: $parent.NewFolioNotes')]";
		Wait.WaitForElement(driver, Noteinputxpath);
		driver.findElement(By.xpath(Noteinputxpath)).sendKeys(Note);
		testSteps.add("Entering Note In TextArea:  <b>"+Note);
		
		
		//Click On Add Button
		
		String AddButtonxpath = "//button[contains(@data-bind , 'click: $parent.btnAdd_Click')]";
		Wait.WaitForElement(driver, AddButtonxpath);
		driver.findElement(By.xpath(AddButtonxpath)).click();
//		Utility.clickThroughJavaScript(driver, driver.findElement(By.xpath(AddButtonxpath)));
		testSteps.add("Click On <b>Add<b> Button");
		...
		int countofRowsAfterAdd = getRowCount(driver, testSteps);
		if(countofRows<countofRowsAfterAdd) {
			testSteps.add("Room Charges Has Successfully added");
		}
		else {
			testSteps.add("Failed: Existing : Folio line item not getting updated when we add from folio line item");
			testSteps.add("<a href='https://innroad.atlassian.net/browse/NG-12632'> Failed: Click here for more info </a>");
		}
		
		
		//Click On Continue Button
		String ContinueButtonxpath = "//button[contains(@data-bind , 'click: ClickContinueButton, enable: isDirty')]";
		Wait.WaitForElement(driver, ContinueButtonxpath);
		driver.findElement(By.xpath(ContinueButtonxpath)).click();
		testSteps.add("Click On <b>Continue<b> Button");
	}
	...
	public ArrayList<String> Create_NewFolio_(WebDriver driver, String FolioName, String FolioDescription,
			ArrayList<String> test_steps) throws InterruptedException {
		...
		// click New Folio
		Wait.explicit_wait_xpath(OR.Click_NewFolio_, driver);
		folio.Click_NewFolio_.click();
		folioLogger.info("Click New Folio");
	...
	public void VerifyRollBackLineItemPendingStatus(WebDriver driver, String Category,HashMap<String, String> categoryStatus,ArrayList<String>list) throws InterruptedException {

		
		String PathPrePost = "//span[text()='" + Category + "']//..//..//td[2]//i";
		Wait.WaitForElement(driver, PathPrePost);
		String iconClass =  driver.findElement(By.xpath(PathPrePost)).getAttribute("class");
		String pendingIconClass = "inncenter-icons-collection transactionstatus-icon1";
		String CancelledIconClass = "inncenter-icons-collection transactionstatus-icon3";
 		if(iconClass.equalsIgnoreCase(pendingIconClass)) {
 			list.add(Category+" Status: Pending");
 			categoryStatus.put(Category, "Pending");
 		}
 		else if(iconClass.equalsIgnoreCase(CancelledIconClass)) {
 			list.add(Category+" Status: Cancelled");
 			categoryStatus.put(Category, "Cancelled");
		}
 		else {
 			list.add(Category+" Status: Not Pending");
 			categoryStatus.put(Category, "Not Pending");
		}
	...
	public void ClickOnAdvanceLedgerTab(WebDriver driver, ArrayList<String> testSteps) throws InterruptedException {
		String xpath = "//a[contains(text (), 'Adv Dep Ledger')]";
		Wait.WaitForElement(driver, xpath);
		Utility.ScrollToElement(driver.findElement(By.xpath(xpath)), driver);
		driver.findElement(By.xpath(xpath)).click();
		folioLogger.info("Click On Adv Dep Ledger Tab");
		testSteps.add("Click On Adv Dep Ledger Tab");
	}
	...
	public void Reservation_Status_DropDown(WebDriver driver) {
	...
	public void Cancel_Reservation_Status_DropDown(WebDriver driver) {
		String dropdownXpath = "(//a[contains(@data-bind , 'ReservationUpdate')]//span[contains(text() , 'Cancel')])[1]";
		Wait.WaitForElement(driver, dropdownXpath);
		Wait.waitForElementToBeClickable(By.xpath(dropdownXpath), driver);
		driver.findElement(By.xpath(dropdownXpath)).click();
	}
	
	public void Cancel_Reservation_PopUp(WebDriver driver , ArrayList<String> testSteps) {
		String HeadingXpath = "//h4[contains(text(),'Cancel Reservation')]";
		Wait.WaitForElement(driver, HeadingXpath);
		String heading =  driver.findElement(By.xpath(HeadingXpath)).getText();
		assertEquals(heading, "Cancel Reservation", "Failed:  to verify Cancel reservation Popup");
		testSteps.add("Pop Up Heading Verified: <b>"+heading);
		
		String CancelReasonXpath = "//textarea[contains(@data-bind,'value: modal.cancelreason')]";
		Wait.WaitForElement(driver, CancelReasonXpath);
		driver.findElement(By.xpath(CancelReasonXpath)).sendKeys("Cancel Reservation");
		testSteps.add("Enter Cancellation Reason: <b>Cancel Reservation<b>");
		
		
		String CancelButtonXpath = "//button[contains(text(),'PROCEED TO CANCELLATION PAYMENT')]";
		Wait.WaitForElement(driver, CancelButtonXpath);
		Wait.waitForElementToBeClickable(By.xpath(CancelButtonXpath), driver);
		driver.findElement(By.xpath(CancelButtonXpath)).click();
		testSteps.add("Click On <b>Processed Cancellation Button<b>");
		
		
	}
	
	public void Verify_Cancel_Reservation_PopUp(WebDriver driver , ArrayList<String> testSteps, String CancellationAmount) {
		String HeadingXpath = "//h4[contains(text(),'Cancellation Payment')]";
		Wait.WaitForElement(driver, HeadingXpath);
		String heading =  driver.findElement(By.xpath(HeadingXpath)).getText();
		assertEquals(heading, "Cancellation Payment", "Failed:  to verify Cancel reservation Popup");
		testSteps.add("Pop Up Heading Verified: <b>"+heading);
		
		
		String inputValueID = "payFormAmountV2";
		String cancellation_Charges = driver.findElement(By.id(inputValueID)).getAttribute("value");
		assertEquals(cancellation_Charges, CancellationAmount, "Failed to verify Cancellation Amount");
		
		
		String ExternalPaymentXpath = "(//input[contains(@data-bind , 'checked:IsPaymentLogAsExternal')]//..//span)[1]";
		Wait.WaitForElement(driver, ExternalPaymentXpath);
//		Wait.waitForElementToBeClickable(By.xpath(ExternalPaymentXpath), driver);
		driver.findElement(By.xpath(ExternalPaymentXpath)).click();
		testSteps.add("Check The checkbox <b>Log as External Payment<b>");
		
		
		String PayButtonXpath = "//button[contains(text(),'Log $ "+CancellationAmount+"')]";
		Wait.WaitForElement(driver, PayButtonXpath);
		Wait.waitForElementToBeClickable(By.xpath(PayButtonXpath), driver);
		driver.findElement(By.xpath(PayButtonXpath)).click();
		testSteps.add("Click On <b>Pay Cancellation Charges Button<b>");
	...
	public ArrayList<String> AddfolioLineItem(WebDriver driver, String Category, String Amount,String Description,
			ArrayList<String> getTest_Steps) throws InterruptedException {
		Elements_FolioLineItemsVoid FolioLineItems = new Elements_FolioLineItemsVoid(driver);
		Wait.explicit_wait_visibilityof_webelement(FolioLineItems.selectCategory, driver);
		Utility.ScrollToElement(FolioLineItems.selectCategory, driver);
		new Select(FolioLineItems.selectCategory).selectByVisibleText(Category);
		getTest_Steps.add("Selecting Category: "+Category);
		Wait.explicit_wait_visibilityof_webelement(FolioLineItems.enterAmount, driver);
		System.out.println("Amount in func:" + Amount);
		FolioLineItems.enterAmount.sendKeys(Amount);
		getTest_Steps.add("Entering Amount: "+Amount);
		Wait.WaitForElement(driver, OR.enterDescription);
		FolioLineItems.enterDescription.clear();
		FolioLineItems.enterDescription.sendKeys("       ");
		getTest_Steps.add("Entering Space bar in Description");
		Wait.explicit_wait_visibilityof_webelement(FolioLineItems.clickCommitButton, driver);
		Utility.ScrollToElement(FolioLineItems.clickCommitButton, driver);
		FolioLineItems.clickCommitButton.click();
		getTest_Steps.add("Clicking On Commit Button");
		Wait.waitForElementToBeVisibile(By.className("toast-message"), driver);
		String text =   driver.findElement(By.className("toast-message")).getText();
		getTest_Steps.add("Error Toast Message Display");
		getTest_Steps.add("Expected: Error Toast Message=> "+text);
		getTest_Steps.add("Actual: Error Toast Message=> "+text);
		folioLogger.info("\n\n"+text+"\n");
		if(text.equalsIgnoreCase("Please fill the required fields in all the new items.")) {
			getTest_Steps.add("Verified: Error Toast Message=> "+text);
			Thread.sleep(500);
			Wait.WaitForElement(driver, OR.enterDescription);
			FolioLineItems.enterDescription.clear();
			FolioLineItems.enterDescription.sendKeys(Description);
			getTest_Steps.add("Entering Text << "+Description+" >> in Description");
			
			Thread.sleep(2000);
			Wait.explicit_wait_visibilityof_webelement(FolioLineItems.clickCommitButton, driver);
			Utility.ScrollToElement(FolioLineItems.clickCommitButton, driver);
			FolioLineItems.clickCommitButton.click();
			getTest_Steps.add("Clicking On Commit Button Again");
			getTest_Steps.add("Line Item Added Successfully");
		}
		
		Wait.explicit_wait_visibilityof_webelement(FolioLineItems.getBalanceFolioLineItems, driver);
		String getBalance = FolioLineItems.getBalanceFolioLineItems.getText();
		getTest_Steps.add("Total Balance: "+getBalance);
		folioLogger.info(" Balance of the Folio Line Items " + getBalance);
		return getTest_Steps;
	}
	...
	public void openFolioLineItem(WebDriver driver, String description, ArrayList<String> testSteps) throws InterruptedException {
		Elements_Folio folio = new Elements_Folio(driver);

		String xpath = "//td/a[contains(@data-bind,'fnPayLineItemDetail') and contains(text(),'"+description+"')]";
		Wait.WaitForElement(driver, xpath);
		WebElement FolioDescription = driver.findElement(By.xpath(xpath));
		Utility.ScrollToElement(FolioDescription, driver);
		FolioDescription.click();
		testSteps.add("Click on Folio line item " +description);
		folioLogger.info("Click on Folio line item " +description);
...
//		jse.executeScript("arguments[0].scrollIntoView();", moveFolio.MoveFolio_SelectFiloItem);
	...
	public String getTotalBalance(WebDriver driver) {
		...
		Elements_FolioLineItemsVoid elementFolio = new Elements_FolioLineItemsVoid(driver);
		folioLogger.info(elementFolio.totalBalance.getText());
		String str = elementFolio.totalBalance.getText().replace("$", " ").trim();
		System.out.println("totalBalance: " + str);
		return str;
	}
	...
	public void clickOnRollBackIcon(WebDriver driver , ArrayList<String> testSteps) {
		String iconXpath = "//button[contains(@data-bind , 'click: $parent.btnRollBack')]";
		Wait.WaitForElement(driver, iconXpath);
		driver.findElement(By.xpath(iconXpath)).click();
		testSteps.add("Clicking Rollback Icon Button");
		
		String ContinueButoonxpath = "//button[@data-bind='click: ClickContinueButton, enable: isDirty']";
		Wait.WaitForElement(driver, ContinueButoonxpath);
		driver.findElement(By.xpath(ContinueButoonxpath)).click();
		testSteps.add("Clicking On Continue Button");
	}
	...
	public ArrayList<String> CheckboxDisplayVoidItem(WebDriver driver, boolean isChecked) throws InterruptedException {
		...
		if (isChecked) {
			if (!elementFolio.checkboxDisplayVoidItem.isSelected()) {
				...
				Utility.clickThroughAction(driver, elementFolio.checkboxDisplayVoidItem);
	...
	public void PostedLineItems(WebDriver driver, String Category, String State, String PostState)
		...
		String PathPrePost = "//span[text()='"+Category+"']//..//..//td//i[@class='inncenter-icons-collection transactionstatus-icon1']";
		System.out.println(PathPrePost);
		WebElement ElementPrePost = driver.findElement(By.xpath(PathPrePost));
		Wait.explicit_wait_visibilityof_webelement(ElementPrePost, driver);
		...
		PathPrePost = "//span[text()='"+Category+"']//..//..//td//i[@class='inncenter-icons-collection transactionstatus-icon2']";
		System.out.println(PathPrePost);
	...
	public void verifyFolioCategoryDropdownList(WebDriver driver, String Name,ArrayList<String> testStep)
			throws InterruptedException, ParseException {

		Wait.wait1Second();
		String xpath = "//div[@class='dropdown-menu open']//ul[@role='menu']//li//span[contains(text(),'"+Name+"')]";
		Wait.WaitForElement(driver, xpath);
//		Wait.waitForElementToBeInVisibile(By.xpath(xpath), driver);
		String FolioDropDownName = driver.findElement(By.xpath(xpath)).getText();
		String[] FolioDropDown_Name = FolioDropDownName.split(" ");
		System.out.println(FolioDropDown_Name[0]);
		assertEquals(FolioDropDown_Name[0].trim(), Name.trim(),"Failed: folio drop down is mismatching!");
		testStep.add("Verify: Folio DropDown also has <<"+FolioDropDownName+">> Option");
	...
	public void VerifyLineItems_State(WebDriver driver, String Category, String State, int index)
		...
		Wait.wait1Second();
		String PathPrePost = "//span[text()='" + Category + "']//..//..//td//i";
		System.out.println(PathPrePost);
		WebElement ElementPrePost = driver.findElement(By.xpath(PathPrePost));
		Wait.explicit_wait_visibilityof_webelement(ElementPrePost, driver);
		assertEquals(ElementPrePost.isDisplayed(), true, "Failed: Line item is not posted state");
	...
	public ArrayList<String> folioTab2(WebDriver driver) throws InterruptedException {
		...
		try {
			...
			Wait.WaitForElement(driver, NewFolio.Foliotab2);
			Wait.waitForElementToBeVisibile(By.xpath(NewFolio.Foliotab2), driver);
			Wait.waitForElementToBeClickable(By.xpath(NewFolio.Foliotab2), driver);
			elementsFolio.Foliotab2.click();
			testSteps.add("Click on Folio");
		}
	...
	public void verifyLineItemsState(WebDriver driver, String Category, String State, int index)
		...
		String PathPrePost = "(//span[text()='" + Category + "']//..//..//td//i[contains(@class, '" + State + "')])[" + index
				+ "]";
		System.out.println(PathPrePost);
		Wait.WaitForElement(driver, PathPrePost);
		WebElement ElementPrePost = driver.findElement(By.xpath(PathPrePost));
//		Wait.explicit_wait_visibilityof_webelement(ElementPrePost, driver);
	...
	public ArrayList<String> changeFolioOption(WebDriver driver, String FolioName) throws InterruptedException {

		Elements_FolioLineItemsVoid elementFolio = new Elements_FolioLineItemsVoid(driver);
		ArrayList<String> testSteps = new ArrayList<>();
		String path = "//button[contains(@title,'Guest Folio For')]";
		Wait.WaitForElement(driver, path);
		Wait.waitForElementToBeVisibile(By.xpath(path), driver);
...
//		liSelect.click();
	...
	public String getFolioLineItemDateWithCategory(WebDriver driver, String Category,ArrayList<String> testSteps) {
		String path = "//span[contains(text(),'"+Category+"')]/../..//span[contains(@data-bind,'text: moment($data.EffectiveDate')]";
		Wait.WaitForElement(driver, path);
		String date = driver.findElement(By.xpath(path)).getText();
		folioLogger.info("LineItem Date against "+Category+" is "+date);
		testSteps.add("LineItem Date against "+Category+" is "+date);
		return date;
	}
	
	public String getFolioLineItemAmountWithCategory(WebDriver driver, String Category,ArrayList<String> testSteps) {
		String path = "//span[contains(text(),'"+Category+"')]/../..//span[contains(@data-bind,'amount')]";
		Wait.WaitForElement(driver, path);
		String[] Amount = driver.findElement(By.xpath(path)).getText().split(" ");
		folioLogger.info("LineItem Amount against "+Category+" is "+Amount[0]);
		testSteps.add("LineItem Amount against "+Category+" is "+Amount[0]);
		return Amount[0];
	}
	
	
	
	public String getAdv_Dep_LedgeDate(WebDriver driver,ArrayList<String> testSteps) {
		String path = "//tbody[@data-bind='foreach: AdvDepositArray, visible: ShowAdvDeposit()']//span[@data-bind = 'text: $data.EffectiveDate']";
		Wait.WaitForElement(driver, path);
		String date = driver.findElement(By.xpath(path)).getText();
		folioLogger.info("LineItem Date is "+date);
		testSteps.add("LineItem Date is "+date);
		return date;
	}
	
	public String getAdv_Dep_LedgeAmount(WebDriver driver,ArrayList<String> testSteps) {
		String path = "//tbody[@data-bind='foreach: AdvDepositArray, visible: ShowAdvDeposit()']//span[@data-bind = 'showInnroadCurrency: { value: (parseFloat($data.Amount) * -1), ShouldShowNegativeAsBracket: false }']";
		Wait.WaitForElement(driver, path);
		String[] Amount = driver.findElement(By.xpath(path)).getText().split(" ");
		folioLogger.info("LineItem Amount: "+Amount[1]);
		testSteps.add("LineItem Amount: "+Amount[1]);
		return Amount[1];
	}
...
//		Wait.waitForElementToBeVisibile(By.xpath(OR.roomNumberInItemDetails), driver);
	...
	public void clikOnLineItemDescriptionAfterSomeAction(WebDriver driver, String category, ArrayList<String> test_steps) {
		String description = "//span[contains(text(),'" + category
				+ "')]/../../td[contains(@data-bind,'lineitem-description')]/a";
		driver.findElement(By.xpath(description)).click();
		test_steps.add("Click on line item description");
...
//		Wait.waitForElementToBeVisibile(By.xpath(OR.roomNumberInItemDetails), driver);
	...
	public void RollbackMC_Category(WebDriver driver,  ArrayList<String> test_steps) throws InterruptedException {
		String NoteTextAreaxpath = "//textarea[@data-bind='value: Notes']";
		Wait.WaitForElement(driver, NoteTextAreaxpath);
//		Wait.waitForElementToBeInVisibile(By.xpath(NoteTextAreaxpath), driver);
//		Wait.waitForElementToBeClickable(By.xpath(NoteTextAreaxpath), driver);
		String note = "Cancellation Note";
		driver.findElement(By.xpath(NoteTextAreaxpath)).sendKeys(note);
		test_steps.add("Adding "+note+" In Note Area: ");
		
		String processxpath = "(//div[@id='ReservationPaymetItemDetail']//Button[contains(text(),'Add')])[1]|(//Button[contains(text(),'Process')])[2]";//"(//Button[contains(text(),'Process')])[2]";
		Wait.WaitForElement(driver, processxpath);
		Wait.waitForElementToBeClickable(By.xpath(processxpath), driver);
		driver.findElement(By.xpath(processxpath)).click();
		test_steps.add("Clicking On Add Button");
		
		Thread.sleep(2000);
		String ContinueButoonxpath = "//button[@type='submit' and @data-bind = 'enable: (EnableContinueBtn() && !ShowLoading() && !ShowLoadingNew()), click: btnSaveTranPayClick']";
		Wait.WaitForElement(driver, ContinueButoonxpath);
		Wait.waitForElementToBeClickable(By.xpath(ContinueButoonxpath), driver);
		driver.findElement(By.xpath(ContinueButoonxpath)).click();
	...
	public ArrayList<String> verifyLineItemDetails(WebDriver driver, String RoomNumber, String roomClassName,
		...
		try {
			...
			Wait.waitForElementToBeVisibile(By.xpath(OR.cancelButton1), driver);
			FolioLineItems.cancelButton1.click();
		}
	...
	public String getToatalBalance(WebDriver driver) throws InterruptedException {
		...
		Utility.ScrollToElement(folioLineItemsVoid.Balance, driver);
		String[] Balance = folioLineItemsVoid.Balance.getText().split(" ");
		return Balance[1] ;
	...
	public ArrayList<String> clickAddLineItemButton(WebDriver driver) throws InterruptedException {
		...
		try{
			...
			Utility.clickThroughJavaScript(driver, elements_FolioLineItemsVoid.AddLineItemButton);
		}
	...
	public ArrayList<String> AddFolioLineItem(WebDriver driver, String Category, String Amount)
		...
		String popup="//div[contains(text(),'You cannot change account')]/..//button[text()='Ok']";
	...
	public ArrayList<String> makePayment(WebDriver driver, String paymentType) throws InterruptedException {
		...
		try {
			...
			Utility.ScrollToElement(payment.paymentSubmitButton2, driver);
			Wait.waitForElementToBeClickable(By.xpath(OR.paymentSubmitButton2), driver);
			String payButtonText = payment.paymentSubmitButton2.getText();
			payment.paymentSubmitButton2.click();
			test_steps.add("Click (" + payButtonText + ") button");
			folioLogger.info("Click (" + payButtonText + ") button");
		}
		...
		Wait.wait10Second();
			...
			Wait.WaitForElement(driver, OR.paymentSuccessFullPopupClose2);
			Wait.waitForElementToBeVisibile(By.xpath(OR.paymentSuccessFullPopupClose2), driver);
			payment.paymentSuccessFullPopupClose2.click();
			folioLogger.info("Click close button");
			test_steps.add("Click close button");
			Wait.wait10Second();
	...
	public ArrayList<String> makePaymentAdvance(WebDriver driver, String paymentType, String PayAmount, String paymentMethod) throws InterruptedException {
		...
		Wait.WaitForElement(driver, OR.paymentAmountInput);
...
//		folioLogger.info("Selected option  : " + paymentType);
	...
	public ArrayList<String> selectGuestFolioSecondRoom(WebDriver driver, String Amount, ArrayList<String> test_steps)
		...
		try {
			...
			int count=driver.findElements(By.xpath("//span[contains(text(),'Guest Folio For ')]")).size();
			
			Wait.waitForElementToBeClickable(By.xpath("(//span[contains(text(),'Guest Folio For ')])["+count+"]"), driver);
	...
	public ArrayList<String> verifyLineItemDetails(WebDriver driver, String RoomNumber, String RoomCharge,
		...
		test_steps.add(" RoomCharge In ItemDetails :" + roomCharge);
		folioLogger.info(" RoomCharge In ItemDetails Actaul:" + roomCharge);
		folioLogger.info(" RoomCharge In ItemDetails Expected :" + RoomCharge);
	...
	public void clikOnSecondLineItemDescription(WebDriver driver, String category, int line,
		...
		Wait.waitForElementToBeVisibile(By.xpath(description), driver);
		driver.findElement(By.xpath(description)).click();
		test_steps.add("Click on line item description");
		folioLogger.info("Click on line item description");
	...
	public void ApplyRouting(WebDriver driver, ArrayList<String> test_steps) throws InterruptedException {
		...
		elements_MovieFolio.SelectAllLineitems.click();
			...
			String selectAllItems="(//input[contains(@data-bind,'click: selectAllLineItems')])";
			driver.findElement(By.xpath(selectAllItems)).click();
		}
		try {
		...
		elements_MovieFolio.ApplyRoutingButton.click();
			...
			try {
			String applyRouting="(//button[contains(text(),'Apply Routing')])[1]";
			driver.findElement(By.xpath(applyRouting)).click(); }
			catch (Exception e1) {
				String applyRouting="(//button[contains(text(),'Apply Routing')])[2]";
				driver.findElement(By.xpath(applyRouting)).click(); }
		}
		...
		folioLogger.info("Waited to loading symbol");
		// int size = Ele_FolioItem.LineItemsAmountFields.size();
		// System.out.println("Size:" + size);
		// String lineitemsize = Integer.toString(size);
		// assertTrue(lineitemsize.equals("0"), "Failed:Room Changes not moved
	...
	public ArrayList<String> selectGuestFolioAccount(WebDriver driver, String reservationNumber,
		...
		try {
			...
			FolioLineItems.SelectGuestFolioRoom.click();
			Wait.wait2Second();
			String guestFolioAccount = "(//span[contains(text(),'Guest Folio For')]/../..//li/a/span[contains(text(),'"+reservationNumber+"')])";
	...
	public ArrayList<String> ChangeFolio(WebDriver driver, String NewFolioName, String SelectFolioName)
		...
		String path1 = "(//span[contains(text(),'" + NewFolioName + "')]//parent::button/..)[1]";
		WebElement folioDropDown = driver.findElement(By.xpath(path1));
		...
		try {
			...
			Utility.clickThroughJavaScript(driver, folioDropDown);
			test_steps.add("Folio DropDown Clicked");
			folioLogger.info("Folio DropDown Clicked");
		}

		String liPath = "(//ul[@class='dropdown-menu inner']//span[contains(text(),'" + SelectFolioName + "')]//parent::a)[1]";
		...
		try {
			...
			Utility.clickThroughJavaScript(driver, liSelect);
			folioLogger.info("Clicked option: " + SelectFolioName);
		}
	...
	public ArrayList<String> addLineItemAccount(WebDriver driver, String SelectCategory, String Amount,
		...
		folioLogger.info("Selected property: " + selectProperty);
	...
	public void verifyAddedLineItem(WebDriver driver, String icon, String date, String category, String amount,
		...
		for (lineNumber = LineItem_Size - 1; lineNumber >= 0; lineNumber--) {
			...
			String IconSrc = Elements_FolioLineItemsVoid.AddedLineItems_Icon.get(lineNumber).getAttribute("class");
		...
		for (lineNumber = LineItem_Size - 1; lineNumber >= 0; lineNumber--) {
			...
			if (LineItemCategory.equalsIgnoreCase(Category) && LineItemAmount.contains(Amount)
				...
				if (clickIcon) {
					...
					for (int i = 0; i < 5; i++) {
						if (Elements_FolioLineItemsVoid.AddedLineItems_Icon.get(lineNumber).getAttribute("class")
								.contains(newIcon)) {
							break;
						}
					}
	...
	public String getLineItemAmount(WebDriver driver, String Category, String sourceIcon, boolean WithTax)
		...
		String Amount = driver
				.findElement(By.xpath("//i[contains(@class,'" + sourceIcon
...
try {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (moveFolio.SaveFolio.isEnabled()) {
				...
				try {
					...
					Utility.clickThroughJavaScript(driver, moveFolio.SaveFolio);
				}
		...
		test_steps.add("Attempt: Clicked on Save");
		folioLogger.info("Clicked on Save");
}catch (Exception e) {
	e.printStackTrace();
}
	...
	public ArrayList<String> MoveFolio(WebDriver driver, String FolioName) {
		Elements_MovieFolio moveFolio = new Elements_MovieFolio(driver);
		ArrayList<String> testStep  =  new ArrayList<String>();
		
		Wait.WaitForElement(driver, OR.MoveFolio_Move);
		moveFolio.MoveFolio_Move.click();
		testStep.add("Clicked on move");
		folioLogger.info("Clicked on move");
		...
		String ChooseItemxpath = "//select[@data-bind='value: $root.selectedFolioId']";
		Wait.WaitForElement(driver, ChooseItemxpath);
		driver.findElement(By.xpath(ChooseItemxpath)).click();
		testStep.add("Clicking On Choose");
		folioLogger.info("Clicking On Choose");
		
		Select sel = new Select(moveFolio.MoveFolio_TargetFolio);
		sel.selectByVisibleText(FolioName);
		testStep.add("Opened the new Folio : " + FolioName);
		folioLogger.info("Opened the new Folio : " + FolioName);
		...
		String SelectAllItemxpath = "//button[@data-bind='click: MoveAllToTarget, enable: SelectedFolio']";
		Wait.WaitForElement(driver, SelectAllItemxpath);
		driver.findElement(By.xpath(SelectAllItemxpath)).click();
		testStep.add("Moving All Item To "+FolioName);
		folioLogger.info("Moving All Item To "+FolioName);
	...
	public ArrayList<String> MoveFolioWithContainFolioName(WebDriver driver, String FolioName) {
		Elements_MovieFolio moveFolio = new Elements_MovieFolio(driver);
		ArrayList<String> testStep  =  new ArrayList<String>();
		...
		String OPtionxpath = "//optgroup//option[contains(text(),'"+FolioName+"')]";
		Wait.WaitForElement(driver, OPtionxpath);
		Wait.waitForElementToBeClickable(By.xpath(OPtionxpath), driver);
	...
	public void verifyLineItemIcon(WebDriver driver, ArrayList<String> test_steps, String category, String imgSRC,
		...
		for (WebElement roomsToSelect : rooms) {
			...
			String imgPah = "//td[contains(@data-bind,'lineitem-category')]/span[contains(text(),'" + category
					+ "')]/../preceding-sibling::td[contains(@class,'lineitems-changestatus')]/i";
		//	Wait.waitForElementToBeVisibile(By.xpath(imgPah), driver, 10);
			Wait.WaitForElement(driver, imgPah);
			/*foundSrc = driver.findElement(By.xpath(imgPah)).getAttribute("src");*/
			foundSrc = driver.findElement(By.xpath(imgPah)).getAttribute("class");
	...
	public void verifyLineItemIcon(WebDriver driver, ArrayList<String> test_steps, String category,	String msg) throws Exception {
		boolean iconFound = false;
		Wait.waitForElementToBeVisibile(
				By.xpath("//button[@class='btn dropdown-toggle btn-default'][contains(@title,'Guest Folio')]"), driver,
				30);
		...
		for (WebElement roomsToSelect : rooms) {
			...
			Wait.wait1Second();
			roomsToSelect.click();
			List<WebElement> icons = driver.findElements(By.xpath("(//td[@class='lineitem-category']/span[text()='"+category
		...
		assertEquals(iconFound, true, "Failed to Verify Icon");
		Utility.app_logs.info("Successfully Verified " + msg);
		test_steps.add("Successfully Verified " + msg);
	}
...
//		Wait.waitForElementToBeClickable(By.xpath(NewFolio.Foliotab), driver);
	...
	public String splitStringBySign(String str,String sign) {

		String[] strSplit = str.split("\\"+sign);
		String amount = strSplit[1].trim();
		return amount;
	}
	public String itemDate(WebDriver driver, String Category, int index)
			throws InterruptedException {

		String datePath = "(//span[text()='"+Category+"']//..//..//td[contains(@class,'lineitem-date')]//span)["+index+"]";
		System.out.println(datePath);
		WebElement element = driver.findElement(By.xpath(datePath));
		Utility.ScrollToElement_NoWait(element, driver);
		return element.getText();

	}
	public String itemDetailsDescroption(WebDriver driver, String Category, int index) throws InterruptedException {

		String path = "(//span[text()='"+Category+"']//..//..//a[contains(@data-bind,'text: $data.Description')])["+index+"]";
		WebElement element = driver.findElement(By.xpath(path));
		Utility.ScrollToElement_NoWait(element, driver);
		return element.getText();
	}

	public String itemAmount(WebDriver driver, String Category, int index) throws InterruptedException {
		String path = "(//span[text()='"+Category+"']//..//..//td[@class='lineitem-amount']//span[contains(@data-bind,'showInnroadCurrency:')])["+index+"]";
		WebElement element = driver.findElement(By.xpath(path));
		return element.getText();

	}
	
	public Double postedFirstLineItemAndGetAmount(WebDriver driver, ArrayList<String> test_steps) {
		
		Double outBoundAmount = 0.00;
		try {
			driver.findElement(By.xpath("(//td[contains(@class,'changestatus')])[1]")).click();
			ClickSaveFolioButton(driver);
			includeTaxinLIneItemCheckbox(driver, true);
			String amount = driver.findElement(By.xpath("//table[contains(@class,'table-foliogrid')]//tr/td[11]/span")).getText();
			outBoundAmount = Double.parseDouble(amount.split(" ")[1]);
		}catch(Exception e) {
			test_steps.add(e.toString());
			folioLogger.info(e.toString());
		}
		folioLogger.info("OutBound amount: "+outBoundAmount);
		return outBoundAmount;
	}
	
	public Double postedLastLineItemAndGetAmount(WebDriver driver, ArrayList<String> test_steps) {
		
		Double outBoundAmount = 0.00;
		try {
			List<WebElement> line = driver.findElements(By.xpath("(//td[contains(@class,'changestatus')])"));
			List<WebElement> amounts = driver.findElements(By.xpath("//table[contains(@class,'table-foliogrid')]//tr/td[11]/span"));
			
			if (line.size()>1) {
				line.get(line.size()-1).click();
				ClickSaveFolioButton(driver);
	...
	public void addCustomFolio(WebDriver driver, String customFolio,  String customFolioDescription, ArrayList<String> testSteps) throws InterruptedException {
		Elements_FolioLineItemsVoid moveFolio = new Elements_FolioLineItemsVoid(driver);		
		Elements_MovieFolio folio = new Elements_MovieFolio(driver);
		Wait.WaitForElement(driver, NewFolio.addCustomFolio);
		Wait.waitForElementToBeVisibile(By.xpath(NewFolio.addCustomFolio), driver);
		Wait.waitForElementToBeClickable(By.xpath(NewFolio.addCustomFolio), driver);
		Utility.ScrollToElement(moveFolio.addCustomFolio, driver);
		moveFolio.addCustomFolio.click();
		testSteps.add("Clicked on add custom folio icon");
		folioLogger.info("Clicked on add custom folio icon");
		
		Wait.WaitForElement(driver, NewFolio.customFolioName);
		Wait.waitForElementToBeVisibile(By.xpath(NewFolio.customFolioName), driver);
		Wait.waitForElementToBeClickable(By.xpath(NewFolio.customFolioName), driver);
		Utility.ScrollToElement(moveFolio.customFolioName, driver);
		moveFolio.customFolioName.clear();
		moveFolio.customFolioName.sendKeys(customFolio);
		testSteps.add("Entered custom folio name : " + customFolio);
		folioLogger.info("Entered custom folio name : " + customFolio);
		
		Wait.WaitForElement(driver, NewFolio.customFolioDescription);
		Wait.waitForElementToBeVisibile(By.xpath(NewFolio.customFolioDescription), driver);
		Wait.waitForElementToBeClickable(By.xpath(NewFolio.customFolioDescription), driver);
		Utility.ScrollToElement(moveFolio.customFolioDescription, driver);
		moveFolio.customFolioDescription.clear();
		moveFolio.customFolioDescription.sendKeys(customFolioDescription);
		testSteps.add("Entered custom folio description : " + customFolioDescription);
		folioLogger.info("Entered custom folio description : " + customFolioDescription);

		Wait.WaitForElement(driver, NewFolio.saveCustomFolio);
		Wait.waitForElementToBeVisibile(By.xpath(NewFolio.saveCustomFolio), driver);
		Wait.waitForElementToBeClickable(By.xpath(NewFolio.saveCustomFolio), driver);
		Utility.ScrollToElement(moveFolio.saveCustomFolio, driver);
		moveFolio.saveCustomFolio.click();
		testSteps.add("Clicked on save custom folio button");
		folioLogger.info("Clicked on save custom folio button");
		

		Wait.WaitForElement(driver, NewFolio.SaveFolio);
		Wait.waitForElementToBeVisibile(By.xpath(NewFolio.SaveFolio), driver);
		Wait.waitForElementToBeClickable(By.xpath(NewFolio.SaveFolio), driver);
		Utility.ScrollToElement(folio.SaveFolio, driver);
		folio.SaveFolio.click();
		testSteps.add("Clicked save folio button");
		folioLogger.info("Clicked save folio button");
	 ...
	 * ' Method Name: <makePayment> ' Description: <This method will make
	 * payment in reservation folio section> ' Input parameters: <WebDriver
	 * driver,String paymentType, String amount> ' Return value: <ArrayList<String>> ' Created
	 * By: <Muhammad Bakar> ' Created On: <12/14/2020>
	 * 
	 * #########################################################################
	 * #########################################################################
	 * ########################
	...
	public void makePayment(WebDriver driver, String paymentType, String amount, ArrayList<String> test_steps) throws InterruptedException {
		Elements_All_Payments payment = new Elements_All_Payments(driver);
		Elements_MovieFolio folio = new Elements_MovieFolio(driver);
		try {
			Wait.WaitForElement(driver, OR.PayButton);
			...
			Wait.WaitForElement(driver, OR.PayButton2);
			Wait.waitForElementToBeVisibile(By.xpath(OR.PayButton2), driver);
			Wait.waitForElementToBeClickable(By.xpath(OR.PayButton2), driver);
			Utility.ScrollToElement(payment.PayButton2, driver);
			payment.PayButton2.click();
			test_steps.add("Click payment button");
			folioLogger.info("Click payment button");
		...
		Wait.WaitForElement(driver, OR.paymentAmountInput);
		Wait.waitForElementToBeVisibile(By.xpath(OR.paymentAmountInput), driver);
		Wait.waitForElementToBeClickable(By.xpath(OR.paymentAmountInput), driver);
		Utility.ScrollToElement(payment.paymentAmountInput, driver);
		try {
		payment.paymentAmountInput.click();
		}catch (Exception e) {
			Utility.clickThroughJavaScript(driver, payment.paymentAmountInput);
		}


		payment.paymentAmountInput.sendKeys((Keys.chord(Keys.CONTROL,"a", Keys.DELETE)));
		payment.paymentAmountInput.sendKeys(amount);
		...
		Wait.waitForElementToBeVisibile(By.xpath(OR.paymentSuccessFullPopupClose), driver);
		payment.paymentSuccessFullPopupClose.click();
		folioLogger.info("Paid Amount: "+amount);
		test_steps.add("Paid Amount: "+amount);
		folioLogger.info("Click close button");
		test_steps.add("Click close button");
		Wait.wait10Second();
		...
		Utility.ScrollToElement(folio.SaveFolio, driver);
		folio.SaveFolio.click();
		test_steps.add("Clicked save folio button");
		folioLogger.info("Clicked save folio button");
	 ...
	 * ########################
	 * 
	 * ' Method Name: enterLineItemDate ' 
	 * Description: This method will enter line item date 
	 * Input parameters: WebDriver driver,ArrayList<String> testSteps, String date 
	 * Return value: void 
	...
	public void enterLineItemDate(WebDriver driver,ArrayList<String> testSteps, String date)
			throws InterruptedException {
		Elements_FolioLineItemsVoid moveFolio = new Elements_FolioLineItemsVoid(driver);		
		Wait.WaitForElement(driver, NewFolio.lineItemDate);
		Wait.waitForElementToBeVisibile(By.xpath(NewFolio.lineItemDate), driver);
		Wait.waitForElementToBeClickable(By.xpath(NewFolio.lineItemDate), driver);
		Utility.ScrollToElement(moveFolio.lineItemDate, driver);
		moveFolio.lineItemDate.clear();
		moveFolio.lineItemDate.sendKeys(date);
		testSteps.add("Date entered : " + date);
		folioLogger.info("Date entered : " + date);
	 ...
	 * Description: This method will add line item 
	 * Input parameters: WebDriver driver, ArrayList<String> testSteps,  String selectCategory, String amount,
			String quantity 
	 * Return value: void 
	 * Created By: Muhammad Bakar 
	 * Created On: 11-19-2020
	...
	public void addLineItem(WebDriver driver, ArrayList<String> testSteps, String date, String selectCategory, String amount,
			String quantity) throws InterruptedException, ParseException {
		...
		elements_FolioLineItemsVoid.LineItemDate.click();
		elements_FolioLineItemsVoid.LineItemDate.clear();
		elements_FolioLineItemsVoid.LineItemDate.sendKeys(date);
		Wait.wait1Second();
		...
		try {
			Wait.explicit_wait_elementToBeClickable(elements_FolioLineItemsVoid.SelectCategory, driver);
			new Select(elements_FolioLineItemsVoid.SelectCategory).selectByVisibleText(selectCategory);
			testSteps.add("Select category: " + selectCategory);
			folioLogger.info("Select category: " + selectCategory);
			
		}catch(Exception e) {
			WebElement categoryEle = driver.findElement(By.xpath("//select[contains(@data-bind, 'LedgerAccountName')]"));
			Wait.explicit_wait_elementToBeClickable
			(categoryEle, driver);
			new Select(categoryEle).selectByVisibleText(selectCategory);
			testSteps.add("Select category: " + selectCategory);
			folioLogger.info("Select category: " + selectCategory);
			
		}
		// elements_FolioLineItemsVoid.Enter_LineItemQuentity.clear();
		elements_FolioLineItemsVoid.Enter_LineItemQuentity.sendKeys(quantity);
		testSteps.add("Enter quentity: " + quantity);
		folioLogger.info("Enter quentity: " + quantity);
		String getQuantity = elements_FolioLineItemsVoid.Enter_LineItemQuentity.getAttribute("value");
		for (int i = 0; i < getQuantity.length(); i++) {
			if (i == 0) {
				System.out.println("in arrow key left");
		...
		elements_FolioLineItemsVoid.Enter_LineItemAmount.clear();
		elements_FolioLineItemsVoid.Enter_LineItemAmount.sendKeys(amount);
		testSteps.add("Enter amount: " + amount);
		folioLogger.info("Enter amount: " + amount);
		Wait.explicit_wait_visibilityof_webelement(elements_FolioLineItemsVoid.CommitButton, driver);
		Utility.ScrollToElement(elements_FolioLineItemsVoid.CommitButton, driver);
		elements_FolioLineItemsVoid.CommitButton.click();
		folioLogger.info("Click on Commit button");
		testSteps.add("Click on Commit button");
	 ...
	 * ########################
	 * 
	 * ' Method Name: getToastMessageSize ' 
	 * Description: This method will return size of toast message 
	...
	public int getToastMessageSize(WebDriver driver)
			throws InterruptedException {
		Elements_FolioLineItemsVoid moveFolio = new Elements_FolioLineItemsVoid(driver);		
		int size = moveFolio.toastMessage.size();
		return size;
	}
	
	public int getLockedItemsSize(WebDriver driver) {
		String path = "//i[contains(@class, 'transactionstatus-icon5')]";
		List<WebElement> lineItemState = driver.findElements(By.xpath(path));
		int size = lineItemState.size(); 
		folioLogger.info("Locked items : " + size);
		return size;
	}

	public int getAllLineItemsSize(WebDriver driver)
			throws InterruptedException {
		Elements_FolioLineItemsVoid moveFolio = new Elements_FolioLineItemsVoid(driver);		
		int size = moveFolio.allLineItemPath.size();
		folioLogger.info("All Line items : " + size);
		return size;
	}
	 ...
	 * ########################
	 */
	public void verifyLineItemClickDescription(WebDriver driver, String Category,
			String Description, boolean clickDescription) throws InterruptedException {
		// Verify payment
		Elements_FolioLineItemsVoid Elements_FolioLineItemsVoid = new Elements_FolioLineItemsVoid(driver);
		Wait.explicit_wait_visibilityof_webelement_350(Elements_FolioLineItemsVoid.AddedLineItems_Category.get(0),
		...
		for (lineNumber = 0; lineNumber < LineItem_Size; lineNumber++) {
			...
			if (LineItemCategory.equalsIgnoreCase(Category) &&  desc.replaceAll(" ", "").contains(Description.replaceAll(" ", ""))) {
				Wait.waitForElementToBeClickable(By.xpath(NewFolio.AddedLineItems_Description), driver);
				Utility.ScrollToElement(Elements_FolioLineItemsVoid.AddedLineItems_Description.get(lineNumber), driver);
				found = true;
				if (clickDescription) {
	...
	public void VerifyPaymentPopup_LineItemWithIcon(WebDriver driver, String category, String Amount,
			String Description, boolean link) throws InterruptedException {
		// Verify payment
		String Desc_Path = "//tbody//span[contains(@data-bind,'data.Category') and text()='"
				+ category + "']//parent::td//parent::tr//span[contains(@data-bind,'data.Description')]";
		if (link) {
			Desc_Path = "//tbody//span[contains(@data-bind,'data.Category') and text()='"
					+ category + "']//parent::td//parent::tr//span[contains(@data-bind,'data.Description')]/../a";
		}
		String AmountPath = "//tbody//span[contains(@data-bind,'data.Category') and text()='"
				+ category + "']//parent::td//parent::tr//span[contains(@data-bind,'data.Amount')]";
		...
		folioLogger.info("LineNumber : " + lineNumber);
		boolean found = false;
		for (lineNumber = 0; lineNumber <= LineItem_Size - 1; lineNumber++) {

			Utility.ScrollToElement(Desc.get(lineNumber), driver);
			String desc = Desc.get(lineNumber).getText();
			String LineItemAmount = Amounts.get(lineNumber).getText();
			folioLogger.info("LineNumber : " + lineNumber);
			folioLogger.info("Amount : " + LineItemAmount);
			folioLogger.info("Description : " + desc);
			if (LineItemAmount.contains(Amount) && desc.replaceAll(" ", "").contains(Description.replaceAll(" ", ""))
					) {
				found = true;
				break;
			}	
	...
	//Added By Adhnan 01/25/2020
	public void verifyLineItemAdded(WebDriver driver, String Category, String Amount, 
			String Description, boolean exist)
			throws InterruptedException {
		// Verify payment
		Elements_FolioLineItemsVoid Elements_FolioLineItemsVoid = new Elements_FolioLineItemsVoid(driver);
		Wait.explicit_wait_visibilityof_webelement_350(Elements_FolioLineItemsVoid.AddedLineItems_Category.get(0),
		...
		assertEquals(found,exist, "Failed: Required item not in required visibility status");
		
	}

	
	//Added By Adhnan Ghaffar 02/15/2021
	public void verifyLineItemClickDescription(WebDriver driver, String Category,
			String Description, boolean clickDescription,int matchingLineNumber) throws InterruptedException {
		// Verify payment
		Elements_FolioLineItemsVoid Elements_FolioLineItemsVoid = new Elements_FolioLineItemsVoid(driver);
		Wait.explicit_wait_visibilityof_webelement_350(Elements_FolioLineItemsVoid.AddedLineItems_Category.get(0),
				driver);
		...
		for (lineNumber = 0; lineNumber < LineItem_Size; lineNumber++) {
			...
			if (LineItemCategory.equalsIgnoreCase(Category) &&  desc.replaceAll(" ", "").contains(Description.replaceAll(" ", ""))) {
				Wait.waitForElementToBeClickable(By.xpath(NewFolio.AddedLineItems_Description), driver);
				Utility.ScrollToElement(Elements_FolioLineItemsVoid.AddedLineItems_Description.get(lineNumber), driver);
				foundSimilarLineItems++;
				if(foundSimilarLineItems==matchingLineNumber) {
				if (clickDescription) {
					found = true;
					Utility.ScrollToElement(Elements_FolioLineItemsVoid.AddedLineItems_Description.get(lineNumber),
							driver);
	...
	//Added By Adhnan 02/15/2020
		public void verifyLineItemAdded(WebDriver driver, String Category, String Amount, 
				String Description, boolean exist, int requiredSameLineItem)
				throws InterruptedException {
			// Verify payment
			Elements_FolioLineItemsVoid Elements_FolioLineItemsVoid = new Elements_FolioLineItemsVoid(driver);
			Wait.explicit_wait_visibilityof_webelement_350(Elements_FolioLineItemsVoid.AddedLineItems_Category.get(0),
			...
			for (lineNumber = 0; lineNumber < LineItem_Size; lineNumber++) {
				...
				if (LineItemCategory.equalsIgnoreCase(Category) && LineItemAmount.contains(Amount)
					...
					Utility.ScrollToElement(Elements_FolioLineItemsVoid.AddedLineItems_Description.get(lineNumber), driver);
					foundSimilarLineItems++;
					if(foundSimilarLineItems==requiredSameLineItem) {
					found = true;
					break;
					}
				}
	...
	public void verifyAdvanceDepositRealized(WebDriver driver, String Category, ArrayList<String> testSteps)
			throws InterruptedException {
		Elements_FolioLineItemsVoid moveFolio = new Elements_FolioLineItemsVoid(driver);		
		testSteps.add("===== Verifying advance deposit realized in payment line item popup =====".toUpperCase());
		folioLogger.info("===== Verifying advance deposit realized in payment line item popup =====".toUpperCase());

		String categoryPath = "(//span[@data-bind='text: $data.Category' and text()='"+ Category+"']//..//following-sibling::td[contains(@class,'lineitem-description')]//a)[1]";
		WebElement categoryElement = driver.findElement(By.xpath(categoryPath));
		Utility.ScrollToElement(categoryElement, driver);
		categoryElement.click();
		testSteps.add("Clicked line item with category : " + Category);
		folioLogger.info("Clicked line item with category : " + Category);
		
		Wait.WaitForElement(driver, NewFolio.advancedDepositRealized);
		Wait.waitForElementToBeVisibile(By.xpath(NewFolio.advancedDepositRealized), driver);
		Wait.waitForElementToBeClickable(By.xpath(NewFolio.advancedDepositRealized), driver);
		Utility.ScrollToElement(moveFolio.advancedDepositRealized, driver);

		assertTrue(moveFolio.advancedDepositRealized.isDisplayed(), "Failed : Advanced deposit is not realized from night audit");
		testSteps.add("Verified advance deposit realized in payment line item popup");
		folioLogger.info("Verified advance deposit realized in payment line item popup");

		moveFolio.closeItemDetailsPopup.click();
		testSteps.add("Click close payment detail popup");
		folioLogger.info("Click close payment detail popup");
		
	}
	

	public ArrayList<String>  verifyPostStatus(WebDriver driver,ArrayList<String> Test_Steps){
		Elements_FolioLineItemsVoid FolioLineItems = new Elements_FolioLineItemsVoid(driver);
		if (FolioLineItems.verifyLineItemPostStatus.isDisplayed() == true) {
			Test_Steps.add("Verified line item realization after checkin");
			
		}
		else {
			Test_Steps.add("Realized line item not found");
		}
		return Test_Steps;
		
	}
	
	public void verifyLineItemState(WebDriver driver, ArrayList<String> testSteps, String icon, int index, boolean isDisplay) {
		String path = "(//td[contains(@class, 'lineitems-changestatus')]//i[contains(@class, '"+ icon +"')])["+ index +"]";
		if(isDisplay) {
			testSteps.add("Verify Line item at index "+ index + " containing class '" + icon + "'is showing");
			assertTrue(Utility.isElementDisplayed(driver, By.xpath(path)), "Failed : Line item at index "+ index + " containing class '" + icon + "'is not showing");			
			testSteps.add("Verified Line item at index "+ index + " containing class '" + icon + "'is showing");
		}else {
			testSteps.add("Verify Line item at index "+ index + " containing class '" + icon + "'is not showing");
			assertTrue(Utility.isElementDisplayed(driver, By.xpath(path)), "Failed : Line item at index "+ index + " containing class '" + icon + "'is showing");			
			testSteps.add("Verified Line item at index "+ index + " containing class '" + icon + "'is not showing");
	...
	public void enterSwipeCardDetailsInFolio(WebDriver driver, ArrayList<String> testSteps, String cardNumberString) throws InterruptedException {
		Elements_All_Payments elements = new Elements_All_Payments(driver);
		Wait.waitForElementByXpath(driver, OR.PayButton);
		WebElement element = driver.findElement(By.xpath(OR.PayButton));
		element.click();
		testSteps.add("Click on pay button");
		Wait.waitForElementByXpath(driver, NewFolio.swipeButton);
		elements.swipeButton.click();
		testSteps.add("Click on swipe button");
			
		Wait.waitForElementByXpath(driver, NewFolio.creditCardNumber);
		elements.creditCardNumber.sendKeys(cardNumberString);
		testSteps.add("Entered Card number string '"+ cardNumberString +"'");
		Wait.wait2Second();

	}
	public void clickPayButton(WebDriver driver, ArrayList<String> testSteps) throws InterruptedException {
		Elements_All_Payments elements = new Elements_All_Payments(driver);
		
		
		try {
			Wait.waitForElementByXpath(driver, NewFolio.payButton1);
			Utility.ScrollToElement(elements.payButton1, driver);
			elements.payButton1.click();
		
		} catch (Exception e) {
			Wait.waitForElementByXpath(driver, NewFolio.payButton);
			Utility.ScrollToElement(elements.payButton, driver);
			elements.payButton.click();
		}
		
		testSteps.add("Payment amount : " + Utility.getElementText(elements.PayButton));
		testSteps.add("Clicked Pay button");
	}
	public void clickClosePaymentPopupButton(WebDriver driver, ArrayList<String> testSteps) throws InterruptedException {
		Elements_All_Payments elements = new Elements_All_Payments(driver);
		Wait.waitForElementByXpath(driver, NewFolio.closePaymentPopup);
		Utility.ScrollToElement(elements.closePaymentPopup, driver);
		elements.closePaymentPopup.click();
		testSteps.add("Clicked close button");
	}
	
	public void verifyMCCardFields(WebDriver driver, ArrayList<String> testSteps, String cardName, String cardNumber, String cardExpiry) {
		Elements_All_Payments elements = new Elements_All_Payments(driver);
		testSteps.add("Verify card Name");
		Wait.waitForElementByXpath(driver, NewFolio.nameOnCard);
		String actualNameOnCard = Utility.getELementText(elements.nameOnCard, true, "value");
		Utility.verifyEquals(actualNameOnCard, cardName, testSteps);
		
		testSteps.add("Verify card Number");
		...
		Wait.waitForElementByXpath(driver, NewFolio.creditCardNumberInput);		
		String actualCardNumber = Utility.getELementText(elements.creditCardNumberInput, true, "value");
		String lastFourDigits2 = "";		 
		if (actualCardNumber.length() > 4) 
		{
			lastFourDigits2 = actualCardNumber.substring(actualCardNumber.length() - 4);
		} 
		else
		{
			lastFourDigits2 = actualCardNumber;
		}
		Utility.verifyEquals(lastFourDigits2, lastFourDigits, testSteps);

		testSteps.add("Verify card expiry");
		Wait.waitForElementByXpath(driver, NewFolio.cardExpiry);
		String actualCardExpiry = Utility.getELementText(elements.cardExpiry, true, "value");
		Utility.verifyEquals(actualCardExpiry, cardExpiry, testSteps);
		
	}

	public ArrayList<String> makePaymentFromOtherReservation(WebDriver driver, String PayAmount, String paymentMethod, String reservationNumber) throws InterruptedException {
		Elements_All_Payments payment = new Elements_All_Payments(driver);
		ArrayList<String> test_steps = new ArrayList<>();
		System.out.println(paymentMethod);
		...
		Wait.waitForElementByXpath(driver, OR.paymentMethod);
		payment.paymentMethod.click();
		String Option = "(//span[contains(text(),'" + paymentMethod + "')])[last()]";
		Utility.printString(Option);
		Wait.waitForElementToBeVisibile(By.xpath(Option), driver);
		driver.findElement(By.xpath(Option)).click();
		
		Wait.waitForElementByXpath(driver, NewFolio.enterReservationNumberInPaymentPopup);
		payment.enterReservationNumberInPaymentPopup.sendKeys(reservationNumber);
	...
	public ArrayList<String> verifyLineItemAmountIsNegative(WebDriver driver, String Category, String Amount) {
		ArrayList<String> test_steps = new ArrayList<String>();
		String amountPath = "//td[contains(@data-bind,'lineitem-category')]/span[contains(text(),'" + Category
				+ "')]/../following-sibling::td[contains(@class,'lineitem-amount')]/span";
		Wait.waitForElementByXpath(driver, amountPath);
		String foundAmount = driver.findElement(By.xpath(amountPath)).getText();
		foundAmount = foundAmount.replace("$", "");
		foundAmount = foundAmount.replace(" ", "");
		foundAmount  = Utility.removeDollarBracketsAndSpaces(foundAmount);
		assertEquals(Float.parseFloat(foundAmount), - Float.parseFloat(Amount), "Failed to Verify Amount");
		Utility.app_logs.info("Successfully Verified Amount : " + foundAmount);
		test_steps.add("Successfully Verified Amount : " + foundAmount);
		return test_steps;
	...
	public ArrayList<String> verifyReservationWithCacnelStausIsNotShowingWhileMakePayment(WebDriver driver, String PayAmount, String paymentMethod, String reservationNumber) throws InterruptedException {
		Elements_All_Payments payment = new Elements_All_Payments(driver);
		ArrayList<String> test_steps = new ArrayList<>();
		System.out.println(paymentMethod);
		...
		test_steps.add("Verify Searching Reservation with status cancelled '"+ reservationNumber +"' don't return any results");
		Wait.waitForElementByXpath(driver, NewFolio.enterReservationNumberInPaymentPopup);
		payment.enterReservationNumberInPaymentPopup.sendKeys(reservationNumber);
		test_steps.add("Entered reservation number : " + reservationNumber);
		Wait.wait3Second();
		String path = "(//input[@name='reservation-autocomplete'])[last()]//following::div[1]//ul[contains(@class,'autocomplete')]//li[1]";
		try {
			test_steps.add("Verified Searching Reservation with status cancelled '"+ reservationNumber +"' don't return any results");
			
		}catch (Exception e) {
			String reservationToSelect = "//b[text()='"+ reservationNumber +"']/../../..//parent::li";
			assertTrue(Utility.isElementDisplayed_1(driver, By.xpath(reservationToSelect)), "Failed : Returned results contianed cancelled reservation");
			test_steps.add("Verified searched reservation with status cancelled '"+ reservationNumber +"' is not showing in returned results");			
		}
	
		return test_steps;
	}
	
	public void clickClosePaymentPopupIcon(WebDriver driver, ArrayList<String> testSteps) {
		
		Elements_All_Payments payment = new Elements_All_Payments(driver);
		Wait.waitForElementByXpath(driver, NewFolio.closePaymentPopupIcon);
		payment.closePaymentPopupIcon.click();
		testSteps.add("Clicked close payment popup");
		
	}

public String getRoomNo_OnCheckOutAll(WebDriver driver, String RowCount, ArrayList<String> testSteps) {
	String RoomNoxpath = "(//span[contains(@data-bind, 'text: roomNumber')])[" + RowCount +"]"; 
	Wait.WaitForElement(driver, RoomNoxpath);
	String RoomNoText = driver.findElement(By.xpath(RoomNoxpath)).getText();
	return RoomNoText;
}

public int getLineItemRowsCount(WebDriver driver) throws InterruptedException {

	Elements_FolioLineItemsVoid elemetFolio = new Elements_FolioLineItemsVoid(driver);
	// Utility.ScrollToElement(elemetFolio.FolioRows.get(0), driver);
	int size = 0;
	try {
		size = elemetFolio.FolioRowsCount.size();
	} catch (Exception e) {
		// TODO: handle exception
	}
	return size;
...
public void Verify_LineItem_(WebDriver driver, String Date, String Category,ArrayList<String> testStep, boolean isFound) {
	// Verify payment
	...
	int LineItem_Size = Elements_FolioLineItemsVoid.AddedLineItems_Category.size();
	folioLogger.info("LineItem_Size : " + LineItem_Size);
	testStep.add("LineItem_Size : " + LineItem_Size);
	int lineNumber = LineItem_Size - 1;
	folioLogger.info("LineNumber : " + lineNumber);
	testStep.add("LineNumber : " + lineNumber);
	boolean found = false;
	testStep.add("**************Verify Folio LineItem***************");
	for (lineNumber = LineItem_Size - 1; lineNumber >= 0; lineNumber--) {
		folioLogger.info("LineNumber : " + lineNumber);
//		testStep.add("LineNumber : " + lineNumber);
		folioLogger.info(
				...
				) 
		{
			...
			testStep.add(
					"Category : " + Elements_FolioLineItemsVoid.AddedLineItems_Category.get(lineNumber).getText());
			testStep.add("Amount : " + Elements_FolioLineItemsVoid.AddedLineItems_Amount.get(lineNumber).getText());
			testStep.add("Description : "
					+ Elements_FolioLineItemsVoid.AddedLineItems_Description.get(lineNumber).getText());
			
			
			testStep.add("Expected Category:  "+ Category);
			testStep.add("Actual Category:  "+ Elements_FolioLineItemsVoid.AddedLineItems_Category.get(lineNumber).getText());
			testStep.add("Expected Date:  "+ Date);
			testStep.add("Actual Date:  "+ Elements_FolioLineItemsVoid.AddedLineItems_Date.get(lineNumber).getText());
			found = true;
			break;
		}
	}
	...
	if(isFound) {
		assertTrue(found, "");
		testStep.add("Verify: Line Item having Category: " + Category + "  found against "+Date);
	}else {
		Assert.assertFalse(false);
		testStep.add("Verify: Line Item having Category: " + Category + " Not found against "+Date);
...
public ArrayList<String> AccountReservationMoveFolioInsideReservation(WebDriver driver, ExtentTest test1, String resNumber1,
		String resNumber2, ArrayList<String> test_steps) throws Exception {
	...
	Wait.wait5Second();
	Wait.WaitForElement(driver, "(//div[@class='sec_nav_in container']/ul/li//span[contains(text(),'VerifyPol')])[1]");
	driver.findElement(By.xpath("(//div[@class='sec_nav_in container']/ul/li//span[contains(text(),'VerifyPol')])[1]")).click();
	Wait.wait2Second();
	moveFolio.FirstOpenedReservation.click();
	Wait.wait2Second();
	Utility.refreshPage(driver);
...
//	moveFolio.MoveFolio_Folio.click();
	...
	String CloseButtonxpath = "//button[@data-bind = 'visible: !showLoading(), click: SaveToServer']";
	Wait.WaitForElement(driver, CloseButtonxpath);
	driver.findElement(By.xpath(CloseButtonxpath)).click();
...
//	reslogger.info("Folio Detail Tab is Enabled");
	...
	String MoveButtonxpath = "(//button[contains(text(),'Move')])[2]";
	String MoveButtonxpath2 = "(//button[contains(text(),'Move')])";
	
	try {
		Wait.WaitForElement(driver, MoveButtonxpath2);
		driver.findElement(By.xpath(MoveButtonxpath2)).click();
		test_steps.add("Clicked on move");
		folioLogger.info("Clicked on move");
		Wait.wait2Second();
	} catch (Exception e) {
		Wait.WaitForElement(driver, MoveButtonxpath);
		driver.findElement(By.xpath(MoveButtonxpath)).click();
		test_steps.add("Clicked on move");
		folioLogger.info("Clicked on move");
		Wait.wait2Second();
	}
	...
	Select sel = new Select(moveFolio.MoveFolio_TargetFolio);
	//sel.selectByVisibleText("Guest Folio");
	List<WebElement> optionsElements=sel.getOptions();
	WebElement element=optionsElements.get(optionsElements.size()-1);
	element.click();
	test_steps.add("Opened the new Folio : " + element.getText());
	folioLogger.info("Opened the new Folio : " + element.getText());
...
public ArrayList<String> MoveFolioInsideReservation_(WebDriver driver, ExtentTest test1, String resNumber1,
		String resNumber2, ArrayList<String> test_steps) throws Exception {
...
//	String xpath = "(//span[contains(text(),'Reservations')])[4]";
//	Wait.WaitForElement(driver,xpath);
////	// Search and open Reservation2
//	Wait.waitForElementToBeClickable(By.xpath(xpath), driver);
//	driver.findElement(By.xpath(xpath)).click();
...
//		reslogger.info("Folio Detail Tab is Enabled");
		...
		//Folio Tab	
				...
				Wait.wait5Second();
				Elements_CPReservation res = new Elements_CPReservation(driver);
				String pathx = "(//li[contains(@data-bind,'Folio')]//a[text()='Folio(s)']/..)[2]";
				Wait.WaitForElement(driver, pathx);
...
public void VoidActionAddOn(WebDriver driver, String Category, ArrayList<String> testSteps, String VoidText) {
		
		String ItemLinexpath = "//span[text()='"+Category+"']//..//..//td[1]";
		Wait.WaitForElement(driver, ItemLinexpath);
		Wait.waitForElementToBeClickable(By.xpath(ItemLinexpath), driver);
		driver.findElement(By.xpath(ItemLinexpath)).click();
		testSteps.add("Checking "+Category+" check box");
		
		String VoidButtonxpath = "(//button[contains(text(),'Void')])[1]";
		Wait.WaitForElement(driver, VoidButtonxpath);
		Wait.waitForElementToBeClickable(By.xpath(VoidButtonxpath), driver);
		driver.findElement(By.xpath(VoidButtonxpath)).click();
		testSteps.add("Clicking On Void Button");
		
//		String headingxpath = "//h4[@class='modal-title' and text()='Notes']";
//		Wait.WaitForElement(driver, headingxpath);
//		String headingText = driver.findElement(By.xpath(headingxpath)).getText();
//		Assert.assertEquals("Notes", headingText,"Failed: Popup Heading is Mismatch");
		
		String VoidNotexpath = "(//textarea[@data-bind='value: notesPopUpNote'])";
		Wait.WaitForElement(driver, VoidNotexpath);
		driver.findElement(By.xpath(VoidNotexpath)).sendKeys(VoidText);
		testSteps.add("Adding Text in Void Pop up Note Area");
		
		String SubmitVoidButtonxpath = "//button[@data-bind='click: NotesSaveClick']";
		Wait.WaitForElement(driver, SubmitVoidButtonxpath);
		Wait.waitForElementToBeClickable(By.xpath(SubmitVoidButtonxpath), driver);
		driver.findElement(By.xpath(SubmitVoidButtonxpath)).click();
		testSteps.add("Clicking On Void Button to Continue");
		
	}


public String getVoidItemLineText(WebDriver driver,ArrayList<String> testSteps) {
	String TextAreaxpath = "(//textarea[@data-bind='value: $parent.NewFolioNotes'])";
	Wait.WaitForElement(driver, TextAreaxpath);
	String text = driver.findElement(By.xpath(TextAreaxpath)).getAttribute("value");
	testSteps.add("Getting Value From Voided Text: "+text);
	
	
	String cancelButtonxpath = "(//button[@class = 'btn blue kbEvent13'])[5]";
	Wait.WaitForElement(driver, cancelButtonxpath);
	Wait.waitForElementToBeClickable(By.xpath(cancelButtonxpath), driver);
	driver.findElement(By.xpath(cancelButtonxpath)).click();
	testSteps.add("Clicking On Cancel Button");
	return text;
}



	public void verifyPostPendingStatus(WebDriver driver,ArrayList<String> test_Steps, String date, String typeOFFolioStatus){
		date= Utility.parseDate(date, "dd/MM/yyyy", "E MMM dd, yyyy");
		String path=null;
		if(typeOFFolioStatus.equalsIgnoreCase("Post")) {
		 path= "//i[@class='inncenter-icons-collection transactionstatus-icon2']"
				+ "/../following-sibling::td[@class='lineitem-date']/span[contains(text(),'"+date+"')]";
		 Wait.WaitForElement(driver, path);
			assertTrue(driver.findElement(By.xpath(path)).isDisplayed(), "Failed to realization after checkin");
			test_Steps.add("Verified line item realization after checkin");
			folioLogger.info("Verified line item realization after checkin");
		
		}else if(typeOFFolioStatus.equalsIgnoreCase("Pending")) {
			 path= "//i[@class='inncenter-icons-collection transactionstatus-icon1']"
						+ "/../following-sibling::td[@class='lineitem-date']/span[contains(text(),'"+date+"')]";	
			 Wait.WaitForElement(driver, path);
				assertTrue(driver.findElement(By.xpath(path)).isDisplayed(), "Failed to realization before checkin");
				test_Steps.add("Verified line item realization before checkin");
				folioLogger.info("Verified line item realization before checkin");
		}else if(typeOFFolioStatus.equalsIgnoreCase("Authorization")) {
			path= "//i[@class='inncenter-icons-collection transactionstatus-icon7']"
					+ "/../following-sibling::td[@class='lineitem-date']/span[contains(text(),'"+date+"')]";	
		 Wait.WaitForElement(driver, path);
			assertTrue(driver.findElement(By.xpath(path)).isDisplayed(), "Failed to verify authorization icon");
			test_Steps.add("Verified line item authorization icon");
			folioLogger.info("Verified line item authorization icon");
		}
		
	 }
	
	public void editFolioLineItemRoomChargeDateWise(WebDriver driver,ArrayList<String> test_Steps, String date, String category, String description) throws InterruptedException {
		date= Utility.parseDate(date, "dd/MM/yyyy", "E MMM dd, yyyy");
		String path= "//tbody[contains(@data-bind,'ComputedFolioItems')]//td[@class='lineitem-date']"
				+ "/span[contains(text(),'"+date+"')]/../following-sibling::td[@class='lineitem-rooms']"
				+ "/span[contains(text(),'776')]/../following-sibling::td[@class='lineitem-category']/span[contains(text(),'"+category+"')]"
						+ "/../following-sibling::td[contains(@class,'lineitem-description')]/a[contains(text(),'"+description+"')]";
			Wait.WaitForElement(driver, path);
			Utility.ScrollToElement(driver.findElement(By.xpath(path)), driver);
			driver.findElement(By.xpath(path)).click();
			test_Steps.add("Click on " + category);
			folioLogger.info("Click on " + category);
			 
	}
	public void addItemDetail(WebDriver driver,ArrayList<String> test_Steps,String adjustLineItem, String adjustamount, String desc, String itemDesc) throws InterruptedException {
		Elements_CPReservation res = new Elements_CPReservation(driver);
		Wait.explicit_wait_10sec(res.itemDetailOnFolioPopup, driver);
	        ...
	        folioLogger.info("Select Catgory " + adjustLineItem);
	        ...
	        folioLogger.info("Entered Description:  " + itemDesc);
	        ...
	        folioLogger.info("Entered Amount:  " + adjustamount);       
			...
			folioLogger.info("Enter Void notes as :" +itemDesc);
	        ...
	        folioLogger.info("Click Continue Button");
	        ...
	        folioLogger.info("Click on Save Folio");
	        String loading = "(//div[@class='ir-loader-in'])";
	        int count = 0;
	        while (true) {
	            if (!driver.findElement(By.xpath(loading)).isDisplayed()) {
...
public void SaveFolioButton(WebDriver driver,ArrayList<String>  test_steps) {
	String saveFolio = "//div[contains(@data-bind,'Folio')]//button[text()='Save']";
...
@SuppressWarnings("deprecation")
public void VerifyFolioDatesSort(WebDriver driver, ArrayList<String> testSteps) throws ParseException {
	List<Integer> date_ =  new ArrayList<Integer>();
	
	String xpathRo = "(//tr[@data-bind = 'if: $parent.ShouldShowThisItem($data)']//td[4])";
	
	
	List<WebElement> web_elements =driver.findElements(By.xpath(xpathRo));
	int sizeOfList  = web_elements.size();
	folioLogger.info("First Day: "+ESTTimeZone.getDateBaseOnDate(web_elements.get(0).getText(), "EEE MMM dd, yyyy", "dd"));
	int First_day = Integer.valueOf(ESTTimeZone.getDateBaseOnDate(web_elements.get(0).getText(), "EEE MMM dd, yyyy", "dd"));
	int Last_day = Integer.valueOf(ESTTimeZone.getDateBaseOnDate(web_elements.get(sizeOfList-1).getText(), "EEE MMM dd, yyyy", "dd"));
	
	folioLogger.info(sizeOfList);
	testSteps.add("<b>===========Folio Date\"===========<b>");
	int count =0;
	for(int i = 0 ; i <sizeOfList; i++) {
		
		folioLogger.info(web_elements.get(i).getText());
		testSteps.add((i+1)+" Folio Date: "+web_elements.get(i).getText());
		int day = Integer.valueOf(ESTTimeZone.getDateBaseOnDate(web_elements.get(i).getText(), "EEE MMM dd, yyyy", "dd")) ;
		date_.add(day);
		//assertEquals(String.valueOf(First_day+count) , String.valueOf(day),"Failed : to match date");
		count++;
	}

	
	testSteps.add("All Folio Item Lines Are Sorted w.r.t <b>Folio Date<b>");
	
	
	
}

public void SelectCategory(WebDriver driver, ArrayList test_steps, String LineItemCategory) throws InterruptedException {
	test_steps.add("******************* Select Folio LineItem Category**********************");
	folioLogger.info("******************* Select Folio LineItem Category**********************");
	...
	String[] item = LineItemCategory.split("\\|");
		...
		folioLogger.info("Adding line item to folio : " + LineItemCategory);
}


public void AddPackageTypeFolioWithQuantity(WebDriver driver, ArrayList<String> testStep, String Quantity ) throws InterruptedException {
	clickAddButton(driver);
	SelectCategory(driver, testStep, "Package");
	ClickOnThreeDotPackagecategoryFolioLineItem(driver, testStep);
	
	String PopupHeadingxpath = "//h4[contains(text(),'Package Picker')]";
	Wait.WaitForElement(driver, PopupHeadingxpath);
	String Popupheading = driver.findElement(By.xpath(PopupHeadingxpath)).getText();
	Assert.assertEquals(Popupheading.trim(), "Package Picker");
	testStep.add("Verified: Popup Heading "+Popupheading.trim());
	Wait.wait2Second();
	String inputxpath = "//input[@data-bind = 'value: $data.Quantity']";
	Wait.WaitForElement(driver, inputxpath);
	driver.findElement(By.xpath(inputxpath)).sendKeys(Quantity);
	Wait.wait2Second();
	String SelectButtonxpath = "//button[@data-bind = 'click: PackagePickerSelectClick']";
	Wait.WaitForElement(driver, SelectButtonxpath);
	Wait.waitForElementToBeClickable(By.xpath(SelectButtonxpath), driver);
	driver.findElement(By.xpath(SelectButtonxpath)).click();
	testStep.add("Clicking On Select Button");
}

public void VerifyPackageCategoryFolioPopup(WebDriver driver, ArrayList<String> test_Steps) {
	String PopupHeadingxpath = "//h4[contains(text(),'Package Picker')]";
	Wait.WaitForElement(driver, PopupHeadingxpath);
	String Popupheading = driver.findElement(By.xpath(PopupHeadingxpath)).getText();
	Assert.assertEquals(Popupheading.trim(), "Package Picker");
	test_Steps.add("Verified: Popup Heading "+Popupheading.trim());
	
	String CancelButtonxpath = "//button[@data-bind = 'click: PackagePickerCancelClick']";
	Wait.WaitForElement(driver, CancelButtonxpath);
	Wait.waitForElementToBeClickable(By.xpath(CancelButtonxpath), driver);
	driver.findElement(By.xpath(CancelButtonxpath)).click();
	test_Steps.add("Clicking On Cancel Button");
}
public void ClickOnThreeDotPackagecategoryFolioLineItem(WebDriver driver, ArrayList<String>testSteps) {
	String threedotsButtonxpath = "(//button[contains(text(),'...' )])[1]";
	Wait.WaitForElement(driver, threedotsButtonxpath);
	Wait.waitForElementToBeClickable(By.xpath(threedotsButtonxpath), driver);
	driver.findElement(By.xpath(threedotsButtonxpath)).click();
}

public void CancelReservation(WebDriver driver, ArrayList<String> testSteps , String Amount) throws InterruptedException {
	Thread.sleep(1000);
	Reservation_Status_DropDown(driver);
	Thread.sleep(1000);
	Cancel_Reservation_Status_DropDown(driver);
	Thread.sleep(1000);
	Cancel_Reservation_PopUp(driver, testSteps);
	Thread.sleep(1000);
	Verify_Cancel_Reservation_PopUp(driver, testSteps, Amount);
}


public void verifyFolioLineItemNote(WebDriver driver, ArrayList<String>testSteps, String date, String paymentType,String note) {
	date=Utility.parseDate(date, "dd/MM/yyyy", "E MMM dd, yyyy");
	String path="//tbody[contains(@data-bind,'ComputedFolioItems')]//td[@class='lineitem-date']"
			+ "/span[contains(text(),'"+date+"')]/../following-sibling::td[@class='lineitem-category']"
			+ "/span[contains(text(),'"+paymentType+"')]"
			+ "/parent::td/preceding::td//i[@data-toggle='tooltip' and contains(@title,'"+note+"')]";
	Wait.WaitForElement(driver, path);
	assertTrue(driver.findElement(By.xpath(path)).isDisplayed(), "Failed to verify Note");
	testSteps.add("Verified Folio Note " + note);
	folioLogger.info("Verified Folio Note " + note);
}

public void verifyBundleLineItemAdded(WebDriver driver, String Description, String Amount, ArrayList<String> testSteps)
		throws InterruptedException {
	// Verify payment
	Elements_FolioLineItemsVoid Elements_FolioLineItemsVoid = new Elements_FolioLineItemsVoid(driver);
	Wait.explicit_wait_visibilityof_webelement_350(Elements_FolioLineItemsVoid.AddedLineItems_Category.get(0),
	...
	String addedLineItemDescription="//a[contains(@data-bind,'data.Description') and text()='"+Description+"']/../..//span[contains(@data-bind,'data.Amount_aggregate')]";

		String LineItemAmount = driver.findElement(By.xpath(addedLineItemDescription)).getText();
		assertEquals(Amount, LineItemAmount, "Failed to verify the bundle amount");	
		testSteps.add("Sucessfully verified the amount for the Folio Description:"+Description+ " as"+Amount);
		folioLogger.info("Sucessfully verified the amount for the Folio Description:"+Description+ " as"+Amount);
		
}

public void addLineItem(WebDriver driver, ArrayList<String> testSteps, String selectCategory, String amount,
		String quantity) throws InterruptedException, ParseException {

	Elements_FolioLineItemsVoid elements_FolioLineItemsVoid = new Elements_FolioLineItemsVoid(driver);
	clickAddButton(driver);
	testSteps.add("Clicked 'Add Line Item'");
	folioLogger.info("Clicked 'Add Line Item'");
...
public ArrayList<String> voidLineItem(WebDriver driver, ArrayList<String> testSteps, String category, String note) throws InterruptedException {
	Elements_Accounts Account = new Elements_Accounts(driver);
	Elements_FolioLineItemsVoid FolioLineItems = new Elements_FolioLineItemsVoid(driver);
	String CheckBox_path = "//td[contains(@data-bind,'lineitem')]/span[text()='" + category
			+ "']//ancestor::tr//input[@type='checkbox']";
	...
	Utility.ScrollToElement(CheckBox, driver);
	CheckBox.click();
	testSteps.add("Click line item(" + category + ") checkbox");
	folioLogger.info("Click line item(" + category + ") checkbox");
	Utility.ScrollToElement(Account.voidButton, driver);
	Wait.waitForElementToBeClickable(By.xpath(OR.voidButton), driver);
	Account.voidButton.click();
	testSteps.add("Click void button");
	folioLogger.info("Click void button");
	Wait.waitForElementToBeVisibile(By.xpath(OR.NotesPopup), driver);
	Account.NotesPopup_Note.sendKeys(note);
	testSteps.add("Entered Notes : " + note);
	folioLogger.info("Entered Notes : " + note);
	Utility.ScrollToElement(Account.NotesPopup_Void, driver);
	Wait.waitForElementToBeClickable(By.xpath(OR.NotesPopup_Void), driver);
	Account.NotesPopup_Void.click();
	testSteps.add("Click void button to close Notes popup");
	folioLogger.info("Click void button to close Notes popup");

	Wait.waitForElementToBeVisibile(By.xpath(OR.saveFolio), driver);
	Wait.waitForElementToBeClickable(By.xpath(OR.saveFolio), driver);
	FolioLineItems.saveFolio.click();
	Utility.app_logs.info("Successfully void Line item " + category);
	testSteps.add("Successfully void Line item " + category);
