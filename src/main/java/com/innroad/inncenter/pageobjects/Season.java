...
public class Season implements ISeason {
	...
	public void NewSeasonButtonClick(WebDriver driver) throws InterruptedException {
		...
		Wait.waitForElementByXpath(driver, OR.New_Season_Button);
	...
	public ArrayList<String> DeleteSeason(WebDriver driver, String seasonName, ArrayList<String> test_steps)
		...
		Utility.ScrollToElement(checkBox, driver);
		checkBox.click();
		Utility.app_logs.info("Check CheckBox with name '"+ seasonName +"'.");
		test_steps.add("Check CheckBox with name '"+ seasonName +"'.");
		// Wait.explicit_wait_visibilityof_webelement(Season.DeleteSeason,
		// driver);
		Wait.waitForElementByXpath(driver, OR.DeleteSeason);
		Wait.wait2Second();
		Utility.ScrollToElement(Season.DeleteSeason, driver);
		Season.DeleteSeason.click();
		test_steps.add("Clicked on delete season");
		return test_steps;
	...
	public String getDateErrorMsg(WebDriver driver) {
		WebElements_Create_Seasons Season = new WebElements_Create_Seasons(driver);
		Wait.waitForElementByXpath(driver, OR_Setup.seasonDateErrorMsg);
		return Season.seasonDateErrorMsg.getText().trim();
	}
	
	public void selectDays(WebDriver driver,HashMap<String, Boolean> daysToCheck, ArrayList<String> testSteps) throws InterruptedException {
		for(Map.Entry<String, Boolean> map : daysToCheck.entrySet()) {
			Utility.printString("map : " + map.getKey() + " : " + map.getValue());
			if(map.getValue()) {
				String daysPath = "//span[contains(text(),'"+ map.getKey() +"')]//preceding-sibling::input[1]";				
				Wait.waitForElementByXpath(driver, daysPath);
				WebElement ele = driver.findElement(By.xpath(daysPath));
				Utility.ScrollToElement(ele, driver);
				ele.click();
				testSteps.add("Checked : " + map.getKey() + " checkbox.");
				Utility.printString("Checked : " + map.getKey() + " checkbox.");
			}
	...
	public void enterSeasonName(WebDriver driver, ArrayList<String> testSteps, String SeasonName) {

		WebElements_Create_Seasons Season = new WebElements_Create_Seasons(driver);
		Wait.WaitForElement(driver, OR.NG_Season_Name);
		Season.NG_Season_Name.sendKeys(SeasonName);
		testSteps.add("Entered Season Name  " + SeasonName);
		Season.Season_Status.click();

	}

	public void selectSeasonStatus(WebDriver driver, ArrayList<String> testSteps, String status) {

		WebElements_Create_Seasons Season = new WebElements_Create_Seasons(driver);
		Wait.WaitForElement(driver, OR.Season_Status);
		Season.Season_Status.click();
		new Select(Season.Season_Status).selectByVisibleText(status);
		testSteps.add("Selected the Status as " + status);
		seasonLogger.info("Selected the Status as " + status);

	}
public ArrayList<String> verifySeason(WebDriver driver) throws InterruptedException {
		...
		Wait.explicit_wait_visibilityof_webelement(Season.NG_Season_Name, driver);
		value=Season.NG_Season_Name.getText();
		assertEquals(value, "","Error in rule name");
		test_steps.add("Verfied rule name is empty");
		Wait.WaitForElement(driver, OR.Season_Status);
		value=new Select(Season.Season_Status).getFirstSelectedOption().getText();
		assertEquals(value, "Active","Error in season type");
		test_steps.add("Verfied season type is reset");
		...
		for(int i=0;i<dateELe.size();i++) {
			...
			test_steps.add("Verfied effect on date is enabled");
		}
		assertTrue(!Season.NG_Season_Save.isEnabled(), "Season_save_button is Enable");
		test_steps.add("Save Button is disable");
		assertTrue(!Season.NG_Season_Reset.isEnabled(), "Season_save_button is Enable");
		test_steps.add("Reset Button is disable");
