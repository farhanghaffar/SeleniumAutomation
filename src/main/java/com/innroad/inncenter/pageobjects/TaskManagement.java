...
public class TaskManagement implements ITaskManagement {
	...
	public String createTaskCategory(WebDriver driver, String categoryName, ArrayList<String> test_steps)
	...
	public String createTask(WebDriver driver, String taskName, String categoryName) throws InterruptedException {
		...
		try {
			...
			/*
			 * Wait.explicit_wait_visibilityof_webelement(driver.findElement(By.xpath(OR.
			 * Toaster_Message)), driver);
			 * Utility.app_logs.info(driver.findElement(By.xpath(OR.Toaster_Message)).
			 * getText());
			 * assertTrue(driver.findElement(By.xpath(OR.Toaster_Message)).getText()
	...
	public void DeleteCategory(WebDriver driver, String categoryName) throws InterruptedException {
		...
		if (found) {
			...
			TaskManagement.Delete.click();
			Wait.wait2Second();
			assertTrue(driver.findElement(By.xpath(OR.Toaster_Message)).getText().contains(" has been deleted."),
		...
		while (true) {
			...
			for (WebElement category : CategoriesName) {
				...
				if (name.replaceAll("\\s", "").startsWith(categoryName.replaceAll("\\s", ""))) {
					...
					Wait.wait2Second();
					assertTrue(
							driver.findElement(By.xpath(OR.Toaster_Message)).getText().contains(" has been deleted."),
							"Failed: Category Delete");
					TaskManagement.Toaster_Message_Close.click();
					Utility.app_logs.info("Click Toaster Close Button");
					found = true;
	...
	public void enableAutoCreate(WebDriver driver, String taskName, boolean isEnabled) throws InterruptedException {

		Elements_TaskManagement TaskManagement = new Elements_TaskManagement(driver); 
		String AutoCreate = "//span[text()='" + taskName
		+ "']//ancestor::li//following-sibling::li[@class='text-center'][1]/a";
		//String AutoCreate="//span[text()='"+taskName+"']//ancestor::li//following-sibling::li[@class='text-center'][1]/a[contains(@class,'off')]";
		boolean isExist= driver.findElement(By.xpath(AutoCreate)).isSelected();
		if(isEnabled) {						
		if(!isExist) {
		/*String AutoCreate = "//span[text()='" + taskName
				+ "']//ancestor::li//following-sibling::li[@class='text-center'][1]/a";*/
		...
		assertTrue(driver.findElement(By.className(OR.Toaster_Message)).getText()
				.contains(taskName + " has been updated."), "Failed: Task Update");
		TaskManagement.Toaster_Message_Close.click();
		Utility.app_logs.info("Click Toaster Close Button");
			}
			else {
				Utility.app_logs.info("Auto Created Already On");
			}
		}else {
			if(isExist) {				
				WebElement Button = driver.findElement(By.xpath(AutoCreate));
				...
				Utility.app_logs.info("Auto Created Already  Off");
			}
		}

	}

	public void scheduleOnCheckOut(WebDriver driver, String taskName, boolean isEnabled) throws InterruptedException {
		Elements_TaskManagement TaskManagement = new Elements_TaskManagement(driver);
		String CheckBox = "//span[text()='" + taskName
				+ "']//ancestor::li//following-sibling::li[@class='text-center'][2]//span";
		/*String CheckBox = "//span[text()='" + taskName
				+ "']//ancestor::li//following-sibling::li[@class='text-center'][2]//span[contains(@class,'checked')]";*/
		boolean isExist=driver.findElement(By.xpath(CheckBox)).isSelected();
		if(isEnabled) {				
		if(!isExist) {
		...
		Utility.app_logs.info("Click Toaster Close Button");
			...
			Utility.app_logs.info("Scheduled On CheckOut Check Box Already Checked");
		}
		}else {
			if(isExist) {
				Wait.WaitForElement(driver, CheckBox);
				...
				Utility.app_logs.info("Scheduled On CheckOut Check Box Already UnChecked");
			}
		}
	}
	...
	public String UpdateTask(WebDriver driver, String taskName, String updateTaskName) throws InterruptedException {
		...
		Wait.wait1Second();
		WebElement enterTaskName = driver.findElement(By.xpath(
				"//label[@title='Click to Edit Task Type']//div//span[text()='" + taskName + "']//..//..//div//input"));
		enterTaskName.click();
	...
	public String Verify_DeleteTask(WebDriver driver, String taskName) throws InterruptedException {
		...
		if (task_size.size() > 0) {
			...
			assertTrue(toaster_message.contains(taskName + " has been deleted."), "Failed: Task Delete");
			TaskManagement.Toaster_Message_Close.click();
			Utility.app_logs.info("Click Toaster Close Button");
		}
	...
	public void deleteAllCategories(WebDriver driver, String categoryName, ArrayList<String> test_steps)
			throws InterruptedException {
	...
	public String getTaskDueTime(WebDriver driver, String taskName) throws InterruptedException {
		...
		String duetime = (String) executor.executeScript("return arguments[0].value", element);
		Utility.app_logs.info("Task Due Time is: " + duetime);
		return duetime;

	}

	public boolean isTaskThere(WebDriver driver, String taskName) {
	...
	public boolean isAutoCreateDisabledForTask(WebDriver driver, String taskName) {
		boolean isExist = false;
		String path = "//span[text()='" + taskName + "']/parent::div/parent::label/parent::li"
				+ "//following-sibling::li/a[contains(@class,'off')]";
		isExist = Utility.isElementPresent(driver, By.xpath(path));
		return isExist;
	}

	public String getFrequency(WebDriver driver, String taskName) throws InterruptedException {
		String path = "//span[text()=" + taskName + "']/parent::div/parent::label/parent::li"
				+ "//following-sibling::li/div/input[contains(@title,'Frequency')]";
		Wait.WaitForElement(driver, path);
		Utility.ScrollToElement(driver.findElement(By.xpath(path)), driver);
		String fequencyName = Utility.getTextThroughJavaScriptInnerText(driver, path);
		return fequencyName;
	}

	public HashMap<String, String> createTask(WebDriver driver, boolean isCategoryRequied, String categoryName,
			String taskName, boolean isAutoCreate, String fequencyName, boolean isCheckout,
			ArrayList<String> testSteps) throws InterruptedException, ParseException {
		...
		if (isCategoryRequied) {
			categoryName = categoryName + randomNum;
			createTaskCategory(driver, categoryName, testSteps);
			Utility.app_logs.info("Created Category: " + categoryName);
		}
		testSteps.add("========== Create Task ==========");
		ArrayList<String> taskNames = Utility.splitInputData(taskName);
		ArrayList<String> isAutoCreates = Utility.splitInputData(String.valueOf(isAutoCreate));
		ArrayList<String> isCheckouts = Utility.splitInputData(String.valueOf(isCheckout));
		ArrayList<String> fequencyNames = Utility.splitInputData(fequencyName);
		for (int i = 0; i < taskNames.size(); i++) {
			taskName = taskNames.get(i) + randomNum;
			createTask(driver, taskName, categoryName);
			Utility.app_logs.info("Create Task: " + taskName);
			enableAutoCreate(driver, taskName, Boolean.parseBoolean(isAutoCreates.get(i)));
				testSteps.add("Enable/Disabled Auto Create of Task " + taskName);
				Utility.app_logs.info("Enable/Disabled Auto Create of Task " + taskName);
				setFrequency(driver, fequencyNames.get(i), taskName);
				testSteps.add("Set Frequency : <b>" + fequencyNames.get(i) + "</b> of Task " + taskName);
				Utility.app_logs.info(" Set Frequency :" + fequencyNames.get(i) + "of Task " + taskName);
				scheduleOnCheckOut(driver, taskName, Boolean.parseBoolean(isCheckouts.get(i)));
				testSteps.add("Schedule On CheckOut On of Task " + taskName);
				Utility.app_logs.info("Schedule On CheckOut On of Task " + taskName);
				
				String dueTime = getTaskDueTime(driver, taskName);
				getDueTimeOfTask.put(taskName, dueTime);

		}
		return getDueTimeOfTask;
	}
}
