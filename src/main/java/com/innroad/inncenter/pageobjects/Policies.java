...
public class Policies implements IPolicies {
	...
	public int Check_Size_Of_Policies_Rows(WebDriver driver) {
		WebElements_Policies CreatePolicy = new WebElements_Policies(driver);
		int size = CreatePolicy.Policies_Size_return.size();
		return size;
	...
	public void Associate_RoomClasses(WebDriver driver, String RoomClassName) throws InterruptedException {
		...
		CreatePolicy.Assoociate_Assign_One_Btn.click();
		policiesLogger.info("Click ADD Selected Room Class : " + RoomClassName);
		Wait.waitForElementToBeClickable(By.xpath(OR.Done_In_Popup), driver);
		CreatePolicy.Done_In_Popup.click();
	}
	...
	public String Save_Policy(WebDriver driver) throws InterruptedException {
		...
		driver.findElement(By.xpath(OR.Policy_Save)).click();
	...
	public void clickSavePolicyButton_2(WebDriver driver, ArrayList<String> test_steps, String policyName)
			throws InterruptedException {
		WebElements_Policies CreatePolicy = new WebElements_Policies(driver);
		Wait.waitForElementToBeClickable(By.xpath(OR.Policy_Save_2), driver);
		Utility.ScrollToElement(CreatePolicy.Policy_Save_2, driver);
		CreatePolicy.Policy_Save_2.click();
		try {
			String getToasterMessage = CreatePolicy.Toaster_Message.getText();
			assertEquals(getToasterMessage, "Successfully Created Policy : " + policyName,
					"Failed - Verify policy creation success toaster message");
	...
	public void Close_Policy_Tab(WebDriver driver) throws InterruptedException {
		...
		try {
			Utility.ScrollToElement(CreatePolicy.Policy_Close_Btn2, driver);
			jse.executeScript("arguments[0].click();", CreatePolicy.Policy_Close_Btn2);
		}catch (Exception e) {
...
//		try {
//			Wait.waitForElementToBeGone(driver, driver.findElement(By.xpath(OR.Policy_ModuleLoading)), 60);
//			Wait.waitUntilPresenceOfElementLocated(OR.First_Element_In__Result, driver);
//			Wait.explicit_wait_visibilityof_webelement(CreatePolicy.First_Element_In__Result, driver);
//		} catch (Exception e) {
//			Wait.explicit_wait_visibilityof_webelement(CreatePolicy.First_Element_In__Result, driver);
//		}
//		Assert.assertEquals(CreatePolicy.First_Element_In__Result.getText(), PolicyName);

	}

	public void Open_Existing_Policy(WebDriver driver, String PolicyName) throws InterruptedException {
		WebElements_Policies CreatePolicy = new WebElements_Policies(driver);
		WebElement Policy_Name_On_Page = driver.findElement(By.xpath("//a[text()='" + PolicyName + "']"));
		Policy_Name_On_Page.click();
	}

	public void _Open_Existing_Policy(WebDriver driver, String PolicyName) throws InterruptedException {
		WebElements_Policies CreatePolicy = new WebElements_Policies(driver);
		Wait.explicit_wait_visibilityof_webelement(CreatePolicy.Policy_Name_On_Policiespage, driver);
		CreatePolicy.Policy_Name_On_Policiespage.clear();
		CreatePolicy.Policy_Name_On_Policiespage.sendKeys(PolicyName);

//		Wait.explicit_wait_visibilityof_webelement(CreatePolicy._On_On_Policiespage, driver);
//		Utility.ScrollToElement(CreatePolicy._On_On_Policiespage, driver);
//		CreatePolicy._On_On_Policiespage.click();
		WebElement Policy_Name_On_SearchPage = driver.findElement(By.xpath("//a[text()='" + PolicyName + "']"));
		Policy_Name_On_SearchPage.click();
	}

	public boolean VerifyPolicyNotExist(WebDriver driver, ArrayList<String> test_steps, String PolicyName) throws InterruptedException 
	{
		boolean isPolicyNotExist = true;
		String xPath4Policy = "//div[contains(text(),'"+PolicyName+"')]";		
		try
		{			
			WebElement elementPolicy = driver.findElement(By.xpath(xPath4Policy));
			if( driver.findElement(By.xpath(xPath4Policy)).isDisplayed())
			{
				isPolicyNotExist = false;
				String policyNamemFromUI = elementPolicy.getText();
				test_steps.add("Policy Name From UI " + policyNamemFromUI);
				policiesLogger.info("Policy Name From UI "+ policyNamemFromUI);			
			}					
		}
		catch(Exception e)
		{
			test_steps.add("Policy Exists");
			policiesLogger.info("Policy Exists");
			isPolicyNotExist=true;
		}							
		//if( driver.findElement(By.xpath(xPath4Policy)).isDisplayed())
			//isPolicyExist = false;	//deposit policy exist
	...
	public ArrayList<String> Cancellation_policy_Attributes(WebDriver driver, String cancellationAttribute1,
		...
		if (cancellationAttribute1.equals("Room Charges") || cancellationAttribute1.equals("Total Charges")) {
			System.out.print("Type: "+cancellationAttribute1);
			...
			System.out.print("\nPercentage: "+cancellationAttribute1_Value);
	...
	public void DeleteAllPolicies(WebDriver driver,ArrayList<String> Steps) throws InterruptedException {
		WebElements_Policies policy = new WebElements_Policies(driver);
		ClickSearchPolicy(driver, Steps);
		try
		{
		...
		if (policy.List_PolicyCheckbox.size() > 1) {
			...
			for (int i = 0; i < policy.List_PolicyCheckbox.size(); i++) {
				...
				Steps.add(i+1+"Checked CheckBox For Deletion");
			}
			...
			Steps.add("Checked CheckBox For Deletion");
			policy.PolicyCheckbox.click();
			Steps.add("Click On Deletion Button");
			policy.Delete_Policy_Btn.click();
			Wait.waitUntilPresenceOfElementLocatedByClassName(OR.Toaster_Title, driver);
			Wait.explicit_wait_visibilityof_webelement(policy.Toaster_Title, driver);
			Steps.add(policy.Toaster_Title.getText());
			...
			Steps.add("No Policy Founded");
			policiesLogger.info("No Policy Founded");
		}
	}
	...
	public ArrayList<String> createPolicy(WebDriver driver, ArrayList<String> test_steps, String policyName,
		...
		try {
			...
			if (!(policyTypeSelected.equalsIgnoreCase(policyType))) {
				test_steps.add("Selecting Policy Type");
				Logger.getLogger("Selecting Policy Type");
	...
	public ArrayList<String> createPolicy_U(WebDriver driver, ArrayList<String> test_steps, String policyName,
			String policyType, String attr1, String attrValue1, String attr2, String attrValue2, String source,
			String policySeason, String roomClass, String ratePlan, String policyText, String policyDesc)
					throws Exception {
		WebElements_Policies policy = new WebElements_Policies(driver);
...
//			policiesLogger.info("Successfully Verified Toaster  is Displayed ");
	...
	public void selectTypeOfFee(WebDriver driver,String typeOfFee,int clauseNo,ArrayList<String> test_steps) throws InterruptedException {//div[1]
		...
		String optionPath = selectPath+"//div[translate(normalize-space(text()),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='"+typeOfFee.toUpperCase()+"']";
		String optionPath2 = "("+selectPath+"//div[translate(normalize-space(text()),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='"+typeOfFee.toUpperCase()+"'])[2]";
		//String optionPath = "("+selectPath+"//div[text()='"+typeOfFee+"'])[2]";
		...
		Wait.wait2Second();
		try {
			Utility.clickThroughJavaScript(driver, driver.findElement(By.xpath(optionPath2)));
		}catch (Exception e) {
	...
	//	String percentPath = "//span[contains(@class,'sr-no') and contains(text(),'"+clauseNo+"')]/../..//div[contains(text(),'Guests will incur a fee of')]/span/div/div/input";
		String percentPath = "(//span[contains(@class,'sr-no') and contains(text(),'"+clauseNo+"')]/../..//input)[2]";
	...
	public void selectChargeType(WebDriver driver,String chargeType,int clauseNo,int percentOfStayCount,ArrayList<String> test_steps) throws InterruptedException {
		...
		String chargeTypeOptionPath =chargeTypePath+"//span[text()='"+chargeType+"']";
//		Utility.clickThroughJavaScript(driver, driver.findElement(By.xpath(chargeTypePath)));
		Wait.waitForElementToBeVisibile(By.xpath(chargeTypePath), driver);
		driver.findElement(By.xpath(chargeTypePath)).click();;
		Wait.wait2Second();
		System.out.println(chargeTypePath);
		boolean flag = true;
		...
		if(flag) {
			...
			System.out.println("SIZE : " + driver.findElements(By.xpath(chargeTypePath)).size());
			Utility.clickThroughJavaScript(driver, driver.findElement(By.xpath(chargeTypePath+"//span/input")));
			System.out.println(chargeTypePath);
			if(flag) {
				//chargeTypeOptionPath = chargeTypeOptionPath + "["+percentOfStayCount+"]";
				chargeTypeOptionPath = "//div[contains(@class,'ant-select-dropdown')]//div[contains(@class,'ant-select-item-option')]//div[text()='"+chargeType+"']";
				Utility.clickThroughJavaScript(driver,driver.findElement(By.xpath(chargeTypeOptionPath)));
				test_steps.add("Select Charge Type : " + chargeType);
				policiesLogger.info("Select Charge Type : " + chargeType);
			}
	...
	public void selectCheckInChargeType(WebDriver driver,String chargeType,ArrayList<String> test_steps) throws InterruptedException {
		...
		//String chargeTypeOptionPath = "(//li[translate(normalize-space(text()),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='"+chargeType.toUpperCase()+"'])";
		String chargeTypeOptionPath = "(//div[translate(normalize-space(text()),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='"+chargeType.toUpperCase()+"'])";
		Utility.clickThroughJavaScript(driver, driver.findElement(By.xpath(chargeTypePath)));
		Wait.wait2Second();
		//	boolean flag = true;
		for(WebElement ele: driver.findElements(By.xpath(chargeTypeOptionPath))) {
	...
	public void selectNoShowDepositChargeType(WebDriver driver, String chargeType, ArrayList<String> test_steps) throws InterruptedException {
		...
		String chargeTypeOptionPath = "(//div[translate(normalize-space(text()),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='"
				+ chargeType.toUpperCase() + "'])";
		//Utility.clickThroughJavaScript(driver, driver.findElement(By.xpath(chargeTypePath)));
		Utility.clickThroughAction(driver, driver.findElement(By.xpath(chargeTypePath)));
		policiesLogger.info("Clicked on Charges type path");
		Wait.wait2Second();
	...
	public void selectCancelType(WebDriver driver, String cancelWithInType,int clauseNo,ArrayList<String> test_steps) throws InterruptedException {
		...
		String cancelOptionPath =cancelTypePath+"//div[text()='"+cancelWithInType+"']";
...
//			Wait.waitForElementToBeGone(driver, element.ADD_POLICY_BUTTON, 5);
		...
		*/
		try {
			...
			Wait.WaitForElement(driver, path);
//			Wait.explicit_wait_visibilityof_webelement(driver.findElement(By.xpath(path)), driver);
			assertTrue(driver.findElement(By.xpath(path)).isEnabled(),"Failed to verify toaster message");
			test_steps.add("Successfully Verified Toaster  is Displayed ");
			policiesLogger.info("Successfully Verified Toaster  is Displayed ");
			Wait.waitUntilPageLoadNotCompleted(driver, 30);
		}catch (Exception e) {
...
//			policiesLogger.info("Successfully Verified Toaster  is Displayed ");
	...
	public void createCancelationPolicy(WebDriver driver,ArrayList<String> test_steps,String delim,String policyName,String TypeOfFees,String GuestsWillIncurAFee,
		...
		policiesLogger.info("Type of fees: "+TypeOfFees);
		policiesLogger.info("Delim: "+delim);
		policiesLogger.info("Type of fees: "+typeOfFees);
		enterPolicyName(driver, policyName, test_steps);
		...
		for(String type:policyTypesList) {
			if(type.equalsIgnoreCase("Cancellation")) {
				...
				if(typeOfFeesList.get(index).equalsIgnoreCase("percent of stay")) {
					...
					createCancelationPolicy(driver, test_steps, secondaryDelim, cancellationolicyName, typeOfFeesList.get(index), percentageList.get(index), chargesTypes, noOfDays, cancelWithInType, false, "");
				}
				Wait.wait2Second();
...
//			}
	...
	public CancellationPolicy findBestCancellationPolicy(String delim,String cancellationPolicyName,String typeOfFees,
		...
		if(finalValues.size()>0) {
			 ...
			 if(typeList.get(finalValues.indexOf(max)).equalsIgnoreCase("percent of stay")) {
	...
	public String getSelectedType(WebDriver driver, String policyType) {
		...
		{
			String xPath = "//span[contains(@class,'sr-no')]/../..//span[contains(@class,'inrd-select fee-type-select')]//div[contains(@class, 'ant-select-selector')]";
			if(!(policyType.equalsIgnoreCase("Cancellation"))) {
				type = driver.findElement(By.xpath(xPath)).getAttribute("title");
			}
		}
	...
	public String getPercentageFieldLength(WebDriver driver, String policyType) {
		...
		{
			String xpath4ChargesValue = "//span[contains(@class,'line')]//div[contains(@class,'ant-select ant-select-single ant-select-show-arrow')]";
			if(!(policyType.equalsIgnoreCase("Cancellation")))	{
			valueIs = driver.findElement(By.xpath(xpath4ChargesValue)).getAttribute("value");	}
		}
	...
	public ArrayList<String> findBestCancellationPolicy(String delim,String typeOfFees,String guestsWillIncurAFee,String chargesTypes,String cancelWithInType,String roomChargeValue,String totalChargeValue) {
		ArrayList<String> typeOfFeesList = Utility.convertTokenToArrayList(typeOfFees, delim);
		ArrayList<String> percentageList = Utility.convertTokenToArrayList(guestsWillIncurAFee, delim);
		ArrayList<String> chargesTypeList = Utility.convertTokenToArrayList(chargesTypes, delim);
		ArrayList<String> cancelWithInTypeList = Utility.convertTokenToArrayList(cancelWithInType, delim);
		...
		ArrayList<String> bestClause = new ArrayList<String>();
		if(finalValues.size()>0) {
			 Float max = Collections.max(finalValues);  
			 
			bestClause.add(typeList.get(finalValues.indexOf(max)));
			bestClause.add(max+"");
		}else { // other than within check-in date which is not possible
			bestClause.add(typeList.get(0));
			bestClause.add("0");
		}
	...
	//AB
	public void Search_Cancellation_Policy(WebDriver driver,ArrayList<String>Steps) {
		WebElements_Policies policies = new WebElements_Policies(driver);
		policies.Select_Policy_Type.sendKeys("Cancellation");
		Steps.add("Chose policy Type: Cancellation");
		driver.findElement(By.xpath("(//button[contains(@data-bind,'enable: availableStatuses().length > 0, click: goSearchPolicies')])")).click();
		Steps.add("Click On Search Policy Button");
	}
	
	
	public void Search_No_Show_Policy_U(WebDriver driver,ArrayList<String>Steps) {
		WebElements_Policies policies = new WebElements_Policies(driver);
		policies.Select_Policy_Type.sendKeys("No Show");
		Steps.add("Chose policy Type: No Show");
		driver.findElement(By.xpath("(//button[contains(@data-bind,'enable: availableStatuses().length > 0, click: goSearchPolicies')])")).click();
		Steps.add("Click On Search Policy Button");
	}
	
	public void Search_All_Policy_U(WebDriver driver,ArrayList<String>Steps) {
		WebElements_Policies policies = new WebElements_Policies(driver);
		policies.Select_Policy_Type.sendKeys("All");
		Steps.add("Chose policy Type: All");
		driver.findElement(By.xpath("(//button[contains(@data-bind,'enable: availableStatuses().length > 0, click: goSearchPolicies')])")).click();
		Steps.add("Click On Search Policy Button");
	}
	
	public void Search_Deposit_Policy_U(WebDriver driver,ArrayList<String>Steps) {
		WebElements_Policies policies = new WebElements_Policies(driver);
		policies.Select_Policy_Type.sendKeys("Deposit");
		Steps.add("Chose policy Type: Deposit");
		driver.findElement(By.xpath("(//button[contains(@data-bind,'enable: availableStatuses().length > 0, click: goSearchPolicies')])")).click();
		Steps.add("Click On Search Policy Button");
	}
	public void Search_CheckIn_Policy_U(WebDriver driver,ArrayList<String>Steps) {
		WebElements_Policies policies = new WebElements_Policies(driver);
		policies.Select_Policy_Type.sendKeys("Check In");
		Steps.add("Chose policy Type: Check In");
		driver.findElement(By.xpath("(//button[contains(@data-bind,'enable: availableStatuses().length > 0, click: goSearchPolicies')])")).click();
		Steps.add("Click On Search Policy Button");
	}
	
	
	  public void deletePolicy(WebDriver driver, ArrayList<String> test_steps, String policyName) throws Exception {
	        WebElements_Policies policies = new WebElements_Policies(driver);
	        String deleteIcon = "//div[text()='"+policyName+"']/..//i[@class='icon-trash']";
	        Wait.waitForElementToBeVisibile(By.xpath(deleteIcon), driver);
	        driver.findElement(By.xpath(deleteIcon)).click();
	        Wait.waitForElementToBeVisibile(By.xpath(OR_Policies.deletePolicyPopupDeleteButton), driver);
	        policies.deletePolicyPopupDeleteButton.click();
	        Wait.waitForElementToBeVisibile(By.xpath("//div[@class='ant-notification-notice-message']"), driver);
	        System.out.println(driver.findElement(By.xpath("//div[@class='ant-notification-notice-message']/span")).getText());
	       
	    }

	  
	  public void deleteAllPoliciesNew(WebDriver driver, String policyType, ArrayList<String> test_steps) throws Exception {
		  String strDelete = "//div[contains(text(),'"+policyType+"')]/../..//div[contains(@class,'policy-actions flex-row')]/i[@class='icon-trash']";
		  
		  while (true) {
			  List<WebElement> deleteIcons = driver.findElements(By.xpath(strDelete));
			  if (deleteIcons.size()>0) {
				  Wait.explicit_wait_visibilityof_webelement(deleteIcons.get(0), driver);
				  Wait.explicit_wait_elementToBeClickable(deleteIcons.get(0), driver);
				  Utility.clickThroughJavaScript(driver, deleteIcons.get(0));
				  clickDeleteButton(driver, test_steps);
				  Wait.wait3Second();
			  }else {
				  break;
	  ...
	  public void DeleteAllPolicies1(WebDriver driver,ArrayList<String> test_steps) throws InterruptedException {
		...
		try {
			...
			test_steps.add("No Policy Founded");
		}
	  }

	  public ArrayList<String> verifyRuleAllField(WebDriver driver) throws InterruptedException {
		  	String value;
			ArrayList<String> test_steps = new ArrayList<String>();
		  	WebElements_Policies CreatePolicy = new WebElements_Policies(driver);
		  	Wait.WaitForElement(driver, OR.policy_property_name);
			Wait.waitForElementToBeVisibile(By.xpath(OR.policy_property_name), driver);
			// Utility.ScrollToElement(CreatePolicy.Enter_Policy_Name, driver);
			value=CreatePolicy.policy_property_name.getText();
			assertEquals(value, "Reports Property","Error in policy property name");
			test_steps.add("Verfied policy property name is "+value);
			value=new Select(CreatePolicy.policy_status).getFirstSelectedOption().getText();
			Wait.wait2Second();
			assertEquals(value, "Active","Error in policy status");
			test_steps.add("Verfied policy status is reset");
			
			Wait.WaitForElement(driver, OR.Enter_Policy_Name);
			Wait.waitForElementToBeVisibile(By.xpath(OR.Enter_Policy_Name), driver);
			// Utility.ScrollToElement(CreatePolicy.Enter_Policy_Name, driver);
			value=CreatePolicy.Enter_Policy_Name.getText();
			assertEquals(value, "","Error in policy property name");
			test_steps.add("Verfied policy name is empty");
			...
			assertTrue(RadioButton.isSelected(), "Failed : Radio Button is not Selected");
			value=new Select(CreatePolicy.Select_Roomcharges_Type_for_Deposit).getFirstSelectedOption().getText();
			Wait.wait2Second();
			assertEquals(value, "Room Charges","Error in policy name");
			test_steps.add("Verfied policy type is reset");
			value =CreatePolicy.Enter_Deposit_Percentage_Charges.getText();	
			assertEquals(value, "","Error in policy name");
			test_steps.add("Verfied policy charges is empty");		
			
			value=CreatePolicy.Enter_Policy_Text.getText();
			assertEquals(value, "","Error in policy text");
			test_steps.add("Verfied policy text is empty");

			
			CreatePolicy.Enter_Policy_Description.getText();
			assertEquals(value, "","Error in policy description");
			test_steps.add("Verfied policy description is empty");
			...
			Wait.explicit_wait_visibilityof_webelement(CreatePolicy.Associate_Sources_Btn, driver);
			Utility.ScrollToElement(CreatePolicy.Associate_Sources_Btn, driver);
			CreatePolicy.Associate_Sources_Btn.click();
			test_steps.add("Associate Seasons Button Clicked");
			value = selectedValueOption.getText();
			assertEquals(value, "");
			Wait.waitForElementToBeClickable(By.xpath(OR.click_associateSeasons_doneButton), driver);
			CreatePolicy.click_associateSeasons_doneButton.click();
			test_steps.add("Verify associate season selected item list is empty");
			CreatePolicy.Associate_Seasons_Btn.click();
			test_steps.add("Associate Seasons Button Clicked");
			value = selectedValueOption.getText();
			assertEquals(value, "");
			Wait.waitForElementToBeClickable(By.xpath(OR.click_associateSeasons_doneButton), driver);
			CreatePolicy.click_associateSeasons_doneButton.click();
			test_steps.add("Verify associate source selected item list is empty");
			CreatePolicy.Associate_Room_Classes_Btn.click();
			test_steps.add("Associate Room Class Button Clicked");
			value = selectedValueOption.getText();
			assertEquals(value, "");
			Wait.waitForElementToBeClickable(By.xpath(OR.click_associateSeasons_doneButton), driver);
			CreatePolicy.click_associateSeasons_doneButton.click();
			test_steps.add("Verify associate room classes selected item list is empty");
			Wait.wait2Second();
			Utility.ScrollToElement(CreatePolicy.Associate_Rate_Plans_Btn, driver);
			CreatePolicy.Associate_Rate_Plans_Btn.click();
			test_steps.add("Associate Rate Plan Button Clicked");
			value = selectedValueOption.getText();
			assertEquals(value, "");
			Wait.waitForElementToBeClickable(By.xpath(OR.click_associateSeasons_doneButton), driver);
			CreatePolicy.click_associateSeasons_doneButton.click();
			test_steps.add("Verify associate rate plan selected item list is empty");
		
			return test_steps;
		}
	  public void clickNewPolicyButton(WebDriver driver, ArrayList<String> test_steps) {
			WebElements_Policies element = new WebElements_Policies(driver);
			try {
				Wait.explicit_wait_visibilityof_webelement(element.New_Policy_Btn, driver);
				element.New_Policy_Btn.click();
				
			}catch (Exception e) {
				Utility.clickThroughJavaScript(driver, element.New_Policy_Btn);
			}
			test_steps.add("Click Create New Policy Icon");
			policiesLogger.info("Click Create New Policy Icon");
		}
	  
	  public ArrayList<String> createPolicyCheckIn_U(WebDriver driver, ArrayList<String> test_steps, String policyName,
				String policyType, String attr1, String attrValue1, String attr2, String attrValue2, String source,
				String policySeason, String roomClass, String ratePlan, String policyText, String policyDesc)
						throws Exception {
			WebElements_Policies policy = new WebElements_Policies(driver);
