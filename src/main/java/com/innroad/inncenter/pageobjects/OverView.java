...
public class OverView {
	...
	public ArrayList<String> VerifyRoomClass_oldVersion(WebDriver driver, String RoomClass, String BaseAmount, boolean click)
			throws InterruptedException {
		ArrayList<String> test_steps = new ArrayList<>();
		WebElementsOverview elements_OverView = new WebElementsOverview(driver);
		Wait.explicit_wait_visibilityof_webelement(elements_OverView.roomClassAbbreviation.get(0), driver);
		Utility.app_logs.info("GetRoomclassNames" + elements_OverView.roomClassAbbreviation.size());
		boolean found = false;
		for (int i = 0; i < elements_OverView.roomClassAbbreviation.size(); i++) {
			if (elements_OverView.roomClassAbbreviation.get(i).getText().equals(RoomClass)) {
				test_steps.add("Verified Room Class '" + RoomClass + "' is Displayed");
				Utility.app_logs.info("Verified Room Class '" + RoomClass + "' is Displayed");
				String Amountxpath = "(//a[contains(text(), '"+ RoomClass +"')]//ancestor::tr[1]//following-sibling::tr[2]//td[2])[1]";
				Utility.app_logs.info(Amountxpath);
				WebElement RateValue = driver.findElement(By.xpath(Amountxpath));
				Utility.ScrollToElement(RateValue, driver);
				Utility.app_logs.info("Text : " + RateValue.getText());
				assertEquals(Double.parseDouble(RateValue.getText().trim()), Double.parseDouble(BaseAmount), "Failed: Rate missmatched");
				test_steps.add("Verified Room Class Rate Value '$ " + BaseAmount + "' is shown");
				Utility.app_logs.info("Verified Room Class Rate Value '$ " + BaseAmount + "' is shown");
				if (click) {
					RateValue.click();
	...
	public ArrayList<String> VerifyRatePopup_oldVersion(WebDriver driver, String BaseAmount, String AdditionalAdults,
			String AdditionalChilds) throws InterruptedException {
		ArrayList<String> test_steps = new ArrayList<>();
		WebElementsOverview elements_OverView = new WebElementsOverview(driver);
			Wait.wait10Second();
		driver.switchTo().frame("dialog-body0");
		accountsLogger.info("Switch to first iframe");
		
		Wait.wait10Second();
		driver.switchTo().frame(elements_OverView.ratePopupFrame);
		accountsLogger.info("Switch to second iframe");

		Wait.explicit_wait_visibilityof_webelement(elements_OverView.ratePopupBaseAmount, driver);
		test_steps.add("Room Rate popup Appeared");
		Utility.app_logs.info("Room Rate popup Appeared");
		...
		Wait.explicit_wait_visibilityof_webelement(elements_OverView.ratePopupBaseAmount, driver);
		Utility.ScrollToElement(elements_OverView.ratePopupBaseAmount, driver);
		assertEquals(Double.parseDouble(elements_OverView.ratePopupBaseAmount.getAttribute("value").trim()), Double.parseDouble(BaseAmount.trim()),
				"Failed: Room Rate missmatched");
		test_steps.add("Verified Room Rate Value '$ " + BaseAmount + "' is shown");
		Utility.app_logs.info("Verified Room Rate Value '$ " + BaseAmount + "' is shown");
		
		Utility.ScrollToElement(elements_OverView.ratePopupExtraAdults, driver);
		test_steps.add("Extra Adults '" + elements_OverView.ratePopupExtraAdults.getAttribute("value") + "'");
		Utility.app_logs.info("Extra Adults '" + elements_OverView.ratePopupExtraAdults.getAttribute("value") + "'");
		assertEquals(elements_OverView.ratePopupExtraAdults.getAttribute("value").trim(), Adults.trim(),
				"Failed: Extra Adults missmatched");
		test_steps.add("Verified Extra Adults '" + AdditionalAdults + "' is shown");
		Utility.app_logs.info("Verified Extra Adults '" + AdditionalAdults + "' is shown");
		test_steps.add("Extra Child '" + elements_OverView.ratePopupExtraChild.getAttribute("value") + "'");
		Utility.app_logs.info("Extra Child '" + elements_OverView.ratePopupExtraChild.getAttribute("value") + "'");
		Utility.ScrollToElement(elements_OverView.ratePopupExtraChild, driver);
		assertEquals(elements_OverView.ratePopupExtraChild.getAttribute("value").trim(), Childs.trim(),
				"Failed: Extra Child missmatched");
		test_steps.add("Verified Extra Child '" + AdditionalChilds + "' is shown");
		Utility.app_logs.info("Verified Extra Child '" + AdditionalChilds + "' is shown");

		elements_OverView.ratePopupCancel.click();
		accountsLogger.info("Click  cancel to close popup");
		test_steps.add("Click  cancel to close popup");
		
		driver.switchTo().defaultContent();
		accountsLogger.info("Swicth to default window");
	...
	public void verifyOutOfOrder(WebDriver driver, ArrayList<String> test_steps, String roomClassName, String Status,
		...
		Utility.ScrollToViewElementINMiddle(driver, elementPlus);
