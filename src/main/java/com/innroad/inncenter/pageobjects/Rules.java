...
public class Rules implements IRule {
	...
	//By_AbuBkr
	public void create_Rule_U(WebDriver driver, String RuleName, String RuleType, String RuleDescription)
	...
	public void clickRule(WebDriver driver) {
		Elements_Inventory deleteRule = new Elements_Inventory(driver);
		Wait.waitForElementByXpath(driver, OR.click_Rules);
		deleteRule.click_Rules.click();
	}
	public void deleteAllRule(WebDriver driver) throws InterruptedException {
		...
		if (driver.findElement(By.xpath("//div[contains(text(),'Rules List')]"))
				.isDisplayed()) {
			int size = driver.findElements(By.xpath(OR.Select_Rules)).size();
			if (size > 1) {
				List<WebElement> ele = driver.findElements(By.xpath(OR.Select_Rules));
	...
	public void deleteNRules(WebDriver driver) throws InterruptedException {
		Elements_Inventory deleteRule = new Elements_Inventory(driver);
		clickRule(driver);
		Wait.waitForElementByXpath(driver, OR.Click_searchButton);
		deleteRule.Click_searchButton.click();
		Wait.wait3Second();
		int size = driver.findElements(By.xpath(OR.Select_Rules)).size();
		Utility.printString("size : " + size);
		while(size > 0) {			
			Utility.printString("size : " + size);
			deleteAllRule(driver);
			Utility.refreshPage(driver);
			Wait.waitforPageLoad(30, driver);
			Wait.wait3Second();
			size = driver.findElements(By.xpath(OR.Select_Rules)).size();
		}
		
	}
	public ArrayList<String> clickNewRuleButton(WebDriver driver) throws InterruptedException{
		Elements_Inventory newRule = new Elements_Inventory(driver);		
		ArrayList<String> testSteps = new ArrayList<>();
		Wait.waitForElementByXpath(driver, OR.Click_newRule_Btn);
		Utility.ScrollToElement(newRule.Click_newRule_Btn, driver);
		newRule.Click_newRule_Btn.click();
		testSteps.add("Clicked new rule button");
		return testSteps;
	}
	public void verifyRulesDetailsPageSections(WebDriver driver, ArrayList<String> testSteps) throws InterruptedException {
		Elements_Inventory newRule = new Elements_Inventory(driver);		
		
		Wait.waitForElementByXpath(driver, OR.Click_closeTab);
		testSteps.add("Verify 'Rule Details' section is showing.");
		Wait.wait10Second();
		assertTrue(Utility.isElementDisplayed(driver, By.xpath(OR_Inventory.rulesDetails)), "Failed : 'Rule Details' is not displaying.");
		testSteps.add("Verified 'Rule Details' section is showing.");
		
		testSteps.add("Verify 'Rule Attributes' section is showing.");
		assertTrue(Utility.isElementDisplayed(driver, By.xpath(OR_Inventory.rulesAttributes)), "Failed : 'Rule Attributes' is not displaying.");
		testSteps.add("Verified 'Rule Attributes' section is showing.");
		
		testSteps.add("Verify 'Apply To' section is showing.");
		assertTrue(Utility.isElementDisplayed(driver, By.xpath(OR_Inventory.rulesApplyTo)), "Failed : 'Apply To' is not displaying.");
		testSteps.add("Verified 'Apply To' section is showing.");
	...
	public void create_Rule(WebDriver driver, String RuleName, String RuleType, String RuleDescription, String Value,
			String RoomClass, String Source, String RatePlan, String Season, String Status, HashMap<String, Boolean> dayCheck)
			throws InterruptedException {
		Elements_Inventory newRule = new Elements_Inventory(driver);
		// Wait.explicit_wait_visibilityof_webelement(newRule.click_Rules,
		// driver);
		...
		for(Map.Entry<String, Boolean> map : dayCheck.entrySet()) {
			Utility.printString("map.getValue()  : " + map.getValue());
			if(map.getValue()) {
				String path = "//span[text()='"+ map.getKey() +"']//preceding-sibling::input[1]";
				Utility.printString("path  : " + path);
				driver.findElement(By.xpath(path)).click();
				rulesLogger.info("Clicked " + map.getKey() + " checkbox");
			}			
		}
	...
	public void clickCloseTab(WebDriver driver, ArrayList<String> testSteps) {
		Elements_Inventory newRule = new Elements_Inventory(driver);
		// Close the created rule
		newRule.Click_closeTab.click();	
		testSteps.add("Clicked close tab");
		rulesLogger.info(" Closed the tab");
	}
	
	public ArrayList<String> openRule(WebDriver driver, String ruleName) {
		ArrayList<String> testSteps = new ArrayList<>();
		Elements_Inventory newRule = new Elements_Inventory(driver);
		Wait.waitForElementByXpath(driver, OR.Click_searchButton);
		newRule.Click_searchButton.click();
		testSteps.add("Clicked Searched button");
		rulesLogger.info("Clicked Searched button");
		
		String path = "//a[text()='"+ ruleName +"']";
		Wait.waitForElementByXpath(driver, path);
		driver.findElement(By.xpath(path)).click();
		testSteps.add("Clicked Searched rule with name : " + ruleName);
		rulesLogger.info("Clicked Searched rule with name : " + ruleName);
		
		return testSteps;
		
	}
	public void create_Rule_without_save(WebDriver driver, String RuleName, String RuleType, String RuleDescription, String Value,
			String RoomClass, String Source, String RatePlan, String Season, String Status, HashMap<String, Boolean> dayCheck)
			throws InterruptedException {
		Elements_Inventory newRule = new Elements_Inventory(driver);
	...
	public ArrayList<String> clickResetRuleButton(WebDriver driver) throws InterruptedException{
		Elements_Inventory newRule = new Elements_Inventory(driver);		
		ArrayList<String> testSteps = new ArrayList<>();
		Wait.waitForElementByXpath(driver, OR.rule_reset_Button);
		Utility.ScrollToElement(newRule.rule_reset_Button, driver);
		newRule.rule_reset_Button.click();
		testSteps.add("Clicked reset rule button");
		return testSteps;
	}
	...
	public ArrayList<String> verifyRuleAllField(WebDriver driver) throws InterruptedException {
		...
		Wait.explicit_wait_visibilityof_webelement(newRule.Enter_ruleName, driver);
		value=newRule.Enter_ruleName.getText();
		assertEquals(value, "","Error in rule name");
		test_steps.add("Verfied rule name is empty");
		value=new Select(newRule.Select_ruleType).getFirstSelectedOption().getText();
		assertEquals(value, "Minimum Stay","Error in rule type");
		test_steps.add("Verfied rule type is reset");
		if (newRule.Enter_MiniStay.isEnabled()) {
			value=newRule.Enter_MiniStay.getText();
			assertEquals(value, "","Error in rule type");
			test_steps.add("Verfied value is empty");
		}
		WebElement element = driver.findElement(By.xpath("(//select[contains(@data-bind,'value: Active')])[1]"));
		Select sel = new Select(element);
		value=sel.getFirstSelectedOption().getText();
		assertEquals(value, "Active","Error in rule status");
		test_steps.add("Verfied rule status is selected as default");

		value=newRule.Enter_ruleDescription.getText();
		assertEquals(value, "","Error in rule type");
		test_steps.add("Verfied rule description is empty");
		String dateIsSelectedOrNot="//label[text()='Effective On:']//..//input[@type='checkbox']";
		List<WebElement> dateELe=driver.findElements(By.xpath(dateIsSelectedOrNot));
		for(int i=0;i<dateELe.size();i++) {
			assertEquals(dateELe.get(i).isSelected(), true,"Error in select Date");
			test_steps.add("Verfied effect on date is disabled");
		}
		...
		newRule.click_associateRoomclass.click();
		rulesLogger.info("Associate Room Class Button Clicked");
		...
		newRule.click_associateRatePlans.click();
		rulesLogger.info("Associate Rate Plan Button Clicked");
