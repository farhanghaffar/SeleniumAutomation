...
public class NightlyRate {
	...
	public void enterRate(WebDriver driver, ArrayList<String> test_steps, String RatePerNight,
			...
			String AdditionalAdultsPerNight, String AdditionalChildPerNight) {
		...
		if (isAdditionalChargesForChildrenAdults.equalsIgnoreCase("Yes")) {
			...
			String[] adults = MaxAdults.split(Utility.DELIM);

			String[] persons = MaxPersons.split(Utility.DELIM);

			String[] addAdults = AdditionalAdultsPerNight.split(Utility.DELIM);
			String[] addChild = AdditionalChildPerNight.split(Utility.DELIM);

			for (int i = 0; i < rate.size(); i++) {

				logger.info("Adults " + adults[i]);
				logger.info("persons " + persons[i]);
				logger.info("addAdults " + addAdults[i]);
				logger.info("addChild " + addChild[i]);
...
//			HashMap<ArrayList<String>, V>
	...
	public void createUpdateSeasonForPackageIntervalRatePlan(WebDriver driver, ArrayList<String> test_steps,
		...
		String[] seasonIsProRateStayInSeason = isProRateStayInSeason.split(Utility.SEASONDELIM);
		String[] seasonIsProRateInRoomClass = isProRateInRoomClass.split(Utility.SEASONDELIM);
		String[] seasonProRateUncheckedRoomClassName = roomClassesWithProRateUnchecked.split(Utility.SEASONDELIM);

		String[] seasonIsCustomPerNight = IsCustomPerNight.split(Utility.SEASONDELIM);
		String[] customRoomClass = CustomRoomClass.split(Utility.SEASONDELIM);
		String[] customRatePerNight = CustomRatePerNight.split(Utility.SEASONDELIM);
		String[] seasonIsAssignPolicyByRoomClass = isAssignPolicyByRoomClass.split(Utility.SEASONDELIM);
		String[] customRateAdultdPerNight = CustomRateAdultdPerNight.split(Utility.SEASONDELIM);
		String[] customRateChildPerNight = CustomRateChildPerNight.split(Utility.SEASONDELIM);
		String[] seasonIsCustomRatePerNightAdultandChild = isCustomRatePerNightAdultandChild.split(Utility.SEASONDELIM);
			...
			if (Boolean.parseBoolean(seasonIsProRateStayInSeason[i])) {
				...
				if (!seasonProRateUncheckedRoomClassName[i].equalsIgnoreCase("no")
					...
					ratesGrid.unCheckProRateValuesOfRoomClasses(driver,
							Boolean.parseBoolean(seasonIsProRateInRoomClass[i]), seasonProRateUncheckedRoomClassName[i],
							test_steps);
				}
			}
	...
	public void selectRoomClassInPolicy(WebDriver driver, String isAssignPolicyByRoomClass,
		...
		if (isAssignPolicyByRoomClass.equalsIgnoreCase("Yes")) {
			...
			for (int i = 0; i < roomClass.length; i++) {
				...
				String selectRoomClassPath = "//li[text()='" + getRoomClass + "']";
	...
	public void createNightlyRatePlan(WebDriver driver, ArrayList<String> test_steps, String ratePlan, String channels,
		...
		createNightlyRatePlan(driver, test_steps, ratePlan, channels,roomClasses, seasonStartDate, seasonEndDate, seasonName, roomsRatePerNight,
				cancellationPolicyName, depositPolicyName, checkInPolicyName, noShowPolicyName, assignPolicies,
				"No", "0", "No", "No", "");
	}

	public void createNightlyRatePlan(WebDriver driver, ArrayList<String> test_steps, String ratePlan, String channels, String roomClasses,
									  String seasonStartDate, String seasonEndDate, String seasonName, String roomsRatePerNight,
									  String cancellationPolicyName, String depositPolicyName, String checkInPolicyName,
		...
		clickRulesRestrictionOnSeason(driver, test_steps);
		enterSeasonLevelRules(driver, test_steps, "Yes", "No", "No",
				rulesApplied, minNightCount, "Yes", "Yes",
				"Yes", "Yes", "Yes", "Yes", "Yes");
		clickSaveSason(driver, test_steps);
		clickCompleteChanges(driver, test_steps);
		clickSaveAsActive(driver, test_steps);
