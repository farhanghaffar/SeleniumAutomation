...
public class VerifyPromoCodeRateOnGroup extends TestCore {
	...
	public void verifyPromoCodeRateOnGroup(String checkinDate, String checkoutdate, String roomClassName,
		...
		try {
			...
			try {
				//sometime rate grid take much time load
		...
		try {
			...
			group.createGroupAccount(driver, test, accountName, marketSegment, groupReferral, groupFirstName,
					groupLastName, groupPhone, groupAddress, groupCity, groupState, groupCountry, groupPostalcode,
					test_steps,accountNo);
			test_steps.add("Group acocunt created succesfully: " + accountName);
			group.navigateRoomBlock(driver, test);
			group.ClickNewBlock(driver);
			test_steps.add("Click at new Block link");
			blockName = blockName + " " + Utility.generateRandomString();
			group.EnterBlockName(driver, blockName);
			group.ClickOkay_CreateNewBlock(driver);
			group.selectCheckinAndCheckoutDateForBlock(driver, checkinDate, checkoutdate, timeZone, test_steps);

			group.SelectRatePlan(driver, ratePlanName, test_steps);
			group.SelectAdults(driver, adult, test_steps);
			group.SelectChilds(driver, child, test_steps);
			group.EnterNights(driver, roomPerNight, test_steps);
			if (Utility.validateString(searchedPromoCode)) {

				group.enterPromoCode(driver, searchedPromoCode, test_steps);
			} else {

				group.enterPromoCode(driver, promoCode, test_steps);
			}

			group.ClickSearchGroup(driver, test_steps);
			boolean isAvail = group.isBlockAvailable(driver, test_steps, roomClassName);
			System.out.println(isAvail);
			test_steps.add("=================== VERFYING SETUP RATE IN GROUP DISPLAYED ======================");
			app_logs.info("=================== VERFYING SETUP RATE IN GROUP DISPLAYED ======================");
			boolean isCorrectRate = group.verifyAvgRateInGroupBlock(driver, roomClassName, ratePerNight, test_steps);
			boolean actualRate = Boolean.parseBoolean(isRateComeFromSetup);
			Assert.assertEquals(isCorrectRate, actualRate, "Failed to verify");
		} catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
		...
		try {
			...
			// nav.Inventory(driver, test_steps);
			nav.navInventoryFromGroupBlock(driver);
			nav.RatesGrid(driver);
			test_steps.add("Navigated to RatesGrid");
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyPromoCodeRateOnGroup", excel);
	}
