...
public class VerifyGroupPoliciesWithAndWithoutTaxExempt  extends TestCore{
	...
	/*	if(Double.parseDouble(cancelAmount)==folioPayment) {
			reservationPage.confirmCancellation(driver, testSteps);
		}else {
			reservationPage.proceesToCancellationPayment(driver, testSteps);
			reservationPage.verifyAmountOnPaymentScreen(driver, cancelAmount, testSteps);
			if(Double.parseDouble(cancelAmount)>folioPayment) {
	...
	public void verifyDepositCheckInAndCancellationPolicies(String policyType,String policyName,
			...
			String isAccountPolicyCreate, String paymentMethod, String cardNumber, String nameOnCard, String isGroupPolicyApplicable, String reservationType){
...
/*				try {				
				...
				try {
					...
					rateGrid.selectAndReturnSpecificRatePlan(driver, testSteps, ratePlan);	
					Wait.waitUntilPageLoadNotCompleted(driver, 50);
					rateGrid.clickEditIcon(driver, testSteps);
					rateGrid.verifyRatePlaninEditMode(driver, testSteps, ratePlan);
					...
					if (guestFirstName.split("\\|").length>1) {
							...
							cancel.add(nightlyRate.getSeasonLevelPolicyByRoomClass(driver, ratesConfig.getProperty("cancelPolicyText"),roomClassArray[i],testSteps));
						}
						...
						cancel.add(nightlyRate.getSeasonLevelPolicyByRoomClass(driver, ratesConfig.getProperty("cancelPolicyText"),roomClassArray[0],testSteps));
					}
					...
					if(!Utility.isEmptyStringArrayList(seasonCancelPolicy)){
						testSteps.add("No Cancellation Policy Associated with Season");
					}
				...
				try{
					...
					if(Utility.isEmptyStringArrayList(seasonCancelPolicy)) {
						testSteps.add("=========Get  Data from Cancellation Policy if policy exist on Season level=========");
						navigation.clickPoliciesAfterRateGridTab(driver,testSteps);
						Wait.waitUntilPageLoadNotCompleted(driver, 5);		
						HashMap<String, String> valueHashmap= new HashMap<String, String>();
						if (guestFirstName.split("\\|").length>1) {
							for(int i=0;i<seasonCancelPolicy.size();i++) {
								getcancelPoliciesData.add(policies.getpoliciesData(driver, testSteps, ratesConfig.getProperty("cancelPolicyText"), seasonCancelPolicy.get(i)));
								valueHashmap.put(seasonCancelPolicy.get(i), getcancelPoliciesData.get(i).get("AttrValue"));								
							}
							String  value=null;
							value=Utility.maxUsingCollectionsMax(valueHashmap);
							cancelPolicyApplied=Utility.getKeyOfValue(valueHashmap, value);
							cancelPoliciesData.put("AttrValue", value);
							app_logs.info(cancelPoliciesData);
						}else {
							cancelPoliciesData=policies.getpoliciesData(driver, testSteps, ratesConfig.getProperty("cancelPolicyText"), seasonCancelPolicy.get(0));
							cancelPolicyApplied=seasonCancelPolicy.get(0);
						
						}
					}
					
					app_logs.info(getcancelPoliciesData);
				}catch (Exception e) {
...
*/
				...
				try {		
					...
					gettestSteps.clear();
					gettestSteps = res.verifyGroupinStayInfo(driver, accountName, true);
					testSteps.addAll(gettestSteps);
					res.verifyStayInfoPromoCodeIsDisplayed(driver, testSteps, true);
					reservationPage.verifyTaxExempt(driver, testSteps);
				}catch (Exception e) {
				...
				/*	if(isGroupPolicyApplicable.equals("Yes") ){
						if(isAccountPolicyCreate.equalsIgnoreCase("Yes")) {	
						...
						if (guestFirstName.split("\\|").length>1) {
							...
							depositAmount=reservationPage.calculationOfDepositAmountToBePaidForRateV2(roomCharges, getdepositPoliciesData.get(0).get("Type"), getdepositPoliciesData.get(0).get("AttrValue"),size);					
				...
				/*	if(isGroupPolicyApplicable.equals("Yes")) {
						...
						if(Utility.isEmptyStringArrayList(seasonDepositPolicy)){
							verificationDepositWithPolicy(paymentMethod,depositPolicyApplied,depositAmount,paymentTypeIs);
						}else {
							verificationDepoistWithoutPolicy(paymentMethod,checkInDates.get(0),ratesConfig.getProperty("noPolicyText"));
						}										
					}	*/
				}catch (Exception e) {
			...
			/*		if(isGroupPolicyApplicable.equals("Yes")) {	
					if(isAccountPolicyCreate.equalsIgnoreCase("Yes")) {
							...
							completeCheckInIfPolicyExist(balance,checkInAmount);
							testSteps.add("==================Verify Check-In Policy On Reservation==================");
							verificationCheckinWithPolicy(paymentMethod,checkinPolicyApplied,checkInAmount,paymentAmount,checkInCardFormat);
						}else
						{
				...
				*/				
				...
				try {
					...
					HashMap<String, String> getcancellationFeeAmount = new HashMap<String,String>();
					double payment= Double.parseDouble(reservationPage.get_Payments(driver, testSteps));
					String size= String.valueOf(groupRoomClassName.split("\\|").length);
					
					cancelAmount=reservationPage.calculationOfCancelletionAmountToBePaidForRateV2(roomCharges,policyAttr1Is[2], policyAttrValueIs[2],size, String.valueOf(payment));
					cancellationFeeAmount=reservationPage.calculationOfDepositAmountToBePaidForRateV2(roomCharges, policyAttr1Is[2], policyAttrValueIs[2], size);
			...
			/*		if(isGroupPolicyApplicable.equals("Yes")){
						...
						if (guestFirstName.split("\\|").length>1) {
							for(int i=0;i<seasonCancelPolicy.size();i++) {
								getCancelAmount.put(seasonCancelPolicy.get(i), reservationPage.calculationOfCancelletionAmountToBePaidForRateV2(roomCharges, getcancelPoliciesData.get(i).get("Type"), getcancelPoliciesData.get(i).get("AttrValue"),size,String.valueOf(payment)));
								getcancellationFeeAmount.put(seasonCancelPolicy.get(i), reservationPage.calculationOfDepositAmountToBePaidForRateV2(roomCharges, getcancelPoliciesData.get(i).get("Type"), getcancelPoliciesData.get(i).get("AttrValue"),size));
								
							}
							ArrayList<Double> dbr= new ArrayList<Double>();
							ArrayList<Double> dbr1= new ArrayList<Double>();
							for (Map.Entry<String, String> entry : getCancelAmount.entrySet()) {
								dbr.add(Double.valueOf(entry.getValue()));
							}
							for (Map.Entry<String, String> entry : getcancellationFeeAmount.entrySet()) {
								dbr1.add(Double.valueOf(entry.getValue()));
							}
							DecimalFormat df = new DecimalFormat("0.00");
							df.setMaximumFractionDigits(2);
							cancelAmount=df.format(Collections.max(dbr));
							app_logs.info(depositAmount);
							cancellationFeeAmount=df.format(Collections.max(dbr1));
							app_logs.info(cancellationFeeAmount);
							cancelPolicyApplied=Utility.getKeyOfValue(getCancelAmount, cancelAmount);
								
						}else {
							cancelAmount=reservationPage.calculationOfCancelletionAmountToBePaidForRateV2(roomCharges, getcancelPoliciesData.get(0).get("Type"), getcancelPoliciesData.get(0).get("AttrValue"),size,String.valueOf(payment));					
							cancellationFeeAmount=reservationPage.calculationOfDepositAmountToBePaidForRateV2(roomCharges, getcancelPoliciesData.get(0).get("Type"), getcancelPoliciesData.get(0).get("AttrValue"),size);
							cancelPolicyApplied=seasonCancelPolicy.get(0);
						}
					}	
						app_logs.info(cancelPolicyApplied);
						
					*/	
						app_logs.info(cancelAmount);
						app_logs.info(cancellationFeeAmount);
						String cancellationCardFormat=null;
						if(paymentMethod.equals("MC")) {
							cancellationCardFormat=""+paymentMethod+" -"+fourDigitCardNo+" ("+expiryDate+")";
						}else if(paymentMethod.equals("Cash")) {
							cancellationCardFormat=paymentMethod;
						}
						ArrayList<String >type=new ArrayList<String>();
						app_logs.info(cancellationCardFormat); 
						
						completeCancellationReservation(payment,cancelAmount,type);
						
						verificationCancellationWithPolicy(policyNames.get(2),paymentMethod,cancelAmount,"Cancellation Fee",cancellationFeeAmount,cancellationCardFormat,type.get(0));
					/*	if(isGroupPolicyApplicable.equals("Yes")) {
							if(isAccountPolicyCreate.equalsIgnoreCase("Yes")) {
								verificationCancellationWithPolicy(policyNames.get(2),paymentMethod,cancelAmount,"Cancellation Fee",cancellationFeeAmount,cancellationCardFormat,type.get(0));
								}
								...
								if(Utility.isEmptyStringArrayList(seasonCancelPolicy)){
									verificationCancellationWithPolicy(cancelPolicyApplied,paymentMethod,cancelAmount,"Cancellation Fee",cancellationFeeAmount,cancellationCardFormat,type.get(0));
								}	
								}	
					*/
				}catch (Exception e) {
					Utility.catchException(driver, e, "Cancel Reservation and Verify Cancelletion", "Reservation", "Reservation", testName,
							test_description, test_catagory, testSteps);
				} catch (Error e) {
					Utility.catchError(driver, e, "Cancel Reservation and Verify Cancelletion", "Reservation", "Reservation", testName,
							test_description, test_catagory, testSteps);
				...
				try {
					testSteps.add("==========Inactive Group==========");
			        navigation.Groups(driver);
			          group.searchGroupAccount(driver, accountName, true, testSteps);
					testStepsOne.clear();
					testStepsOne = group.changeActiveStatus(driver, "Inactive");
					testSteps.addAll(testStepsOne);
					testStepsOne.clear();
					group.save(driver, test, testStepsOne);
					testSteps.addAll(testStepsOne);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyPoliciesInGroup", excel);
	}
