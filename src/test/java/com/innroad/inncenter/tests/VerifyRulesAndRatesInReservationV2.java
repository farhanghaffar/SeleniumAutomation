...
public class VerifyRulesAndRatesInReservationV2 extends TestCore {
	...
	public void verifyRulesAndRatesInReservationV2(String TestCaseId, String TestCaseName, String RatePlanName,
			String roomClassName, String RulesUpdateType, String Type_RulesUpdate, String RuleMinStayValue_RulesUpdate,
			String isSun_RulesUpdate, String isMon_RulesUpdate, String isTue_RulesUpdate, String isWed_RulesUpdate,
			String isThu_RulesUpdate, String isFri_RulesUpdate, String isSat_RulesUpdate,String isTask, String TaskCategory,
...
//			nightlyRate.verifyTitleSummaryValue(driver, "Channel", summaryChannels, test_steps);
		...
		HashMap<String, // RoomClass
				HashMap<String, // Source
						HashMap<String, // date
								RatesGridChannelRatesRules>>> roomClassWiseSourceRatesRule = null;
		try {

			startSeasonDate = SeasonStartDate;
			endSeasonDate = Utility.getCustomDate(SeasonStartDate, "dd/MM/yyyy", "dd/MM/yyyy", 1);
			allDatesBW = Utility.getAllDatesBetweenCheckInOutDates(startSeasonDate, endSeasonDate);

			test_steps.add("=================== OVERIDE RULES ======================");
			app_logs.info("=================== OVERIDE RULES ======================");

//			String RulesUpdateType = "Override";
//			String RulesUpdateStartDate = startSeasonDate;
//			String RulesUpdateEndDate = endSeasonDate;
...
//			String parentRatePlanName = RatePlanName;
...
//
//			rateGrid.bulkUpdateOverideRules(driver, "\\|", RulesUpdateType, startSeasonDate, endSeasonDate,
//					isSun_RulesUpdate, isMon_RulesUpdate, isTue_RulesUpdate, isWed_RulesUpdate, isThu_RulesUpdate,
//					isFri_RulesUpdate, isSat_RulesUpdate, RatePlanName, roomClassName, channelName, Type_RulesUpdate,
//					RuleMinStayValue_RulesUpdate, test_steps);

			test_steps.add("=================== GETTING DATE WISE ROOMCLASS RATES ======================");
			app_logs.info("=================== GETTING DATE WISE ROOMCLASS RATES ======================");

			navigation.ratesGrid(driver);
			roomClassWiseSourceRatesRule = rateGrid.getSourceWiseRatesRules(driver, test_steps, RatePlanName,
					RatePlanName, false, "\\|", roomClassName, channelName, startSeasonDate, endSeasonDate);

			for (String date : allDatesBW) {
				app_logs.info(date);
				app_logs.info(roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(date));
				test_steps.add(roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(date).toString());
			}
		} catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(test_name, test_description, test_catagory, test_steps);
				Utility.updateReport(e, "Failed", test_name, "Rates V2", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		try {String ReservationRoomClass = roomClassName;
		...
		try {
			...
			res.searchDataForFindRooms(driver, test_steps, CheckInDate, CheckOutDate, adult, children, RatePlanName,
					"");
			...
			ArrayList<String> roomNumbers = res.getAllRoomNumbersFromRoomClassDropDown(driver, test_steps,
					roomClassName);

			FindRoomDetail findRoomDetail = res.getFindRooDetail(driver, RatePlanName, roomClassName, test_steps);
			
			Double rate = Double.parseDouble(roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(CheckInDate)
					.getRooomRate()) * (allDatesBW.size()-1);
			
			res.verifyRoomClassDetail(findRoomDetail, roomClassName, true, true, RatePlanName, true, true, "rooms", false, false, rate+"", true, true, test_steps);
			System.out.println(findRoomDetail);String noCheckinColor = "",noCheckoutColor = "";
			res.select_RoomWithRatePlanRulesValidation(driver, test_steps,
...
//			res.selectRoomToReserve(driver, test_steps, roomClassName, roomNumbers.get(0));
			...
			if (roomClassWiseSourceRatesRule != null) {
				res.verifyTripSummaryDetail(tripSummary,
						roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(CheckInDate)
								.getRooomRate(),
						true, true,
						Utility.parseDate(CheckInDate, "dd/MM/yyyy", "EEE MMM dd") + " - "
								+ Utility.parseDate(CheckOutDate, "dd/MM/yyyy", "EEE MMM dd"),
						true, true, adult + " " + adultStr + ", " + children + " " + childStr, true, true,
						roomClassName + " : " + roomNumbers.get(0), true, true, "incidentals", false, false, "fee",
						false, false, "taxes", false, false, "triptotal", false, false, "paid", false, false, "balance",
						false, false, test_steps);
			} else {
				res.verifyTripSummaryDetail(tripSummary, "roomRate", false, false,
						Utility.parseDate(CheckInDate, "dd/MM/yyyy", "EEE MMM dd") + " - "
								+ Utility.parseDate(CheckOutDate, "dd/MM/yyyy", "EEE MMM dd"),
						true, true, adult + " " + adultStr + ", " + children + " " + childStr, true, true,
						roomClassName + " : " + roomNumbers.get(0), true, true, "incidentals", false, false, "fee",
			...
			if (GuestFirstName.split("\\|").length > 1) {
				res.add_PrimaryRoom(driver, test_steps);
				res.add_AdditionalRoom(driver, test_steps);
			}

			res.enter_GuestName(driver, test_steps, salutation, GuestFirstName, GuestLastName);
			res.turnOnOffcreateGuestProfileToggle(driver, test_steps, isGuesProfile);

			res.enter_Phone(driver, test_steps, phoneNumber, altenativePhone);
			res.enter_Email(driver, test_steps, email);
			
			res.enter_City(driver, test_steps, city);
			res.select_Country(driver, test_steps, country);
			res.select_State(driver, test_steps, state);
			res.enter_PostalCode(driver, test_steps, postalCode);
			res.enter_Address(driver, test_steps, address1, address2, address3);
			res. clickOnGuestProfilePopupSaveButton(driver, test_steps);
			if (Utility.validateString(PaymentMethod)) {

				res.enter_PaymentDetails(driver, test_steps, PaymentMethod, CardNumber, NameOnCard, ExpiryDate,
						accountName);
			}
			res.enter_MarketingInfoDetails(driver, test_steps, "", marketSegment, referral, "Yes");
			
			res.AddTask(driver, test_steps, isTask, TaskCategory, TaskType, TaskDetails, TaskRemarks,
					TaskDueon, TaskAssignee, TaskStatus);
			
			res.clickBookNow(driver, test_steps);
res.waitForLoadingSymbol(driver);
			data.put("ReservationNumber", res.get_ReservationConfirmationNumber(driver, test_steps));
			data.put("ReservationStatus", res.get_ReservationStatus(driver, test_steps));
			res.clickCloseReservationSavePopup(driver, test_steps);
			TripSummary tripSummary2 = res.getTripSummaryDetail(driver);
			data.put("RoomCharges", tripSummary2.getTS_ROOM_CHARGE());
			data.put("Taxes", tripSummary2.getTS_TAXES());
		} catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
...
//					TestCore.TestRail_AssignToID);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyRulesAndRatesInResV2", HS_EXCEL);
	}
