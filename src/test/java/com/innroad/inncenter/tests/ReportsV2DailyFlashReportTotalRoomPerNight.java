...
public class ReportsV2DailyFlashReportTotalRoomPerNight extends TestCore{
...
//			}
		...
		try {
			...
			test_steps.add("========= Validating Guest Count Statistics table =========");
						report.validateDailyFlashReportPropertyStatistics_RV2(driver, beforePropertyStatisticsDetails, afterPropertyStatisticsDetails, "Total Room Nights", totalRooms, test_steps);
		...
		try {
		roomsAction="Delete room class";
		if (Utility.validateString(roomsAction)) {
	
			if (roomsAction.equalsIgnoreCase("Delete room class")) {
				Utility.switchTab(driver, applicationTab);
				nav.setup(driver);
				nav.roomClass(driver);
				rc2.deleteRoomClassIfExist(driver, test_steps, newRoomClassName);				
				totalRooms = 0;				
				app_logs.info("Rooms quantity: "+totalRooms);
			}
		}
		app_logs.info("Expected changes: "+expChanges);
		test_steps.add("========== Action completed at "+java.time.LocalTime.now()+"==========");
	...
	public Object[][] getData() {
		...
		return Utility.getData("DailyFlashReportTotalRoomNight", excelReservationV2);
	}
