...
public class VerifyBusAfterChangePayment extends TestCore {
	...
	public void verifyBusAfterChangePayment(String version, String roomClassName, String channel, String message,
			...
			String infeeName, String feeType, String infeeValues, String categoryFee, String ItemAddedDate,
			String action) {
		...
		String startDate = null, endDate = null, confirmationNo = null, expiryDate = null, startDate1 = null,
				...
				ledgeraccountName = null, resStatus = "CONFIRMED", ratePlanVrbo = "";
		...
		try {
			...
			if (action.equalsIgnoreCase("FeeLedgerInactive")||action.equalsIgnoreCase("TaxLedgerInactive")) {
				navigation.navigateToSetupfromRoomMaintenance(driver);
				navigation.LedgerAccounts(driver);
				ledger.NewAccountbutton(driver);
				ledgeraccountName = Utility.generateRandomStringWithGivenLength(5);
				if(action.equalsIgnoreCase("TaxLedgerInactive")) {
				ledger.enterLedgerAccountDetails(driver, ledgeraccountName, ledgeraccountName, "Tax", "Active");
				}else
				ledger.enterLedgerAccountDetails(driver, ledgeraccountName, ledgeraccountName, "Fee", "Active");
				ledger.SaveLedgerAccount(driver);
			...
			if (isInncenterFeeCreate.equalsIgnoreCase("Yes")) {
				...
				for (int i = 0; i < infeeNames.size(); i++) {
					if (action.equalsIgnoreCase("FeeLedgerInactive")) {
						taxFee.createFee(driver, testSteps, updatedFeeName.get(i), updatedFeeName.get(i),
								ledgeraccountName, updatedFeeName.get(i), categoryFees.get(i), infeeValue.get(i), false,
								"", "", "");
					} else {
						taxFee.createFee(driver, testSteps, updatedFeeName.get(i), updatedFeeName.get(i),
		...
		try {
			if (!data.get("BODY").contains("PROPERTY_NOT_AVAILABLE") && !data.get("BODY").contains("SERVER_ERROR")) {
				...
				if (action.equalsIgnoreCase("RemovePaymentMethod")) {
					reservation.clickOnDetails(driver);

					testSteps.add("<b>===== REMOVE PAYMENT METHOD FROM RESERVATION =====</b>");
					app_logs.info("===== REMOVE PAYMENT METHOD FROM RESERVATION=====");
					beforeAction = Utility.getCurrentDatTimeInUTC("yyyy-MM-dd'T'HH:mm:ss'Z'");
					// reservation.AddLineItem(driver, testSteps, "Room Charge",
					// addedRoomChargeAmount);
					reservation.clickEditPaymentMethodInfo(driver, testSteps);
					reservation.clickonPaymentMetod(driver, testSteps);
					reservation.selectPaymentMethod(driver, "--Select--", testSteps);
				...
				} else if (action.equalsIgnoreCase("CancelPaymentLineItem")) {
					testSteps.add("<b>==== CANCEL PAYMENT LINE ITEM =====</b>");
					app_logs.info("=====CANCEL PAYMENT LINE ITEM=====");
					beforeAction = Utility.getCurrentDatTimeInUTC("yyyy-MM-dd'T'HH:mm:ss'Z'");
					reservation.clickOnDetails(driver);
					String paymentMethod = reservation.getPaymentMethodInExistingRes(driver);
					reservation.click_Folio(driver, testSteps);
					reservation.cancelPaymentLineItem(driver, testSteps, paymentMethod, "cancelTest");
					...
					// phoneCode = getPhone.split("-")[0];
					// updatedphoneNumberVrbo = "+" + phoneCode + " " +
					// getPhone.split("-")[1].trim() + "-"
					// + getPhone.split("-")[2];
				...
				} else if (action.equalsIgnoreCase("CreateNewPayment")) {
					...
					testSteps.add("<b>===== ADD ROOM CHARGE LINE ITEM OUTSIDE STAY =====</b>");
					app_logs.info("===== ADD ROOM CHARGE LINE ITEM OUTSIDE STAY =====");
					beforeAction = Utility.getCurrentDatTimeInUTC("yyyy-MM-dd'T'HH:mm:ss'Z'");

					reservation.AddLineItemWithDate(driver, testSteps, "Room Charge", addedRoomChargeAmount,
							Utility.parseDate(ItemAddedDate, "dd/MM/yyyy", "MM/dd/yyyy"));
					...
					calcInTaxVal = reservation.getVrboTaxesInRoomChargeFolio(driver, chargeName, updatedTaxName);

					System.out.println("calcInTaxVal: " + calcInTaxVal);
					...
					vrboObject.validateBookingUpdateXML(busContent, advertiserId, classId, classId, confirmationNo,
							"en_US", calcInTaxVal, calcInFeeVal, roomchargeAmounts, getExternalIdOfTaxes,
							getExternalIdOfFees, setTaxRates, setFeeRates, payStatus, title, nameONCard, email,
							phoneCode, phoneNumberVrbo, getAddress1, getAddress2, getAddress3, countryName,
							getAaddress4, postalCode, adult, child, startDate, endDate, startDate, resStatus, tripTotal,
							false, dueDates, testSteps);
				} else if (action.equalsIgnoreCase("FeeLedgerInactive") || action.equalsIgnoreCase("TaxLedgerInactive")) {

					testSteps.add("<b>=====INACTIVE FEE LEDGER ACCOUNT =====</b>");
					app_logs.info("===== INACTIVE FEE LEDGER ACCOUNT =====");
					beforeAction = Utility.getCurrentDatTimeInUTC("yyyy-MM-dd'T'HH:mm:ss'Z'");
					navigation.setup(driver);
					navigation.LedgerAccounts(driver);
					ledger.ChangeStatus(driver, ledgeraccountName, "InActive");
					testSteps.add("Made successfully inactive ledger account: " + ledgeraccountName);
					...
					try {
						...
						testSteps.add("Failed due to issue OTA-222: " + e.toString());
					} catch (Error e) {
						testSteps.add("Failed due to issue OTA-222: " + e.toString());
					}
					...
					try {
						vrboObject.validateBookingUpdateXML(busContent, advertiserId, classId, classId, confirmationNo,
								...
								getExternalIdOfFees, setTaxRates, setFeeRates, payStatus, title, nameONCard, email,
								phoneCode, phoneNumberVrbo, getAddress1, getAddress2, getAddress3, countryName,
								getAaddress4, postalCode, adult, child, startDate, endDate, startDate, resStatus,
	...
	public Object[][] getDataOne() {
		...
		return Utility.getData("VerifyBusAfterChangePayment", otaexcel);
