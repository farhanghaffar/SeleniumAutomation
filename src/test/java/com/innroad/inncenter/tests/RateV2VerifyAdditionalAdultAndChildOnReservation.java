...
public class RateV2VerifyAdditionalAdultAndChildOnReservation extends TestCore {
	...
	String[] roomClassArray = null, maxAdultsIS = null, maxPersonsIs = null, ratePerNightIs = null,
			addAdultPerNight = null, addChildPerNight = null ;
	String abb=null;
	...
	public void verifyAdditionalAdultAndChildOnReservation(String checkInDate, String checkOutDate, String ratePlanName,
		...
		exAdultList.clear();
		exChildList.clear();
		ratePlanAdults.clear();
		ratePlanChilds.clear();
		capacityChild1.clear();
		capacityAdult1.clear();
		...
		try
		{
			...
			roomAbbri=newRoomClassV2.getAbbrivation(driver, "|", roomClassArray[0], testSteps);
		...
		try
		{
			...
			rateIs=	tapechart.calculateRatesAsPerAdultsAndChildCapacity(driver, ratesList.get(0), ratePlanAdults.get(0),
					...
					children, testSteps, ratesConfig.getProperty("tapeChartMsg"), roomClassArray[0],checkInDates, checkOutDates, ratesConfig.getProperty("defaultDateFormat"),abb);
		...
		try		
		{
			...
			boolean roomClassAvail = tapechart.isRoomClassAvailableInTapeChart(driver, abb);
			if(roomClassAvail)
			{
				
				tapechart.clickAvailableSlot(driver, abb,testSteps);				
			}
