...
public class VerifyTripSummaryDetailInReservationV2 extends TestCore {
	...
	public void verifyGuestInfoSpecialCharReservationV2(String TestCaseId, String TestCaseName, String RatePlanName,
			String roomClassName, String adult, String children,String isTask, String TaskCategory,
...
//					accountFirstName, accountLastName, isTaxExempt, taxExemptID);
			...
			FindRoomDetail  detail = res.getFindRooDetail(driver, RatePlanName, RoomClasses, test_steps);
			System.out.println(detail);
			ArrayList<String> roomNumbers = res.getAllRoomNumbersFromRoomClassDropDown(driver, test_steps,
					roomClassName);
			//res.verifyRoomClassDetail(detail, roomClassName, true, true, RatePlanName, true, true, roomNumbers.size()+"", true, true, roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(CheckInDate).getRooomRate(), true, true, test_steps);
			
//			try {
//				res.selectRoomToReserve(driver, test_steps, roomClassName, roomNumber);
...
if(roomClassWiseSourceRatesRule != null) {
			res.verifyTripSummaryDetail(tripSummary, roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(CheckInDate).getRooomRate(), true, true,
					...
					roomClassName + " : " + roomNumbers.get(0), true, true, "incidentals", false, false, "fee", false,
					false, "taxes", false, false, "triptotal", false, false, "paid", false, false, "balance", false,
					false, test_steps);
}else {
	res.verifyTripSummaryDetail(tripSummary, "roomRate", false, false,
			...
			res.verifyBackButtonInTaskPopup(driver, test_steps);
...
//			GuestInfo guestInfo = res.getGuestInfoDetail(driver);
//			
//			res.verifyGuestInfoDetail(guestInfo,
//					salutation + " " + GuestFirstName + " " + GuestLastName, true, true, 
//					"contact", false, false, 
//					email, true, true, 
//					"phone", false, false, "alternate", false, false, 
//					address1, true, true, city, true, true, state, true, true, postalCode, true, true, country, true, true, 
//					"fax", false, false, accountName, false, false, test_steps);
...
//					TestCore.TestRail_AssignToID);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyTripSummaryDetailInResV2", HS_EXCEL);
	}
