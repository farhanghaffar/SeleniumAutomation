...
public class VerifyReservationPaymentViaAnotherReservation extends TestCore{
	...
	public void CP_CreateReservation(String TestCaseID,String url, String ClientCode, String Username, String Password,String CheckInDate,String CheckOutDate,String Adults,String Children,String Rateplan,String PromoCode,
		...
		if(isExecutable) {
		//TestCaseID="848106|848338";
		Utility.initializeTestCase(TestCaseID, caseId, statusCode, statusCode,"");


		test_name = "Reservation payment via Reservation";
		...
		try {
			...
			res.enter_MarketSegmentDetails(driver, test_steps, TravelAgent, MarketSegment, Referral);
			res.verify_NotesSections(driver, test_steps);
			falg=res.verify_TaskSections(driver, test_steps);
			res.enter_Notes(driver, test_steps, IsAddNotes, NoteType, Subject,Description);
			
			res.clickBookNow(driver, test_steps);
			reservation1=res.get_ReservationConfirmationNumber(driver, test_steps);
			status=res.get_ReservationStatus(driver, test_steps);
			res.clickCloseReservationSavePopup(driver, test_steps);
			res.close_FirstOpenedReservation(driver, test_steps);
			...
			res.editPaymentInfoAndMakeReservationAsPayment(driver, test_steps, reservation1);
			res.verifyReservationAspaymentDetailsInDetailsPage(driver, test_steps, reservation1);
			res.click_Folio(driver, test_steps);
			String amountPaid=res.paywithRes(driver, test_steps, reservation1);
			res.close_FirstOpenedReservation(driver, test_steps);
			
			driver.navigate().refresh();
			res.Search_ResNumber_And_Click(driver, reservation1);
			res.click_Folio(driver, test_steps);
			res.verifyRespaymentinPaidReservation(driver, test_steps, reservation, amountPaid);
			
			statusCode.set(0, "1");
			statusCode.set(1, "1");
			comments="verified the reservation payment via reservation";
			RetryFailedTestCases.count = Utility.reset_count;
			Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Object[][] getData() {
		...
		return Utility.getData("ResPayment", SanityExcel);
	}
