...
public class ReportsV2LedgerBalancesReportValidation extends TestCore {
		...
		try {
			...
			if (CheckInDate.equalsIgnoreCase("NA") || CheckInDate.isEmpty() || CheckInDate.equalsIgnoreCase("")) {
				...
				if (!ResType.equalsIgnoreCase("MRB")) {
					CheckInDate = Utility.getCurrentDate("dd/MM/yyyy", clientTimeZone);
					CheckOutDate = Utility.parseDate(Utility.getDatePast_FutureDate(1, clientTimeZone), "MM/dd/yyyy", "dd/MM/yyyy");
				} else {
					if (CheckInDate.equalsIgnoreCase("NA") || CheckInDate.isEmpty()
						...
						for (int i = 1; i <= numberOfRooms; i++) {
							...
							checkInDates.add(Utility.getCurrentDate("dd/MM/yyyy", clientTimeZone));
							checkOutDates.add(
									Utility.parseDate(Utility.getDatePast_FutureDate(1, clientTimeZone), "MM/dd/yyyy", "dd/MM/yyyy"));
...
/*		try {
			...
			if (ratesGrid.isRatePlanExist(driver, Rateplan, test_steps)) {
				...
				String FolioDisplayName = ratePlanType, Description = ratePlanType, Channels = "InnCenter";
				...
				if (Utility.reTry.get(testName) == Utility.count) {
					RetryFailedTestCases.count = Utility.reset_count;
					Utility.AddTest_IntoReport(testName, test_description, test_category, test_steps);
					Utility.updateReport(e, "Failed to verify Rooms class and Rate plan", testName, "Room class and Rate plan", driver);
				} else {
					Assert.assertTrue(false);
				}
			} catch (Error e) {
...
*/
		...
		try {			
			...
			if (acc.contains("Fee") || LedgerAccount.equalsIgnoreCase("Fee")) {
				...
				if (feesval) {
					feeValueList = report.getFeeDetailsFromSetupFees(driver); // 20|USD|pernight/stay 
			...
			if (accountTypeOptions.equalsIgnoreCase("Reservations")) {
				...
				if (ResType.equalsIgnoreCase("MRB")) {
					...
					try {
						...
						app_logs.info("Fee Exp: "+feeExp);
						if (FolioFees == feeExp) {
							app_logs.info("Fees matched");
							app_logs.info("Folio Fees: "+FolioFees);
							app_logs.info("Fee Value Fees: "+feeExp);
							
						}else {
							app_logs.info("Fees not macthed");
							app_logs.info("Folio Fees: "+FolioFees);
							app_logs.info("Fee Value Fees: "+feeExp);
						}
...
//					}
	...
	public Object[][] getData() {
		...
		return Utility.getData("ReportsV2LedgerBalances", envLoginExcel);
	}
