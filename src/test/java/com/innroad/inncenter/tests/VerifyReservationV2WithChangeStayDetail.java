...
public class VerifyReservationV2WithChangeStayDetail extends TestCore {
...
//			nightlyRate.verifyTitleSummaryValue(driver, "Channel", summaryChannels, test_steps);
	...
	/*	String startSeasonDate = "";
		...
		String CheckOutDateModify = Utility.getCustomDate(CheckInDate, "dd/MM/yyyy", "dd/MM/yyyy", 5);
		...
		try {
			...
			Wait.wait5Second();
			//Changed check out date from Jan 22, 2022 to Feb 9, 2022
			res.switchHistoryTab(driver, test_steps);
			res.enterTextToSearchHistory(driver, "Changed check out date from "+Utility.parseDate(CheckOutDate, "dd/MM/yyyy", "MMM d, yyyy")+" to "+Utility.parseDate(CheckOutDateModify, "dd/MM/yyyy", "MMM d, yyyy"), true, ReservationV2.HistoryTable.DESCRIPTION, 1, test_steps);
			
			//Changed check in date from Jan 20, 2022 to Feb 1, 2022
//			res.switchHistoryTab(driver, test_steps);
			res.enterTextToSearchHistory(driver, "Changed check in date from "+Utility.parseDate(CheckInDate, "dd/MM/yyyy", "MMM d, yyyy")+" to "+Utility.parseDate(CheckInDateModify, "dd/MM/yyyy", "MMM d, yyyy"), true, ReservationV2.HistoryTable.DESCRIPTION, 1, test_steps);
...
//					TestCore.TestRail_AssignToID);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyResV2WithChangeStayDetail", HS_EXCEL);
	}
