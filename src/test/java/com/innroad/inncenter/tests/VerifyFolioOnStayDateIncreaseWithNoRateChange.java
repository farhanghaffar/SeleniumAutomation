...
public class VerifyFolioOnStayDateIncreaseWithNoRateChange extends TestCore {
...
//	NewRoomClassesV2 obj1 = new NewRoomClassesV2();
	...
	public void verifyFolioOnStayDateIncreaseWithNoRateChange(String TestCaseId, String TestCaseName,String RatePlanName,String roomClassName,String changeOption,String RoomClassChange) throws Exception {
...
//		String roomClassName = roomClassData.get("roomClassName");
		...
		try {
			...
			boolean isRoomClassAvailable = rc.searchRoomClassV2(driver, roomClassName);
			if (!isRoomClassAvailable) {
				...
				rc.clickOnNewRoomClasshButton(driver, test_steps);
				
				test_steps.add("Clicking On New Room Class");
				app_logs.info("Clicking On New Room Class");
				rc.createRoomClassWithDescriptionV2(driver, test_steps, roomClassName, roomClassAB,
...
//			nightlyRate.verifyTitleSummaryValue(driver, "Channel", summaryChannels, test_steps);
		...
		try {
			...
			startSeasonDate = Utility.getCurrentDate("dd/MM/yyyy");// ratePlanData.get("SeasonStartDate");
			endSeasonDate = Utility.getCustomDate(SeasonStartDate, "dd/MM/yyyy", "dd/MM/yyyy", 5);
		...
		String CheckInDate = Utility.getCustomDate(SeasonStartDate, "dd/MM/yyyy", "dd/MM/yyyy", 2);// ratePlanData.get("SeasonStartDate");
		String CheckOutDate = Utility.getCustomDate(SeasonStartDate, "dd/MM/yyyy", "dd/MM/yyyy", 3);
		...
		String CheckInDateModify = Utility.getCurrentDate("dd/MM/yyyy");// ratePlanData.get("SeasonStartDate");
		String CheckOutDateModify = Utility.getCustomDate(SeasonStartDate, "dd/MM/yyyy", "dd/MM/yyyy", 5);
		...
		try {
			...
			res.verifyFolioLineItem(folioLineItems.get(0), "Pending", true, true, Utility.getCustomDate(CheckInDate,"dd/MM/yyyy","MMM d,yyyy",0), true, true,
					"Room Charge", true, true, RatePlanName, false, false, "1", true, true, roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(CheckInDateModify).getRooomRate(), true,
					true, "0", false, false, "0", false, false, test_steps);
			...
			res.selectCheckInDate(driver, test_steps, CheckInDateModify);
			res.selectCheckOutDate(driver, test_steps, CheckOutDateModify);
			res.enter_Adults(driver, test_steps, "2");
			res.selectStayInfoOption(driver, changeOption);
			res.clickFindRoomsStayInfo(driver);
			res.waitForSweetLoading(driver);
			
			ArrayList<String> roomNumbers = res.getAllRoomNumbersFromRoomClassDropDown(driver, test_steps, RoomClassChange);
			res.selectRoomToReserve(driver, test_steps, RoomClassChange, roomNumbers.get(1));
			
			try {
				res.clickSaveStayInfo(driver);
				Wait.waitForElementToBeVisibile(By.xpath(OR_ReservationV2.TOASTER_MESSAGE), driver, 10);
				app_logs.info("Successfully updated the checkout date");
				...
				try {
					...
					res.clickChangeStayDetailsConfirmPopup(driver, "Yes");
					app_logs.info("Clicked on Yes on Confirmation popup");
					Wait.waitForElementToBeVisibile(By.xpath(OR_ReservationV2.TOASTER_MESSAGE), driver, 10);
					app_logs.info("Successfully updated the checkout date");
					test_steps.add("Successfully updated the checkout date");
				}catch (Exception e1) {
					app_logs.info(e1.toString());
				}
			}	
			
			res.switchFolioTab(driver, test_steps);
			ArrayList<FolioLineItem> folioLineItemsAfterModify = res.getAllFolioLineItems(driver);
			res.verifyFolioLineItem(folioLineItemsAfterModify.get(0), "Pending", true, true, Utility.getCustomDate(CheckInDateModify,"dd/MM/yyyy","MMM d,yyyy",0), true, true,
					"Room Charge", true, true, RatePlanName, false, false, "1", true, true, roomClassWiseSourceRatesRule.get(roomClassName).get(channelName).get(CheckInDateModify).getRooomRate(), true,
					true, "0", false, false, "0", false, false, test_steps);
			
			res.verifyFolioLineItem(folioLineItemsAfterModify.get(1), "Pending", true, true, Utility.getCustomDate(CheckInDateModify,"dd/MM/yyyy","MMM d,yyyy",1), true, true,
					"Room Charge", true, true, "NoRateChange", true, true, "1", true, true, "0", true,
					true, "0", false, false, "0", false, false, test_steps);
			res.verifyFolioLineItem(folioLineItemsAfterModify.get(2), "Pending", true, true, Utility.getCustomDate(CheckInDateModify,"dd/MM/yyyy","MMM d,yyyy",2), true, true,
					"Room Charge", true, true,  "NoRateChange", true, true, "1", true, true, "0", true,
					true, "0", false, false, "0", false, false, test_steps);
			res.verifyFolioLineItem(folioLineItemsAfterModify.get(3), "Pending", true, true, Utility.getCustomDate(CheckInDateModify,"dd/MM/yyyy","MMM d,yyyy",3), true, true,
					"Room Charge", true, true, "NoRateChange", true, true, "1", true, true, "0", true,
					true, "0", false, false, "0", false, false, test_steps);
			res.verifyFolioLineItem(folioLineItemsAfterModify.get(4), "Pending", true, true, Utility.getCustomDate(CheckInDateModify,"dd/MM/yyyy","MMM d,yyyy",4), true, true,
					"Room Charge", true, true,  "NoRateChange", true, true, "1", true, true, "0", true,
					true, "0", false, false, "0", false, false, test_steps);
			
			res.switchHistoryTab(driver, test_steps);
			res.enterTextToSearchHistory(driver, "Adult", false, ReservationV2.HistoryTable.DESCRIPTION, 1, test_steps);
		} catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
...
//					TestCore.TestRail_AssignToID);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyFolioOnStayDateIncrease", HS_EXCEL);
	}
