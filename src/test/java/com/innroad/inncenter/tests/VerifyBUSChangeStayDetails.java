...
public class VerifyBUSChangeStayDetails extends TestCore{
	...
	public void verifyBUSChangeStayDetails(String version, String roomClassName, String channel, String message,
			...
			String updateCheckInDate,String noOfNightsUpdated, String Update,
			String ratePlanName,String updateRate, String isAdditionalChargesForChildrenAdults,String extraAdult,
		...
		try {
			...
			app_logs.info("Start Date1: "+startDate1);
			app_logs.info("End Date1: "+endDate1);
			if(Update.equalsIgnoreCase("extend")) {	
		...
		try {
			...
			checkInDates.clear();
			if (Utility.validateInput(updateCheckInDate)) {			
				checkInDates = Utility.splitInputData(updateCheckInDate);
				startDate1 = checkInDates.get(0);
			}	
			if(Update.equalsIgnoreCase("reduce")) {	
			if (!Utility.validateString(noOfNightsUpdated)) {
				endDate = Utility.minusDays(endDate1, 1);
			} else {
				endDate = Utility.minusDays(endDate1, Integer.parseInt(noOfNightsUpdated));
			}
			}else if(Update.equalsIgnoreCase("extend")) {
				if (!Utility.validateString(noOfNightsUpdated)) {
					endDate = Utility.addDays(endDate1, 1);
				} else {
					endDate = Utility.addDays(endDate1, Integer.parseInt(noOfNightsUpdated));
				}
				
			}
	
			app_logs.info(startDate1);
			app_logs.info(endDate);
			...
			testSteps.add("====================CHANGE STAY DATES======================");	
			app_logs.info("====================CHANGE STAY DATES======================");	
			...
			reservation.select_CheckInDate(driver, testSteps, startDate1);
			reservation.select_CheckoutDate(driver, testSteps, endDate);
			reservation.selectChangeOnlyForAddedRemovedDates(driver, true,testSteps);
			...
			e.printStackTrace();
			Utility.catchException(driver,e, test_description, test_catagory, "Change Stay Details", testName, test_description,
					test_catagory, testSteps);

		} catch (Error e) {
			e.printStackTrace();
			Utility.catchError(driver,e, test_description, test_catagory, "Change Stay Details", testName, test_description,
					test_catagory, testSteps);
		}
	...
	public Object[][] getDataOne() {
		...
		return Utility.getData("BusChangeStayDetails", otaexcel);
