...
public class VerifyBUSAfterAddAdjustVoidAnTaxAndFee extends TestCore{
	...
	LedgerAccount ledgerAccountClass= new LedgerAccount();
	Response response;
	...
	public void verifyBUSChangeStayDetails(String version, String roomClassName, String channel, String message,
			...
			String newAmount,String status) {
		...
		String startDate = null, endDate = null,  confirmationNo = null, expiryDate = null,
				startDate1 = null, endDate1 = null, advertiserId = null, classId = null, currency = null,propertyId = null,
						phoneCode = null, phoneNumberVrbo = null,
								getAddress1 = null, getAddress2 = null, getAddress3 = null, getAaddress4 = null,countryName=null,
								property=null, extraFee=null, taxNameAdd=null, feeNameAdd=null;
		int nightsDays=3;
		...
		HashMap<String, Double> calcInTaxVal = new HashMap<String, Double> (); 
		HashMap<String, Double> calcInFeeVal= new HashMap<String, Double> ();
		app_logs.info("##################################################################################");
		...
		try {
			...
			if(status.equalsIgnoreCase("AddFee")) {
			navigation.clickLedgerAccountsTab(driver);
			ledgerAccountClass.NewAccountbutton(driver);
		    extraFee = feeName +  Utility.generateRandomStringWithGivenLength(3);
			ledgerAccountClass.enterLedgerAccountDetails(driver, extraFee, extraFee, feeName, "Active");
			ledgerAccountClass.SaveLedgerAccount(driver);
		...
		try {
			...
			if (!(Utility.validateInput(beginDate))) {
				...
				if (!Utility.validateString(noOfNights)) {
					endDate1 = Utility.parseDate(Utility.getDatePast_FutureDate(nightsDays), "MM/dd/yyyy", "dd/MM/yyyy");
				} else {
				...
				if (!Utility.validateString(noOfNights)) {
					endDate1 = Utility.addDays(startDate1, nightsDays);
				} else {
		...
		try {
			...
			calcInTaxVal = reservation.getVrboTaxesInRoomChargeFolio(driver, chargeName,
					updatedTaxName);
			calcInFeeVal = reservation.getVrboFeeInFolio(driver, updatedFeeName, days);
		...
		try {
			...
			reservation.click_Folio(driver, testSteps);
			if(status.equalsIgnoreCase("AddTax")||status.equalsIgnoreCase("AddFee")) {
				if(status.equalsIgnoreCase("AddTax")) {
					testSteps.add("====================Add Tax======================");
					reservation.addLineItem(driver, testSteps, endDate1,type, newAmount);
				}else {
					testSteps.add("====================Add Fee======================");
					reservation.addLineItem(driver, testSteps, endDate1,extraFee, newAmount);	
				}
			}else if(status.equalsIgnoreCase("AdjustTax")) {	
				testSteps.add("====================Adjust Tax======================");
				taxNameAdd="Adjust Tax";
				reservation.adjust_LineItem(driver, testSteps, updatedTaxName.get(0), newAmount,"HomeAway Rate",taxNameAdd);
				Wait.waitforPageLoad(50, driver);				
			}else if(status.equalsIgnoreCase("AdjustFee")) {
				testSteps.add("====================Adjust Fee======================");
				feeNameAdd="Adjust Fee";
				reservation.adjust_LineItem(driver, testSteps, feeType, newAmount,updatedFeeName.get(0),feeNameAdd);
				Wait.waitforPageLoad(50, driver);		
			}else if(status.equalsIgnoreCase("VoidTax")) {
				testSteps.add("====================Void Tax======================");
				taxNameAdd="Void Tax";
				reservation.void_LineItemFromRoomCharge(driver, testSteps, updatedTaxName.get(0), newAmount,"HomeAway Rate",taxNameAdd);
				Wait.waitforPageLoad(50, driver);		
			}else if(status.equalsIgnoreCase("VoidFee")) {
				testSteps.add("====================Void Fee======================");
				feeNameAdd="Void Fee";
				reservation.void_LineItemFromRoomCharge(driver, testSteps, feeType, newAmount,updatedFeeName.get(0),feeNameAdd);
				Wait.waitforPageLoad(50, driver);		
				days=days-1;
			}
		}catch (Exception e) {
			e.printStackTrace();
			Utility.catchException(driver,e, test_description, test_catagory, "Add Void Adjust Tax and Fee", testName, test_description,
					test_catagory, testSteps);

		} catch (Error e) {
			e.printStackTrace();
			Utility.catchError(driver,e, test_description, test_catagory, "Add Void Adjust Tax and Fee", testName, test_description,
					test_catagory, testSteps);
		}
		...
		try {
			...
			if(status.equalsIgnoreCase("AddFee") || status.equalsIgnoreCase("AdjustTax") || status.equalsIgnoreCase("AdjustFee")||status.equalsIgnoreCase("VoidTax")
					||status.equalsIgnoreCase("VoidFee")) {
			String tripTotals = reservation.getTripTotal_TripSummary(driver).trim();
			tripTotal = Double.parseDouble(tripTotals);
			}
			reservation.click_Folio(driver, testSteps);
			...
			app_logs.info(getExternalIdOfFee);
			 ...
			 if(status.equalsIgnoreCase("AddTax")||status.equalsIgnoreCase("AddFee")||status.equalsIgnoreCase("VoidTax")) {
			 calcInTaxVal = reservation.getVrboTaxesInRoomChargeFolio(driver, chargeName,
					updatedTaxName);
			  calcInFeeVal = reservation.getVrboFeeInFolio(driver, updatedFeeName, days);
			 }else if(status.equalsIgnoreCase("VoidFee")) {
			...
			if (status.equalsIgnoreCase("AddFee")) {
				setFeeRates.put(extraFee, "No");
				getExternalIdOfFee.put(extraFee, "0");
				calcInFeeVal.put(extraFee, Double.parseDouble(Utility.convertDecimalFormat(newAmount)));
			} else if (status.equalsIgnoreCase("AdjustTax")) {
				double taxValue = calcInTaxVal.get(updatedTaxName.get(0));
				taxValue = taxValue + Double.parseDouble(newAmount);
				calcInTaxVal.put(updatedTaxName.get(0), taxValue);
			} else if (status.equalsIgnoreCase("AdjustFee")) {
				double feeValue = calcInFeeVal.get(updatedFeeName.get(0));
				feeValue = feeValue + Double.parseDouble(newAmount);
				calcInFeeVal.put(updatedFeeName.get(0), feeValue);
			}	
			...
			e.printStackTrace();
			Utility.catchException(driver,e, test_description, test_catagory, "VERIFY BOOKING UPDATE", testName, test_description,
					test_catagory, testSteps);

		} catch (Error e) {
			e.printStackTrace();
			Utility.catchError(driver,e, test_description, test_catagory, "VERIFY BOOKING UPDATE", testName, test_description,
					test_catagory, testSteps);
		}
		try {
			...
			testSteps.add("<b>=====Change Status of Ledger Account =====</b>");
			app_logs.info("===== Change Status of Ledger Account =====");
			if(status.equalsIgnoreCase("AddFee")) {
				navigation.clickLedgerAccountsTab(driver);
				app_logs.info("Click on ledger account");
				ledgerAccountClass.ChangeStatus(driver, extraFee, "Obsolete");
				app_logs.info("Update ledger account status");
			}
			RetryFailedTestCases.count = Utility.reset_count;
			Utility.AddTest_IntoReport(testName, test_description, test_catagory, testSteps);
		}catch (Exception e) {
			e.printStackTrace();
			Utility.catchException(driver,e, test_description, test_catagory, "Delete Tax", testName, test_description,
					test_catagory, testSteps);

		} catch (Error e) {
			e.printStackTrace();
			Utility.catchError(driver,e, test_description, test_catagory, "Delete Tax", testName, test_description,
					test_catagory, testSteps);
		}
	}
	@DataProvider
	...
	public Object[][] getDataOne() {
		...
		return Utility.getData("BUSAddAdjustVoidAnTaxAndFee", otaexcel);
