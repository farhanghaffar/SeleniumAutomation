...
public class ReportsV2TransactionsReportCheck extends TestCore  {
...
//			try {
				...
				if (isPayment.equalsIgnoreCase("Yes")) {
					...
					reservationV2Page.takePayment(driver, test_steps, false, null, "Test");
					...
					try {
						...
						try {
							reservationV2Page.displayVoidItems(driver, test_steps);
						}catch(Exception e) {
							reservationV2Page.displayVoidItems(driver, test_steps);
						}
						
						String strItems = "//table[contains(@data-bind,'filteredItems')]//tbody/tr/td[5]/span";
						String strDesc = "//table[contains(@data-bind,'filteredItems')]//tbody/tr/td[6]/a";
						String strAmount = "//table[contains(@data-bind,'filteredItems')]//tbody/tr/td[9]/span";
						Wait.WaitForElement(driver, strItems);
						Wait.wait10Second();
						...
						for (int i = 0; i < items.size(); i++) {
							...
							folioPaymentItemValues.put(items.get(i).getText(), itemsAmmount.get(i).getText());
							

						}
						
						app_logs.info("Items Description: "+itemDescription);
						app_logs.info("Items Values: "+folioPaymentItemValues);
						
				} catch(Exception e) {
					app_logs.info("Items Description: failed to get ");
					app_logs.info("Items Values: failed to get ");
				}
	...
	*/
		...
		try {
			...
			if (isPayment.equalsIgnoreCase("Yes")) {
				...
				report.validateDailyFlashReportPaymentsMethodTypes_RV2(driver, paymentMethod, beforePaymentDetails, afterPaymentDetails, ledgerAccounts, ledgerAmounts, folioPaymentItemValues, noOfRooms, test_steps);
...
*/
	...
	public Object[][] getData() {
		...
		return Utility.getData("TransactionsReportCheck", excelReservationV2);
	}
