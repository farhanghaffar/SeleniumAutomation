...
public class ReportsV2DailyFlashReportRoomRevenue  extends TestCore{
...
//	CPReservationPage reservationPage = new CPReservationPage();
	...
	HashMap<String, ArrayList<String>> beforeRoomRevenueDetails = new HashMap<>();
	HashMap<String, ArrayList<String>> afterRoomRevenueDetails = new HashMap<>();
	...
	public void validateLedgerBalancesReport(String TestCaseID, String Scenario, String dateRange, String dateEffective, 
		...
		try {
			...
			beforeRoomRevenueDetails=report.getPropertyPerformanceDataWithTable(driver, "Rooms Revenue", test_steps);
			
			app_logs.info("Revenue Details Before: "+beforeRevenueDetails);
			app_logs.info("Room Revenue Details Before: "+beforeRoomRevenueDetails);
...
//					reservationPage.rollBackReservation(driver, test_steps, guestFullName, "RESERVED");
		...
		try {
			...
			afterRoomRevenueDetails = report.getPropertyPerformanceDataWithTable(driver, "Rooms Revenue", test_steps);
			
			
			app_logs.info("Revenue Details After: "+afterRevenueDetails);
			app_logs.info("Revenue Details After: "+afterRoomRevenueDetails);
		...
		try {
			...
			if (!ledgerAccount.isEmpty()) {
				test_steps.add("========= Validating Room Revenue Type table =========");
				
				report.validateDailyFlashReportRoomsRevenue_RV2(driver, beforeRoomRevenueDetails, afterRoomRevenueDetails, ledgerAccounts, ledgerAmounts, folioItemValues, noOfRooms, isTaxExempt, test_steps);
	...
	public Object[][] getData() {
		...
		return Utility.getData("DailyFlashReportRoomRevenue", excelReservationV2);
	}
