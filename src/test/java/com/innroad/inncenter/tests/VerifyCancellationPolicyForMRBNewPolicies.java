...
public class VerifyCancellationPolicyForMRBNewPolicies extends TestCore {
...
//			login.login(driver, envURL, ClientCode, Username, Password);
		...
		try {
			...
			policyDetails = policies.createPolicies(driver, test_steps, "|", "", policyType, policyNames.get("policy1"), "", "",
					"", policyFeeTypeList.get("policy1"), policyFeeList.get("policy1"), policyChargesType, noOfDaysToCancel, cancelWithInType.split("\\|")[0], "No", "");
			...
			policyDetails = policies.createPolicies(driver, test_steps, "|", "", policyType, policyNames.get("policy2"), "", "",
					"", policyFeeTypeList.get("policy2"), policyFeeList.get("policy2"), policyChargesType, noOfDaysToCancel, cancelWithInType.split("\\|")[1], "No", "");
...
//					"", "Capture", policyName, policyType, roomBaseAmount, totalNights, tripTaxes, policyAttr1, policyAttrValue1);
			cancelPaymentWithTaxes = reservationPage.paymentCheckInPopupVerify(driver, test_steps, "Cancellation Payment", 
					cardExpDate, nameOnCard, "XXXX "+cardNumber.substring(cardNumber.length()-4), paymentMethod, paymentType,
					"", "Capture", policyName, policyType, roomBaseAmount, totalNights, tripTaxes, chargesAttribute, expPolicyFee);
	...
	public Object[][] getData() {
		return Utility.getData("ConcellationPolicyMRBNewPolicy", HS_EXCEL);
	}
