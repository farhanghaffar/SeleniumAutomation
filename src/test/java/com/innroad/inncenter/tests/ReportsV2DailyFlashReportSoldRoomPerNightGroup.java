...
public class ReportsV2DailyFlashReportSoldRoomPerNightGroup extends TestCore {
					...
					if (!Utility.validateString(roomsAction)) {
						if (!isQuote || Scenario.contains("Quote Book") || Scenario.contains("book") || !resType.contains("Group")) {
							if (resType.equalsIgnoreCase("Copy")) {							
								...
								app_logs.info("Trip Summary: "+ledgerAmounts);
								app_logs.info("Folio Balances: "+folioBalances);
							}else if(resType.equalsIgnoreCase("GroupBlock"))
							{
								soldRooms=noOfRooms;
								app_logs.info("soldRooms: "+soldRooms);
							}
							else
							{
								soldRooms = noOfRooms;
...
//			}
		...
		try {
			...
			test_steps.add("========= Validating Guest Count Statistics table =========");
						report.validateDailyFlashReportPropertyStatistics_RV2(driver, beforePropertyStatisticsDetails, afterPropertyStatisticsDetails, "Sold Room Nights", soldRooms, test_steps);
	...
	public Object[][] getData() {
		...
		return Utility.getData("DailyFlashSoldRoomNightGroup", excelReservationV2);
	}
