...
public class VerifyQuoteReservationV2 extends TestCore {
...
//	NewRoomClassesV2 obj1 = new NewRoomClassesV2();
	...
	public void verifyQuoteReservationV2(String TestCaseId, String TestCaseName, String RatePlanName,
			...
			String TaskStatus, String IsAddNotes, String NoteType, String NotesSubject, String NotesDescription)
			throws Exception {
...
//			nightlyRate.verifyTitleSummaryValue(driver, "Channel", summaryChannels, test_steps);
		...
		try {
			...
			res.verifyStatusBarDetail(statusBar, salutation + " " + guestFirstName + " " + guestLastName, true, true,
					"QUOTE", true, true, data.get("ReservationNumber"), true, true,
		...
		try {
			...
			res.enter_Notes(driver, test_steps, IsAddNotes, NoteType, NotesSubject, NotesDescription);
			reservationPage.verify_Notes(driver, test_steps, NoteType, NotesSubject, NotesDescription, "autores s", 1);
		} catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
...
//					TestCore.TestRail_AssignToID);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyQuoteReservationV2", HS_EXCEL);
	}
