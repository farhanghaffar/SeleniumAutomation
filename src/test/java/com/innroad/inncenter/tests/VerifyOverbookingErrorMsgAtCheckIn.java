...
public class VerifyOverbookingErrorMsgAtCheckIn extends TestCore {
	...
	public void verifyOverbookingOnCopyingReservation( String TestCaseID ,String url, String ClientCode, String Username, String Password,
			...
			String policy,String sortOrder ,String maxPerson, String details,String roomQuant,
		...
		test_name = "VerifyOverbookingAssignedReservationChckIn";
...
//			nav.Reservation_Backward_1(driver);
		...
		IsAssign = "No";
		try {
			app_logs.info("======Over book Reservation Creation=======");
			test_steps.add("======Over book Reservation Creation=======");
			res.click_NewReservation(driver, test_steps);
			...
			res.select_Rateplan(driver, test_steps, Rateplan, PromoCode);
			res.clickOnFindRooms(driver, test_steps);
			res.click_SelectRoomWithoutSelectingRoomNumber(driver, test_steps, RoomClassName);
			res.verify_OverBookingPopupAndClickYes(driver, test_steps, RoomClassName);
			...
			res.verify_StayInfo(driver, test_steps, CheckInDate, CheckOutDate, Adults, Children, RoomClassName, TripSummaryRoomCharges);
			...
			res.Click_CheckInButton(driver, test_steps);
			boolean falg1=res.verifyOverBookErrorMsgAtCheckIn(driver, test_steps);
			if(flag&&room.equalsIgnoreCase("Unassigned")&&falg1) {
				statusCode.set(0, "1");
				comments="successfully verified overbooking reservation and verified over booking error message on check in also";
			}else {
				comments="Assertion Error failed to verify overbooking reservation and failed to verify over booking error meaasge on check in";	
			}
