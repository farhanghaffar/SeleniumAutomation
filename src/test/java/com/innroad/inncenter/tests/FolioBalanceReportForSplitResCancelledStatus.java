...
public class FolioBalanceReportForSplitResCancelledStatus extends TestCore  {
	...
	public void folioBalanceReportForSplitReservation(String TestCaseID, String Scenario, String effectiveDate, 
		...
		{
		test_name = Scenario;
		test_description = "Validate foliobalace  Report<br>" + "<a href=''>" + "Click here to open TestRail: </a>";
		test_category = "ReportsV2 - foliobalace Report";
		...
		app_logs.info("guestFullName2: " +guestFullName2);		
		...
		String incidental= Utility.GenerateRandomNumber(10, 20);
			...
			for (int i = 0; i < noOfRooms; i++) {									
				...
				if(true && j<i)
				  ...
				  reservationType="Cancelled";
					...
					folioNew.addFolioLineItem(driver, test_steps, "Bar", incidental);
					reservationV2Page.cancelReservation(driver, test_steps, true, "yguy");					

					folioBalance=folioNew.getFolioBalanceOfFirstFolio(driver, test_steps)+10.00;					
					folioNew.makePayment(driver, test_steps,  String.format("%.2f",folioBalance), paymentMethod, "", "", cardNumber, guestFullName, cardExpDate, "123");					
					...
					String refund=String.format("%.2f",folioBalance);
					folioNew.refundPaymentinGuestFolio(driver, test_steps, Utility.removeBracketsAndMinusSign(refund), paymentMethod,
							cardNumber, guestFullName, cardExpDate,"123");				
					folioBalance= folioNew.getFolioBalanceOfFirstFolio(driver, test_steps);  					
		...
		app_logs.info("==========Verify Testcase id:849114==========");	
		test_steps.add("==========Verify Testcase id:849114==========");
