...
public class ValidateDerivedRatePlanV2 extends TestCore {
	...
	public void validateDerivedRatePlanV2(String delim, String RatePlanName, String FolioDisplayName,
		...
		test_name = "ValidateDerivedRatePlanV2";
		test_description = "Rates V2 - Create Derived Rate Plan (with always available) and update<br>"
		...
		String ratePlanNameBackup = RatePlanName;
		...
		try {
			...
			// ratePlanNames = ratesGrid.getRatePlanNames(driver, "active");

			ratePlanNames = ratesGrid.getActiveRatePlans(driver, "Nightly", "Interval",140);
			// need to write code for interval and nightly rate Plan
			existingRateName = ratePlanNames.get(0);
			
			testSteps.add("Get all("+ratePlanNames.size()+") Nightly and Interval rate plan " + ratePlanNames);
			app_logs.info("Get all("+ratePlanNames.size()+") Nightly and Interval rate plan " + ratePlanNames);
		...
		try {
			...
			app_logs.info("=================== "
					+ "Verify that the Rate Plan deriving rates from shows list of all Active Rate Plans for the Property (which 							includes Rate Plans created in V2 (Nightly Package and Interval Rates)"
							.toUpperCase()
					+ " ======================");
			derivedRate.verifyRatePlansDisplayed(driver, ratePlanNames, testSteps);
	...
	public Object[][] getData() {
		...
		return Utility.getData("ValidateDerivedRatePlanV2", envLoginExcel);
	}
