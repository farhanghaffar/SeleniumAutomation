...
public class VerifyNoShowCPReservationMRB extends TestCore{
	...
	public void verify_NoShow_WithoutPolicy(String TestCaseID ,String url, String ClientCode, String Username, String Password,String CheckInDate,String CheckOutDate,String Adults,String Children,String Rateplan,String PromoCode,String IsSplitRes,
			...
			String WithPolicy) throws InterruptedException, IOException, ParseException {
		Utility.RoomNo ="1";boolean isExecutable=Utility.getResultForCase(driver, TestCaseID);
		if(isExecutable) {
			Utility.initializeTestCase(TestCaseID, caseId, statusCode, statusCode,"");
		/*
		 * caseId.add("848355"); caseId.add("848410"); caseId.add("848459");
		 * caseId.add("848460"); caseId.add("848461");
		 * 
		 * statusCode.add("4"); statusCode.add("4"); statusCode.add("4");
		 * statusCode.add("4"); statusCode.add("4");
		 */
		...
		try {
			...
			String policy = "Non-Smoking";
			String sortOrder = "1";
			String maxPerson = "15";
			String details = "R_ Room For Automation";
			String roomQuant = "100";
			String roomName= "1";
			String statinId = "1";
			...
			getTest_Steps.clear();
			getTest_Steps = roomClassObj.createRoomClassWithDescriptionV2(driver, getTest_Steps, roomClassName1, roomClassAbb1,
		...
		/*	newRoomclass.deleteRoomClassV2(driver, roomClassNameWithoutNum);
			//newRoomclass.createRoomClassV2(driver, roomClassName1, roomClassAbb1, maxAdults, maxPersons, roomQuantity, test, test_steps);
...
*/
			...
			getTest_Steps.clear();
			getTest_Steps = roomClassObj.createRoomClassWithDescriptionV2(driver, getTest_Steps, roomClassName2, roomClassAbb2,
			policy, sortOrder, maxAdults, maxPerson, details, roomQuant, roomName, statinId, sortOrder, zone);
			test_steps.addAll(getTest_Steps);
			test_steps.add("New Room Class Created");
			app_logs.info("New Room Class Created");
			//newRoomclass.createRoomClassV2(driver, roomClassName2, roomClassAbb2, maxAdults, maxPersons, roomQuantity, test, test_steps);
		...
		try
		{
			if (WithPolicy.equals("No")) {
				...
				res.enter_Adults(driver, test_steps, "1");
				res.enter_Children(driver, test_steps, "1");
				res.select_Rateplan(driver, test_steps, "Rack Rate",PromoCode);
				res.clickOnFindRooms(driver, test_steps);
				
				String count="//span[text()='"+roomClassName1+"']/../..//span[contains(@data-bind,'roomAvailabilityCount')]";
				String text=driver.findElement(By.xpath(count)).getText();
				
				if(text.trim().contains("1 rooms")) {
					test_steps.add("Successfully verified the room are released after no show");
					app_logs.info("Successfully verified the room are released after no show");
				}else {
					test_steps.add("Assertion error: failed to verifiy the room are released after no show");
					app_logs.info("Assertion error: failed to verifiy the room are released after no show");
				}
		...
		try
		{
			if(WithPolicy.equals("Yes"))
			{
				...
				app_logs.info("<b>****Start Verifying No Show Successful Window****</b>");
				...
				res.clickCloseButtonOfNoShowSuccessfully(driver, test_steps);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyNoShowCPReservationMRB", SanityExcel);
	}
