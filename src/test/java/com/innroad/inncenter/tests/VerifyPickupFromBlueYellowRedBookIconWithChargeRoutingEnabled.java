...
public class VerifyPickupFromBlueYellowRedBookIconWithChargeRoutingEnabled extends TestCore {
	...
	public void verifyPickupFromBlueBookIconWithChargeRoutingEnabled(String TestCaseID, String url, String clientcode,
			String username, String password, String AccountName, String MargetSegment, String Referral,
			String AccountFirstName, String AccountLastName, String Phonenumber, String Address1, String city,
			String Country, String State, String Postalcode, String ChargeRoutingAllItem,
			String ChargeRoutingRoomChargeOnly, String BlockName, String RoomPerNight, String RoomClassAbb,
			String RoomClassName, String bedsCount, String maxAdults, String maxPersons, String roomQuantity,
			String LineCategory, String LineAmount, String checkInDate, String checkOutDate, String adults,
			String children, String rateplan, String Salutation, String CardNumber, String NameOnCard,
			String PaymentType, String RateName, String BaseAmount, String AddtionalAdult, String AdditionalChild,
			String AssociateSession, String RatePolicy, String RateDescription, String ratePlan,
		...
		String test_description = "Create Group Account with Charge Routing Enabled.<br>"
				...
				+ "Click here to open TestRail: C554170</a><br/>" + "Pickup from Yellow/Red book button.<br>"
		...
		try {
			...
			assertEquals(beforeBookIconClass, "book", "Book Icon Color Not Matched");

			getTest_Steps.clear();
			getTest_Steps = group.bookIconClick(driver, RoomClassName);
			test_steps.addAll(getTest_Steps);
			
			comments="Verified charge routing for all items of group pick up";
			comments=comments+". Picked up from blue book icon";
		...
		try {
			...
			resNO1 = reservationPage.get_ReservationConfirmationNumber(driver, test_steps);
			...
			comments=comments+". Verify able to create group reservation with Blue Book icon";
			comments=comments+". Verified the flow of picking up and creating a Group block Reservation via Group Pickup button from Dashboard";
		} catch (Exception e) {
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(TestName, TestDescription, TestCategory, test_steps);
		...
		// after Creation of Reservation verifying folio line items from reservation
		// side
		/*
		 * try { test_steps.
		 * add("=============verifying folio line items from reservation side============"
		 * ); app_logs.
		 * info("==============verifying folio line items from reservation side============="
		 * );
		 * 
		 ...
		 * reservationPage.selectFolioOption(driver, "Guest Folio");
		 * test_steps.addAll(getTest_Steps);
		 * 
		 * getTest_Steps.clear(); getTest_Steps = res.verifyLineitemsExist(driver,
		 * false, 0); test_steps.addAll(getTest_Steps);
		 * 
		 ...
		 * 
		 * getTest_Steps.clear(); DecimalFormat df = new DecimalFormat("0.00");
		 * df.setMaximumFractionDigits(2); String
		 * amount=df.format(Double.parseDouble(BaseAmount));
		 * Utility.app_logs.info(amount);
		 * 
		 * getTest_Steps = res.VerifyFolioLineItem(driver, "Room Charge", amount, "1");
		 * test_steps.addAll(getTest_Steps); //Bug while adding preview folio options
		 * 
		...
		try {
			...
			reservationPage.enter_MarketSegmentDetails(driver, test_steps, "", MargetSegment, Referral);
			reservationPage.clickBookNow(driver, test_steps);
			resNO2 = reservationPage.get_ReservationConfirmationNumber(driver, test_steps);
			reservationPage.get_ReservationStatus(driver, test_steps);
			reservationPage.clickCloseReservationSavePopup(driver, test_steps);
			test_steps.add("Successfully Associated Account to  Reservation");
			app_logs.info("Successfully Associated Account to Reservation");
		 ...
		 * 
		 * getTest_Steps.clear(); getTest_Steps = res.verifyLineitemsExist(driver, true,
		 * 1); test_steps.addAll(getTest_Steps); DecimalFormat df = new
		 * DecimalFormat("0.00"); df.setMaximumFractionDigits(2); String amount =
		 * df.format(Double.parseDouble(BaseAmount)); Utility.app_logs.info(amount);
		 * 
		 * getTest_Steps.clear(); getTest_Steps = res.VerifyFolioLineItem(driver,
		 * "Room Charge", amount, "1"); test_steps.addAll(getTest_Steps);
		 * 
		 * } catch (Exception e) { if (Utility.reTry.get(testName) == Utility.count) {
		 * RetryFailedTestCases.count = Utility.reset_count;
		 * Utility.AddTest_IntoReport(TestName, TestDescription, TestCategory,
		...
		try {
			...
			getTest_Steps = group.bookIconClick(driver, RoomClassName);
			test_steps.addAll(getTest_Steps);
			comments=comments+". Picked up red book icon";
		...
		try {
			...
			reservationPage.enter_MarketSegmentDetails(driver, test_steps, "", MargetSegment, Referral);
			reservationPage.clickBookNow(driver, test_steps);
			resNO3 = reservationPage.get_ReservationConfirmationNumber(driver, test_steps);
			reservationPage.get_ReservationStatus(driver, test_steps);
			reservationPage.clickCloseReservationSavePopup(driver, test_steps);
			test_steps.add("Successfully Associated Account to  Reservation");
			app_logs.info("Successfully Associated Account to Reservation");
			comments=comments+". Verified able to create group reservation with red Book icon.";
		} catch (Exception e) {
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(TestName, TestDescription, TestCategory, test_steps);
		...
		/*	comments = "Verified pick up from yellow book icon. " + "Verified pick up from blue book icon. "
					+ "Verified pick up from red book icon. "
					+ "Reservation is created with yellow book icon with Reservation number:" + resNO2 + " ."
					+ "Reservation is created with blue book icon with Reservation number:" + resNO1 + " ."
					+ "Reservation is created with red book icon with Reservation number:" + resNO3 + " ."
					+ "Verified the availability count when the reservation is picked up from yellow book icon. "
					+ "Verified the title in reservation page on clicking blue book icon from groups page. "
					+ "Group block available counts are adjusted to the booked room from "+beforeAvailableRoom+ " to "+afterAvailableRoom+ " .";
			statusCode.set(0, "1");
			statusCode.set(1, "1");
			statusCode.set(2, "1");
			statusCode.set(3, "1");
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyPikupBluBookChargeRouting", BEExcel);
	}
