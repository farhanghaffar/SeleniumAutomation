...
public class VerifyUserCanProvideNotesWhileRefundV2 extends TestCore
{
	...
	{
		return Utility.getData("VerifyUserProvideNotes4Refund", excelRiddhi);
	}
	...
	public void verifyUserCanProvideNotesWhileRefundV2(String TestCaseId, String TestCaseName, 
			String RatePlanName,String marketSegment, String Referral, 
			String PaymentMethod, String CardNumber, String NameOnCard, String ExpiryDate, 
			String RefundAmount, String RefundNotes) throws Exception 
	{

		Utility.initializeTestCase(TestCaseId, caseId, statusCode, comments, "907821");
...
//			loginResV2(driver);
	  ...
	  catch (Exception e) 
	  {
		...
		String taxExemptID = "TAX_AUTO0011";
		String refundNotes = RefundNotes;
		...
		try
		{
			...
			//Click on Refund Button From Details Tab >> Enter all fields >> Click on refund button to make refund
			guestInfoObj.refundPaymentFromResDetailsTabwithRefundNotes(driver, test_steps, refundAmount, true, refundNotes);
			...
			amount = "-$" + amount + "";						
			folioNewobj.verifyFolioLineItems(driver, test_steps, CheckInDate, PaymentMethod, amount, NameOnCard, fourDigitCardNo, ExpiryDate);
			folioNewobj.verifyFolioLineItemNote(driver, test_steps, CheckInDate, PaymentMethod, refundNotes);
			
			//Navigate to History Log and verify History Log for Refund
			...
			res.verifyHistoryWithRefundedPayment(driver, test_steps, refundAmount, CardNumber, PaymentMethod, ExpiryDate, NameOnCard);
		}
		catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
