...
public class VerifyAbleToCreateGroupReservationWithYellowBookIcon extends TestCore {
	...
	public void verifyAbleToCreateGroupReservationWithBlueBookIcon(String TestCaseID, String rateName, String baseAmount,
			...
			String isChecked) throws Exception {
		...
		try {
			...
			comments = " Created reservation from yellow book icon with number (" + reservationNumber + ")";
			statusCode.add(0, "1");
			RetryFailedTestCases.count = Utility.reset_count;
			Utility.AddTest_IntoReport(ScriptName, TestDescription, TestCategory, testSteps);
		} catch (Exception e) {
	...
	public Object[][] getData() {
		...
		return Utility.getData("CreateResFromYellowIcon", envLoginExcel);
	}
