...
public class VerifyOverbookingMsgResModify extends TestCore {
	...
	public void verifyOverbookingOnCopyingReservation(String TestCaseID,String url, String ClientCode, String Username, String Password,
		...
		test_name = "VerifyOverbookingReservationModify";
		...
		try {
			...
			app_logs.info("=====Reservation Creation=======");
			test_steps.add("======Reservation Creation=======");
			res.click_NewReservation(driver, test_steps);
			...
			res.selectReferral(driver, Referral);
			res.clickBookNow(driver, test_steps);
			String reservationNumber1=res.get_ReservationConfirmationNumber(driver, test_steps);
			res.clickCloseReservationSavePopup(driver, test_steps);
			...
			res.verify_ReservationInHistoryTab(driver, test_steps, reservationNumber1);
			if(depositAmount>0) {
			...
			res.ClickEditStayInfo(driver, test_steps);
			res.clickChangeStayDetails(driver);
			boolean flag1=res.verify_OverBookingAtReservationModification(driver, test_steps, RoomClassName);
