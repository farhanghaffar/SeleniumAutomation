...
public class VerifyEditAndDeleteTask extends TestCore{
	...
	public void Verify_Notes_Tasks_CP_Reservation(String url, String ClientCode, String Username, String Password,String PropertyName,String CheckInDate,String CheckOutDate,String Adults,String Children,String Rateplan,String PromoCode,
		...
		try {
			...
			String taskType=res.editTask(driver, test_steps);
			
			comments="user is able to update the task";
			statusCode.set(0, "1");
			
			nav.Admin(driver);
			nav.Roles(driver);
			admin.selectAdminstatorRole(driver, test_steps);
			admin.makeDeleteTaskEnableOrDisable(driver, test_steps,false);
			test_steps.add("Delete task entitlement is removed for the current user");
			app_logs.info("Delete task entitlement is removed for the current user");
			...
			boolean flag=res.verifyDeleteTaskIsDisabled(driver, test_steps, taskType);
			if(flag) {
				statusCode.set(2, "1");
			}
			nav.Admin(driver);
			nav.Roles(driver);
			admin.selectAdminstatorRole(driver, test_steps);
			admin.makeDeleteTaskEnableOrDisable(driver, test_steps,true);
			test_steps.add("Delete task entitlement is enabled for the current user");
			app_logs.info("Delete task entitlement is enabled for the current user");
			
			login.logout(driver);
			login_CP(driver);
			res.Search_ResNumber_And_Click(driver, reservation);
			
			res.deleteTask(driver, test_steps, taskType);
			statusCode.set(1, "1");
			comments= comments+ " and also able to delete the task";
	...
	public void closeDriver() throws MalformedURLException, IOException, APIException {
		...
		Utility.updateTestCaseAndCloseDriver(driver, caseId, statusCode, comments, TestRail_AssignToID);
		driver.quit();
	}
}
