...
public class CreateMRBReservationV2 extends TestCore {
...
//	NewRoomClassesV2 obj1 = new NewRoomClassesV2();
	...
	public void verifyQuoteReservationV2(String TestCaseId, String TestCaseName, String RatePlanName,
			String roomClassName,String isTask, String TaskCategory,
...
//			nightlyRate.verifyTitleSummaryValue(driver, "Channel", summaryChannels, test_steps);
		...
		String guestFirstName = "John|John";
		String guestLastName = "Wick_" + Utility.generateRandomNumberWithGivenNumberOfDigits(4) + "|" +"Wick_" + Utility.generateRandomNumberWithGivenNumberOfDigits(4);
		...
		try {
			...
			data = res.createReservation(driver, test_steps, "From Reservations page", CheckInDate, CheckOutDate, adult,
					children, RatePlanName+"|"+RatePlanName, "", roomClassName+"|"+roomClassName, roomClassAB+"|"+roomClassAB, salutation, guestFirstName, guestLastName,
			...
			res.verifyBookButton(driver, test_steps);
		} catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
		...
		try {
			...
			res.verifyStatusBarDetail(statusBar, salutation + " " + guestFirstName + " " + guestLastName, false, true,
		...
		/*	res.closeAllOpenedReservations(driver);
			...
			String reservationNumber = statusBar.getSB_CONFIRMATION_NO();
			
			res.basicSearch_WithReservationNo(driver, reservationNumber, false);
			
			searchReservation.verifyReservationStatusWithName(driver, guestFirstName + " " + guestLastName, "Quote", test_steps);
			
			reservationPage.clickOnAdvance(driver);
			getTest_Steps.clear();
			getTest_Steps = searchReservation.advanceSearchWithReservationNumber(driver, reservationNumber);
			test_steps.addAll(getTest_Steps);
			
			getTest_Steps.clear();
			getTest_Steps = searchReservation.advanceSearchWithStatus(driver, "Quote");
			test_steps.addAll(getTest_Steps);
			...
			*/
			res.enterTask(driver, test_steps, TaskCategory, TaskType, TaskDetails, TaskRemarks,
...
//					TestCore.TestRail_AssignToID);
	...
	public Object[][] getData() {
		...
		return Utility.getData("CreateMRBReservationV2", HS_EXCEL);
	}
