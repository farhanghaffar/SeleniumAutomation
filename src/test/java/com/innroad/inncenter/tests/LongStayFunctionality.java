...
public class LongStayFunctionality extends TestCore {
	...
	public void checkRunMode() {
		...
		if (!Utility.isExecutable(scriptName, envLoginExcel))
	...
	public void longStayFunctionality(
			String TestCaseID, String propertyName, String MarketeSegment, String referral, String firstName, 
			String lastName,
		...
		if(!Utility.validateString(TestCaseID)) {
			...
			for(int i=0;i<testcase.length;i++) {
				System.out.println(testcase[i] + " : " + i);
	...
	//		folio.VerifyTaxExempt(driver, true);
		...
		try {
			...
			System.out.println(caseId.get(2) + " status code value before change :" +  statusCode.get(2));
			statusCode.add(0, "1");
			System.out.println(caseId.get(2) + " status code value after change :" +  statusCode.get(2));
		...
		try {
			...
			comments = " Verified in the Folio that the Back dated Reservations are getting Tax Exempted which satisfies Long Stay after Clicking on Set Now in Night Audit";
			statusCode.add(1, "1");			
			statusCode.add(2, "1");
	...
	public Object[][] getData() {
		...
		return Utility.getData("LongStayFunctionality", envLoginExcel);
	}
