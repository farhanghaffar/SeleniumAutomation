...
public class ReportsV2TransactionsReportVisaCard  extends TestCore  {
...
//	CPReservationPage reservationPage = new CPReservationPage();
	...
	HashMap<String, Double> creditcardPaymentAmount= new HashMap<>();
	
	HashMap<String, ArrayList<String>> beforeCreditCardTypeTotal = new HashMap<>();
	HashMap<String, ArrayList<String>> afterCreditCardTypeTotal = new HashMap<>();
	HashMap<String, String> beforeCreditCardTransactionTypeTotal=new HashMap<>();
	HashMap<String, String> afterCreditCardTransactionTypeTotal = new HashMap<>();
	...
	public void validateLedgerBalancesReport(String TestCaseID, String Scenario, String dateRange, String dateEffective, 
		...
		try {
			...
			beforeCreditCardTypeTotal=report.getCreditCardTypeDetailofTransectionReport(driver, test_steps);
			app_logs.info("Before Summary view: "+beforeCreditCardTypeTotal);
			test_steps.add("Before Summary view: "+beforeCreditCardTypeTotal);
			
			
			beforeCreditCardTransactionTypeTotal=report.getCreditCardTransactionTypeInTransectionReport(driver, test_steps);
			app_logs.info("Before Summary view: "+beforeCreditCardTransactionTypeTotal);
			test_steps.add("Before Summary view: "+beforeCreditCardTransactionTypeTotal);
...
//					reservationPage.rollBackReservation(driver, test_steps, guestFullName, "RESERVED");
		...
		if (paymentData.get("Payment Type").equalsIgnoreCase("MC")||paymentData.get("Payment Type").equalsIgnoreCase("Visa")||paymentData.get("Payment Type").equalsIgnoreCase("Discover")) {
			expChanges2.put("Credit Card", Double.parseDouble(amount));	
			
		} else if (paymentData.get("Payment Type").equalsIgnoreCase("Cash")) {
			expChanges2.put("Cash", Double.parseDouble(amount));								
		} else if (paymentData.get("Payment Type").equalsIgnoreCase("Check")) {
			expChanges2.put("Check", Double.parseDouble(amount));								
		}
		expChanges2.put("Total", Double.parseDouble(amount));
		test_steps.add("Changes expected in report after reservation creation is : <b>"+expChanges2+"</b>");
		
		if (paymentData.get("Payment Type").equalsIgnoreCase("MC"))
				{
			creditcardPaymentAmount.put("MC", Double.parseDouble(amount));
				}else if(paymentData.get("Payment Type").equalsIgnoreCase("Visa"))
				{
					creditcardPaymentAmount.put("Visa", Double.parseDouble(amount));
				}else if (paymentData.get("Payment Type").equalsIgnoreCase("Discover"))
				{
					creditcardPaymentAmount.put("Discover", Double.parseDouble(amount));
				}
			
		
		
		app_logs.info("Expected changes: "+expChanges2);
		...
		try {
			...
			afterCreditCardTypeTotal=report.getCreditCardTypeDetailofTransectionReport(driver, test_steps);
			app_logs.info("After Summary view: "+afterCreditCardTypeTotal);
			test_steps.add("After Summary view: "+afterCreditCardTypeTotal);
			
			
			afterCreditCardTransactionTypeTotal=report.getCreditCardTransactionTypeInTransectionReport(driver, test_steps);
			app_logs.info("After Summary view: "+afterCreditCardTransactionTypeTotal);
			test_steps.add("After Summary view: "+afterCreditCardTransactionTypeTotal);
			
			report.validateCreditCardTransactionTypeDetailInTransectionReport_RV2(driver, transactionType, beforeCreditCardTransactionTypeTotal, afterCreditCardTransactionTypeTotal, paymentTransactionTypeAndAmount, test_steps);
			
			
			report.validateCreditCardTypeDetailofTransectionReport_RV2( driver,  paymentMethod, beforeCreditCardTypeTotal,
					afterCreditCardTypeTotal,creditcardPaymentAmount, test_steps);
			
			
			
			test_steps.add("**************<b> COMPARING SUMMARY VIEW REPORT BEFORE AND AFTER </b>**************");
			report.compareTransactionReportSummaryView_RV2(test_steps, summaryViewReportBeforeAction, summaryViewReportAfterAction, expChanges2,paymentMethod);
			...
			report.validateReservationAddedToDetailedViewForTransactionReport_RV2(driver, test_steps, checkInDate, guestFullName, reservationNumber, 
					paymentMethod, transactionType, cardNumber, guestFullName, amount);
	...
	public Object[][] getData() {
		...
		return Utility.getData("TransactionsReportVisaCard", excelReservationV2);
	}
