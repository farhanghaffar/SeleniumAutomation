...
public class VerifyEditingOfThePoliciesForAnExistingReservation extends TestCore {
	...
	String roomClassNameWithoutNum, rateNameWithoutNum, policyNameWithoutNum, typeToValidate, policyDesc, policyText,
			...
			tripPaid, tripBalance, nameOnCard, roomChargeAmount, guestFullName;
	...
	public void verifyEditingOfThePoliciesForAnExistingReservation(String TestCaseID, String Scenario, String ClientCode, String Username, String Password, String resType,
...
//		String ratePlan2 = ratePlan.split("\\|")[1];
		...
		try {
			...
			policyNames.put("Cancel", "CancellationPolicy" + Utility.GenerateRandomNumber());
			policyNames.put("NoShow", "NoShowPolicy" + Utility.GenerateRandomNumber());
...
//			login.login(driver, envURL, ClientCode, Username, Password);
		...
		try {
			...
			admin.enableSpecialFunctionsForRoleUsingType(driver, "Restrict Policy Changes", false, test_steps);
		...
		try {
			...
			policies.deleteAllPoliciesNew(driver, "Cancellation", test_steps);
			policies.deleteAllPoliciesNew(driver, "No Show", test_steps);
			HashMap<String, ArrayList<String>> policyDetails = new HashMap<>();
			policyDetails = policies.createPolicies(driver, test_steps, "|", ",", "Cancellation", policyNames.get("Cancel"), "", "",
					"", policyFeeType, policyFee, policyChargesType, noOfDaysToCancel, cancelWithInType, "No", "");
			policyTextList.put("Cancel", policyDetails.get(policyNames.get("Cancel")).toString().replace("[", "").replace("]", "").trim());
			Wait.wait3Second();
			String noShowPolicyNmae = "NoShow"+Utility.generateRandomNumber();
			policyDetails = policies.createPolicies(driver, test_steps, "", "", "No Show", "", "", "",
					policyNames.get("NoShow"), policyFeeType, policyFee, policyChargesType, "", "", "No", "");
			policyTextList.put("NoShow", policyDetails.get(policyNames.get("NoShow")).toString().replace("[", "").replace("]", "").trim());
...
//			rateGrid.clickOnGivenPolicy(driver, "Cancellation", true);
...
//			nightlyRate.selectPolicy(driver, policyType, true, test_steps);
			nightlyRate.selectPolicy(driver, "Cancellation", policyNames.get("Cancel"), true, test_steps);
		...
		try {
			...
			reservationPage.rollBack(driver, test_steps);
			...
			reservationPage.verifyCancellationPolicy(driver, test_steps, policyNames.get("Cancel"), policyTextList.get("Cancel"));
			test_steps.add("<b>========== Reservation status: Reserved - Verifying Policy dropdown is able to Edit and Changing policy and verify at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownEdit(driver, test_steps);
			reservationPage.selectPolicyFromPolicyDisclaimersTab(driver, policyNames.get("NoShow"), policyTextList.get("NoShow"), test_steps);
			
			reservationPage.changeReservationStatus(driver, test_steps, "Confirmed");
			test_steps.add("<b>========== Reservation status: Confirmed - Verifying Policy dropdown is able to Edit at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownEdit(driver, test_steps);
			
			reservationPage.changeReservationStatus(driver, test_steps, "Guaranteed");
			test_steps.add("<b>========== Reservation status: Guaranteed - Verifying Policy dropdown is able to Edit at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownEdit(driver, test_steps);
			
			reservationPage.changeReservationStatus(driver, test_steps, "On Hold");
			test_steps.add("<b>========== Reservation status: On Hold - Verifying Policy dropdown is able to Edit at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownEdit(driver, test_steps);	
			reservationPage.changeReservationStatus(driver, test_steps, "Reserved");
			
			comments = "Verified Editing of the policies for an existing reservation which has status in Reserved/confirmed/guaranteed/onhold";
			statusCode.add(0, "1");	   
		}catch (Exception e) {
			e.printStackTrace();
			app_logs.info(e.toString());
		...
		try {
			...
			test_steps.add("<b>========== Reservation status: In-House/Check-In - Verifying Policy dropdown is not able to Edit at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownNoEdit(driver, test_steps);
			
			reservationPage.checkoutReservation(driver, test_steps);
			test_steps.add("<b>========== Reservation status: Departed/CheckOut - Verifying Policy dropdown is not able to Edit at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownNoEdit(driver, test_steps);
			...
			test_steps.add("<b>========== Reservation status: Cancelled - Verifying Policy dropdown is not able to Edit at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownNoEdit(driver, test_steps);
			...
			reservationPage.searchWithGuestName(driver, test_steps, guestFullName);
			reservationPage.clickOnGuestName(driver, test_steps);
			reservationPage.noShowReservation(driver, test_steps);
			test_steps.add("<b>========== Reservation status: NoShow - Verifying Policy dropdown is not able to Edit at Policies And Disclaimers tab ==========</b>");
			reservationPage.verifyNoShowPolicyDropDownNoEdit(driver, test_steps);
			
			comments = "Verified No- Editing of the policies for an existing reservation having In-House/departed/cancelled/no show status";
			statusCode.add(1, "1");	 
		}catch (Exception e) {
			app_logs.info(e.toString());
			e.printStackTrace();
	...
	public Object[][] getData() {
		return Utility.getData("VerifyEditingOfThePolicies", HS_EXCEL);
	}
