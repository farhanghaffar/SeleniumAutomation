...
public class VerifyAuthorizationThroughTakePaymentButton extends TestCore {
	...
	public void verifyAuthorizationThroughTakePaymentButton(String TestCaseId, String TestCaseName,
			...
			String subAccountId, String username, String password, String restAPIKey, String hituserId, String hitKey,
			String transactionKeyCode, String associateProperties, String checkInDate, String checkOutDate,
			String adults, String children, String rateplan, String roomClass, String rate, String salutation,
			String guestFirstName, String guestLastName, String paymentType, String cardNumber, String nameOnCard,
			String marketSegment, String referral, String amount, String TakePaymentType, String IsChangeInPayAmount)
			throws Exception {
		if (Utility.getResultForCase(driver, TestCaseId)) {
			...
			try {
				...
				data = res.createReservation(driver, test_steps, "From Reservations page", checkInDate, checkOutDate,
						...
						"", "", "", "", "", "", "", "", "", false, marketSegment, referral, paymentType, cardNumber,
						nameOnCard, yearDate, 0, "", false, "", "", "", "", "", "", "", "", "", "", "", "", "", "",
						false, "");
...
//Authorize from Take payment 
			try {
				app_logs.info("/Authorize from Take payment");
				res.clickTakePayment(driver, test_steps);
				res.takePayment(driver, test_steps, paymentType, cardNumber, nameOnCard, CardExpDate1,
						Utility.splitInputData(TakePaymentType).get(1), IsChangeInPayAmount, amount, "", "", true,
						typeList, true);
				res.click_Folio(driver, test_steps);
				folioNew.verifyFolioLineItems(driver, test_steps, checkInDate, paymentType, amount, nameOnCard,
						last4Digit, CardExpDate1);
			...
			try {
				app_logs.info("Capture from Pay button");
				res.click_Folio(driver, test_steps);
				String amount1 = Utility.convertDecimalFormat(amount);
				amount1 = "$" + amount1 + "";
				guestFolio.takePaymentFromGuestFolio_RV2(driver, test_steps,
						Utility.splitInputData(TakePaymentType).get(0), amount, paymentType, cardNumber, nameOnCard);
				folioNew.verifyFolioLineItems(driver, test_steps, checkInDate, paymentType, amount1, nameOnCard,
						last4Digit, yearDate);
				app_logs.info("******************* Successfully Verified Folio **********************");
				res.click_History(driver, test_steps);
			...
			try {
				app_logs.info("Authorize from Pay button ");
				res.click_Folio(driver, test_steps);
				String amount1 = Utility.convertDecimalFormat(amount);
				amount1 = "$" + amount1 + "";
				guestFolio.takePaymentFromGuestFolio_RV2(driver, test_steps,
						Utility.splitInputData(TakePaymentType).get(1), amount, paymentType, cardNumber, nameOnCard);
				folioNew.verifyFolioLineItems(driver, test_steps, checkInDate, paymentType, amount1, nameOnCard,
						last4Digit, yearDate);
				app_logs.info("******************* Successfully Verified Folio **********************");
			...
			try {
				app_logs.info("Capture from Take Payment button.");
				res.switchDetailTab(driver, test_steps);
				res.clickTakePayment(driver, test_steps);
				res.takePayment(driver, test_steps, paymentType, cardNumber, nameOnCard, CardExpDate1,
						Utility.splitInputData(TakePaymentType).get(0), IsChangeInPayAmount, amount, "", "", true,
						typeList, true);
				res.click_Folio(driver, test_steps);
				folioNew.verifyFolioLineItems(driver, test_steps, checkInDate, paymentType, amount, nameOnCard,
						last4Digit, CardExpDate1);
	...
	public Object[][] getData() {
		return Utility.getData("AuthPaymentWithTakePaymnt", SNExcel);
	}
