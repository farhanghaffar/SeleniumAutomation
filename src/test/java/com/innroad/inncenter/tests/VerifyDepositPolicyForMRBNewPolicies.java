...
public class VerifyDepositPolicyForMRBNewPolicies extends TestCore {
...
//			login.login(driver, envURL, ClientCode, Username, Password);
		...
		try {
			...
			policyDetails = policies.createPolicies(driver, test_steps, "|", "", policyType, "", policyNames.get("policy1"), "",
					"", policyFeeTypeList.get("policy1"), policyFeeList.get("policy1"), policyChargesType, "", "", "No", "");
			...
			policyDetails = policies.createPolicies(driver, test_steps, "|", "", policyType, "", policyNames.get("policy2"), "",
					"", policyFeeTypeList.get("policy2"), policyFeeList.get("policy2"), policyChargesType, "", "", "No", "");
...
//			rateGrid.clickOnGivenPolicy(driver, policyType, true);
		...
		try {
			...
			tripPaid = reservationPage.get_TripSummaryPaidAmount(driver, test_steps).replace(".00", "");			
			app_logs.info("Paid amount: "+tripPaid);
...
//		try {			
			...
			test_steps.add("========== Verify Deposit Policy Amount==========");
			Assert.assertEquals(tripPaid, expPolicyFee, "Failed to validate Deposit policy amount");
			test_steps.add("Successfully verified Deposit policy amount");
			app_logs.info("Successfully verified Deposit policy amount");

			comments = "Verify Deposit policy calculation for MRB when each room has different policy fee";
...
//		}
	...
	public Object[][] getData() {
		return Utility.getData("DepositPolicyMRBNewPolicies", HS_EXCEL);
	}
