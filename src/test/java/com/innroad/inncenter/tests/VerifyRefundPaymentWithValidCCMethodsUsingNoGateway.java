...
public class VerifyRefundPaymentWithValidCCMethodsUsingNoGateway extends TestCore {
	...
	public void verifyRefundPaymentWithValidCCMethodsUsingNoGateway(String TestCaseId, String TestCaseName,
			...
			String cardNumber, String nameOnCard, String marketSegment, String referral, String amount) throws Exception {
		...
		try {
			...
			folioNew.refundPaymentinGuestFolio(driver,test_steps,amount,paymentType,cardNumber,nameOnCard,yearDate,"");
			String last4Digit= Utility.getCardNumberHidden(cardNumber);
			amount = Utility.convertDecimalFormat(amount);
			amount = "-$" + amount + "";
			folioNew.verifyFolioLineItems(driver, test_steps, checkInDate, paymentType, amount, nameOnCard, last4Digit, yearDate);
		...
		try {
			...
			for(int i=0;i<Utility.testId.size();i++) {
				Utility.testCasePass(Utility.statusCode, i, Utility.comments, "Verify payment and Refund with valid  card ");
			}
	...
	public Object[][] getData() {
		return Utility.getData("VerifyRefundPaymentWithValidCC", SanityExcel);
	}
