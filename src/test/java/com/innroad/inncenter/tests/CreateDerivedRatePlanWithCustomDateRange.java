...
public class CreateDerivedRatePlanWithCustomDateRange extends TestCore {
	...
	public void createDerivedRatePlanWithCustomDateRange(String clientName, String delim,
			...
			String policiesName, String percentValue, String currencyValue,
			String parentRatePlanName,
			String parentBaseAmount, String addtionalAdult, String additionalChild, String parentRateFolioName,
			String parentRateDescription, String parentRoomClass, String parentRoomClassSize, String istakenRulesFromParentRateplan, 
			...
			String packageRatePlanProductAmount,
			String seasonName, String seasonStartDate,
		...
		try {
			...
			if(parentRatePlanType.equalsIgnoreCase("Package rate plan")){
				...
				testSteps.add(
						"=================== ADDING PRODUCT IN PACKAGE RATE PLAN ======================");
				app_logs.info(
						"=================== ADDING PRODUCT IN PACKAGE RATE PLAN ======================");
	
				for(String prod : packageRatePlanProductName.split(",")){
					getTestSteps.clear();
					getTestSteps = ratePackage.addProductInPackageRatePlan(driver,packageRatePlanProductAmount,  prod, packageratePlanProductFirstCalculationMethod, packageratePlanProductsecondCalculationMethod);
					testSteps.addAll(getTestSteps);
				}
...
//			testSteps.add("Navigated to RatesGrid");
		...
		try {
			...
			getTestSteps.clear();
			getTestSteps = derivedRate.selectCustomStartAndEndDates(driver,seasonStartDate,seasonStartDate,seasonDateFormat);
			testSteps.addAll(getTestSteps);
		...
		try {
			...
			app_logs.info(
					"=================== " + "DELETE PARENT "+parentRatePlanType.toUpperCase()+" ======================");
			getTestSteps.clear();
		...
		try {
			RetryFailedTestCases.count = Utility.reset_count;
			Utility.AddTest_IntoReport(testName+"_DerivedFrom"+parentRatePlanType.replaceAll(" ", ""), testDescription, testCategory, testSteps);
	...
	public Object[][] getData() {
		...
		return Utility.getData("CreateDerivedRateCustomDateV2", envLoginExcel);
	}
