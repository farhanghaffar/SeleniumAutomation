...
public class VerifyTaxesOnPackageInCPRes extends TestCore {
	...
	public void verifyTaxesOnPackageInCPRes(String TestCaseID,
			...
			String value, String category, String IsPercent, String bedsCount, String MaxAdults, String maxPersopns, String roomQuantity) throws Exception {
		...
		try {
			...
			nav.navInventoryFromListMgmt(driver, test_steps);
			test_steps.add("Navigate to Inventory");
			nav.Rate(driver);
			test_steps.add("Navigate to Rate");
			randomString =Utility.GenerateRandomNumber();
			...
			// After reservation save verify the validation
					...
					// ------------------------------------
			            ...
			            double taxAfterSave = Double.parseDouble(totalTaxChargesbeforesave.replace("$", "").trim());
							...
							test_steps.add("Room charge differ due to issue AUTOMATION-1999 "+ e.toString());
							} catch(Error e) {
								test_steps.add("Room charge differ due to issue AUTOMATION-1999 "+ e.toString());
							}
	...
	public Object[][] getData() {
		return Utility.getData("VerifyTaxesOnPackageInCPRes", envLoginExcel);
	}
