...
public class Create_House_Account extends TestCore{
	...
	public void Create_House_Account(String TestCaseID,String PropertyName,String CheckInDate,String CheckOutDate,String Adults,String Children,String Rateplan,String PromoCode,String IsSplitRes,
	...
	//	RoomClass rc = new RoomClass();
		...
		try {
			...
			comments="Verified House account auto-suggestion in the reservation advance search page. ";
			...
			String paymentMethods="Amex|Cash|Check|Discover|House Account|Imported Payment|MC|Paidout|Prepaid|Visa|Gift Certificate|Reservation";
			String[] allMeth = paymentMethods.split("\\|");
			ArrayList<String> allMethods=new ArrayList<>();
			allMethods.add("Account - "+AccountName+" ("+AccountNumber+")");
			Collections.addAll(allMethods, allMeth);
			app_logs.info("Expected"+allMethods);
			boolean isMethodsAvailable=res.verifyallPaymentMethodInFolio(driver, test_steps, allMethods);
			Assert.assertEquals(isMethodsAvailable, true,"Failed: to verify all the payment methods availability");
			comments="Verified that all the payment methods are displayed in payment method drop down while trying to pay for a house account from folio tab.";
			
			
			
			
			String amount=res.pay_Account(driver, test_steps, AccountName, IsChangeInPayAmount, ChangedAmountValue, TakePaymentType);
			//res.verify_AccountPaymentLineItemHA(driver, test_steps, amount, "Account");
			
			res.click_History(driver, test_steps);
		//	res.verify_AcountPaymentInHistory(driver, test_steps, Double.parseDouble(amount), AccountName,RoomAbbri,Rooms,timeZone);
			
			String getHistoryCategory = res.getHistoryCategory(driver, 0);
			String HistoryCategory = "PAYMENT";
			...
			String getHistoryDate = res.gettHistoryDate(driver, 0);
			String getDate = ESTTimeZone.DateFormat(0);
			...
			String gettHistoryDescription = res.getHistoryDescription(driver, 0);
			String description = "Made a payment $"+amount+" using Account (House Account)";
			...
			String expectedRoom = RoomAbbri + ": " + Rooms;
			String getHistoryRoom = res.gettHistoryRoom(driver, 0);
			//test_steps.add("Expected room: " + expectedRoom);
			//test_steps.add("Found: " + getHistoryRoom);
		////	assertTrue(getHistoryRoom.contains(""+Rooms), "Failed: History room is mismatching!");
			//test_steps.add("Verified room");
			res.click_Folio(driver, test_steps);
			res.inHouseReservation(driver);
			res.click_Folio(driver, test_steps);
			folio.verifyPostStatus(driver, test_steps);
			comments=comments+"Verified advance deposit realization when MRB reservation is checked in. ";
			res.closeReservationTab(driver);
			comments=comments+"Verified creating a House Account ,associate a reservation and verified payments.";
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);
				Utility.updateReport(e, "Failed to payment using house account", testName, "Reservation", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
	...
	public Object[][] getData() {
		...
		return Utility.getData("Create_House_Account", SNExcel);
	}
