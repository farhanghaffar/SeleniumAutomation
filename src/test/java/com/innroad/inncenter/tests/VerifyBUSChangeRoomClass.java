...
public class VerifyBUSChangeRoomClass extends TestCore{
	...
	public void verifyBUSChangeRoomClass(String version, String roomClassName, String channel, String message,
			...
			String numberToken, String testCaseName, String ratePlanName,String beginDate, String noOfNights, String originationDate,
			...
			String updatedRoomClassName,String updateRate, String isAdditionalChargesForChildrenAdults,String extraAdult,
			String extraChild, String extraAdultAmounts, String extraChildAmounts) {
		 
		String testName = testCaseName;
		String startDate = null, endDate = null,  confirmationNo = null, expiryDate = null,
				startDate1 = null, endDate1 = null, advertiserId = null, currency = null,propertyId = null,
						phoneCode = null, phoneNumberVrbo = null, totalAmount=null, roomCharges=null,
		...
		try {
			...
			classId.add(roomClassIds.get("roomClassId"));
			roomClass.closeRoomClassTabV2(driver, roomClassName);
			roomClassIds.clear();
			roomClassIds = roomClass.getRoomClassDetails(driver, updatedRoomClassName, testSteps);
			classId.add(roomClassIds.get("roomClassId"));
		...
		try {
			testSteps.add("==================== UPDATE SEASON  IN RATE PLAN ======================");
			app_logs.info("==================== UPDATE SEASON IN RATE PLAN ======================");			
			...
			app_logs.info("Start Date: "+startDate1);
			app_logs.info("End Date: "+endDate1);
			...
			datesRangeList = Utility.getAllDatesStartAndEndDates(startDate1, endDate1);
			try {
			...
			nightlyRate.selectSeasonDates(driver, testSteps,startDate1, endDate1);
			...
			ArrayList<String> roomClassList= new ArrayList<String>();
			roomClassList.add(roomClassName);
			roomClassList.add(updatedRoomClassName);
			nightlyRate.selectAdditionalChargesForChildrenAdults(driver, testSteps, isAdditionalChargesForChildrenAdults);
			nightlyRate.enterRate(driver, testSteps, updateRate, isAdditionalChargesForChildrenAdults, extraAdult, extraChild,
					extraAdultAmounts, extraChildAmounts);
		...
		try {
			...
			File requestBody = bookingRequest.createBookingRequest(testName, version, advertiserId, classId.get(0), channel,
		...
		try {
			...
			if(vrvoproperties.getProperty("Environment").equalsIgnoreCase("QA")) {
			 responseBody = vrboObject.getResponseBodyOfVrboRateEndPoint(propertyId, classId.get(0), vrvoRateToken);
			}else {
				 responseBody = vrboObject.getResponseBodyOfVrboRateEndPoint(propertyId, classId.get(0), vrvoproperties.getProperty("BookingRequestTokenQA3"));
			}
			...
			vrboObject.validateBookingUpdateXML(response, advertiserId, classId.get(0), classId.get(0), confirmationNo, "en_US",
		...
		try {
			...
			testSteps.add("====================CHANGE ROOM CLASS======================");	
			...
			ArrayList<String> rooms =reservation.getAllRoomNumbersFromRoomClassDropDown(driver, testSteps, updatedRoomClassName);
			reservation.selectRoomToReserve(driver, testSteps, updatedRoomClassName, rooms.get(0));
			...
			e.printStackTrace();
			Utility.catchException(driver,e, test_description, test_catagory, "Change Room Class", testName, test_description,
					test_catagory, testSteps);

		} catch (Error e) {
			e.printStackTrace();
			Utility.catchError(driver, e, test_description, test_catagory, "Change Room Class", testName, test_description,
					test_catagory, testSteps);
		}
		...
		try {
			...
			String EDate=Utility.parseDate(dates.get("ChecOUT"), "MMM dd, yyyy", "YYYY-MM-dd");
			 ...
			 String checkInDate=Utility.parseDate(dates.get("CheckIN"), "MMM dd, yyyy", "dd/MM/yyyy");
			 String checkOutDate=Utility.parseDate(dates.get("ChecOUT"), "MMM dd, yyyy", "dd/MM/yyyy");
			datesRangeList.clear();
			...
			vrboObject.validateBookingUpdateXML(response, advertiserId, classId.get(1), classId.get(1), confirmationNo, "en_US",
					calcInTaxVal, calcInFeeVal, roomchargeAmounts, getExternalIdOfTax, getExternalIdOfFee, setTaxRates,
					setFeeRates, payStatus, title, nameONCard, email, phoneCode, phoneNumberVrbo, getAddress1,
	...
	public Object[][] getDataOne() {
		...
		return Utility.getData("BUSChangeRoomClass", otaexcel);
