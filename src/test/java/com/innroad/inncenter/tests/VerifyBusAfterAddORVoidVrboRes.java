...
public class VerifyBusAfterAddORVoidVrboRes extends TestCore {
	...
	public void verifyBusAfterAddORVoidVrboRes(String version, String roomClassName, String channel, String message,
			...
			String infeeName, String feeType, String infeeValues, String categoryFee, String action) {
		...
		try {
			if (!data.get("BODY").contains("PROPERTY_NOT_AVAILABLE") && !data.get("BODY").contains("SERVER_ERROR")) {
				...
				if (action.equalsIgnoreCase("AddRoomChargeLineItem")) {
					// reservation.clickOnDetails(driver);

					testSteps.add("<b>===== ADD ROOM CHARGE LINE ITEM =====</b>");
					app_logs.info("=====ADD ROOM CHARGE LINE ITEM=====");
					beforeAction = Utility.getCurrentDatTimeInUTC("yyyy-MM-dd'T'HH:mm:ss'Z'");
					reservation.AddLineItem(driver, testSteps, "Room Charge", addedRoomChargeAmount);
					Wait.wait30Second();
					...
					/*
					 * vrboObject.validateBookingUpdateXML(busContent, advertiserId, classId,
					 * classId, confirmationNo, "en_US", calcInTaxVal, calcInFeeVal,
					 * roomchargeAmounts, getExternalIdOfTaxes, getExternalIdOfFees, setTaxRates,
					 * setFeeRates, payStatus, title, nameONCard, email, phoneCode, phoneNumberVrbo,
					 * getAddress1, getAddress2, getAddress3, countryName, getAaddress4, postalCode,
					 * adult, child,
					 * 
					 * startDate, endDate, startDate, resStatus, tripTotal, false, dueDates,
					 * testSteps);
					 */
					vrboObject.validateOrderItemInBus(busContent, calcInTaxVal, calcInFeeVal, roomchargeAmounts, getExternalIdOfTaxes,
							getExternalIdOfFees, setTaxRates, setFeeRates, resStatus, false, testSteps);

				} else if (action.equalsIgnoreCase("VoidRoomChargeLineItem")) {
					testSteps.add("<b>===== VOID ROOM CHARGE LINE ITEM =====</b>");
					app_logs.info("=====VOID ROOM CHARGE LINE ITEM=====");
					beforeAction = Utility.getCurrentDatTimeInUTC("yyyy-MM-dd'T'HH:mm:ss'Z'");
					// reservation.AddLineItem(driver, testSteps, "Room Charge", "50");
					reservation.void_LineItem(driver, testSteps, "Room Charge");
					Wait.wait30Second();
					afterAction = Utility.getCurrentDatTimeInUTC("yyyy-MM-dd'T'HH:mm:ss'Z'");
					reservation.clickOnDetails(driver);
					roomCharge = reservation.getRoomCharge_TripSummary(driver);
					...
					if (days > 1) {
						...
						vrboObject.validateOrderItemInBus(busContent, calcInTaxVal, calcInFeeVal, roomchargeAmounts, getExternalIdOfTaxes,
								getExternalIdOfFees, setTaxRates, setFeeRates, resStatus, true, testSteps);
					}
	...
	public Object[][] getDataOne() {
		...
		return Utility.getData("VerifyBusAfterAddORVoidVrboRes", otaexcel);
