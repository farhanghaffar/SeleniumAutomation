...
public class ReportsV2DailyFlashReportOccupancy extends TestCore  {
...
//			}
		...
		if (Utility.validateString(roomsAction)) {
			...
			if (roomsAction.equalsIgnoreCase("NewRoomClass")) {
				...
				rm.addRoomMaintenance(driver, "1", subjectOOO, roomQuantity,test_steps,roomClass);
		...
		try {
			...
			report.validatePropertyStatisticsBookableRoomNights_RV2(driver, afterPropertyStatisticsDetails, test_steps);
					}catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
	...
	public Object[][] getData() {
		...
		return Utility.getData("DailyFlashReportOccupancy", excelReservationV2);
	}
