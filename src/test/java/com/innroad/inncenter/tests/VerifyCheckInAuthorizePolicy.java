...
public class VerifyCheckInAuthorizePolicy extends TestCore{
	...
	public void verifyCheckInAuthorizePolicy(String roomClassName,String maxAdults, 
			String maxPersons, String roomQuantity, String ratePlanName, String rateAmount, String policyName, 
			String typeOfFees, String percentage, String chargesTypes,
			String checkInDate, String checkOutDate, String adultsForRes, String childrenForRes,
			String salutation, String guestFirstName,String guestLastName,String paymentMethod, String cardNumber,String nameOnCard,String marketSegment,
		...
		String testDescription = "Verify authorization checkin policy while checkin the reservation<br>"
				+ "<a href='https://innroad.testrail.io/index.php?/cases/view/825338' target='_blank'>"
				+ "Click here to open TestRail: 825338</a><br/>";
		String testCatagory = "Policies";

		app_logs.info("##################################################################################");
		app_logs.info("EXECUTING: " + testName + " TEST.");
		app_logs.info("##################################################################################");
		Utility.initializeTestCase("825338", Utility.testId, Utility.statusCode, Utility.comments, "");
		...
		try {
			...
			newRoomClass.createRoomClassV2(driver, test_steps, roomClassNames, roomClassNames, maxAdults, maxPersons,
					roomQuantity);
			getRoomNumbers.add(Utility.RoomNo);
		...
		try {
			...
			policyName = policyName + Utility.generateRandomStringWithoutNumbers();
				policiesNames = policies.createPolicies(driver, test_steps, "", "", "Check-in", "", "",
						policyName, "", typeOfFees, percentage, chargesTypes, "", "", "No", "");
			app_logs.info(policiesNames);
			app_logs.info(policyName);

		} catch (Exception e) {
			Utility.catchException(driver, e, "Create Policy", "Inventory", "Inventory", testName, testDescription,
					testCatagory, test_steps);
		} catch (Error e) {
			Utility.catchError(driver, e, "Create Policy", "Inventory", "Inventory", testName, testDescription,
					testCatagory, test_steps);
		}
		
		try {
			app_logs.info("========== Creating and Associating a  rate plan with room class ==========");
			test_steps.add("========== Creating and Associating a  rate plan with room class ==========");
			...
			nightlyRate.createSeasonForExistingRatePlan(driver, test_steps, ratePlanName, datesRangeList,
					seasonStartDate, seasonEndDate, "", roomClassNames, rateAmount, "", "", "", "", false);
			...
			nightlyRate.selectPolicy(driver, "Check-in", policyName, true, test_steps);
...
*/
		try {
			...
			guestFullName = guestFirstName+" "+guestLastName;
			app_logs.info(guestFullName);		
			confirmationNo = reservationPage.createBasicReservation(driver, checkInDate, checkOutDate, adultsForRes, childrenForRes,
					ratePlanName, salutation, guestFirstName, guestLastName, "No", paymentMethod, cardNumber,
					nameOnCard, marketSegment, referral, roomClassNames, true, false);
			test_steps.add("Reservation Created: <b>" + confirmationNo + "</>");
			app_logs.info("Reservation Created: " + confirmationNo);
			app_logs.info(roomNumber);			
			roomNumber = reservationPage.getStayInfoRoomNo(driver, test_steps);
			app_logs.info(roomNumber);
			List<String> amt= new ArrayList<String>();
			amt=reservationPage.getStayInfoRoomCharges(driver, test_steps);
			for(String str: amt) {
				roomBaseAmount.add(str);
			}
		...
		try {
			...
			reservationPage.clickOnProceedToCheckInPaymentButton(driver, test_steps);
					...
					Utility.expiryDate, guestFullName, "XXXX "+cardNumber.substring(cardNumber.length()-4), paymentMethod, 
					typeToValidate, percentage, policyFor, policyName, policyText, roomBaseAmount, totalNights, taxes, null, null);
			reservationPage.clickOnPayButtonOnPaymentPopup(driver, test_steps);
			String abc = paymentMethod+" - "+Utility.getCardNumberHidden(cardNumber)+" ("+guestFullName+") (Exp. "+Utility.expiryDate+")";
			...
			Utility.getCardNumberHidden(cardNumber)+" "+Utility.expiryDate+")";
			reservationPage.verifyAuthAmountInFolioHistory(driver, test_steps, expDesc, "PAYMENT", policyFor);
			
			test_steps.add("Verify authorization checkin policy while checkin the reservation"
					+ "<br><a href='https://innroad.testrail.io/index.php?/cases/view/825338' target='_blank'>"
					+ "Click here to open TestRail: 825338</a><br>");
			Utility.testCasePass(Utility.statusCode, 0, Utility.comments, "Verify authorization checkin policy while checkin the reservation");
		...
		try {
			...
			policyDelete.add(policyName);
			policies.deleteAllPolicies(driver, test_steps, policyDelete);
