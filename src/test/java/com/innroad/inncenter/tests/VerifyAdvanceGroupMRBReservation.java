	...
	public void verifyAdvanceGroupMRBReservation(String TestCaseID,String checkInDate, String checkOutDate,
		...
		try {
			...
			nightlyRate.addExtraRoomClassInSeason1(driver, test_steps, "Yes", roomClassName,
					"No", BaseAmount, BaseAmount, "0", "0", "0",
					"0");
		...
		try {
			...
			ArrayList<String> roomNumbers = res.getAllRoomNumbersFromRoomClassDropDown(driver, test_steps, roomClassName);

			getTestSteps.clear();
			res.selectRoomNumber(driver, roomClassName, roomNumbers.get(0), false, getTestSteps);
			test_steps.addAll(getTestSteps);
			...
			getTestSteps.clear();
			res.selectRoomNumber(driver, roomClassName, roomNumbers.get(1), false, getTestSteps);
			test_steps.addAll(getTestSteps);
			...
			getTestSteps.clear();
			getTestSteps = res.selectPrimaryRoom(driver, roomClassName, roomNumbers.get(0));
			test_steps.addAll(getTestSteps);
...
//			test_steps.addAll(getTest_Steps);
			...
			getTestSteps.clear();
			getTestSteps = res.selectAdditionalRoom(driver, roomClassName, roomNumbers.get(1));
			test_steps.addAll(getTestSteps);
			...
			test_steps.addAll(getTestSteps);
			res.enter_PaymentDetails(driver, test_steps, "Cash", "", "", expiryDate);
			...
			comments="Verified creating MRB reservation from Grouo Block with Reservation NO:"+resNo;
			test_steps.add("Verified creating MRB reservation from Grouo Block with Reservation NO:"+resNo);
			app_logs.info("Verified creating MRB reservation from Grouo Block with Reservation NO:"+resNo);

			getTestSteps.clear();
			getTestSteps = res.verifyAccountName(driver, AccountName,false);
			test_steps.addAll(getTestSteps);
			
			//res.changeReservationStatus(driver, "InHouse", "SecondaryCheckin", test_steps);
			//secondayGuestCheckoutButtonClick
			//secondayGuestCheckInButtonClick
...
*/
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyAdvanceGroupMRBReser", BEExcel);
	}
