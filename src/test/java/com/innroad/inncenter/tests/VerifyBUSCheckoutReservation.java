...
public class VerifyBUSCheckoutReservation extends TestCore{
	...
	public void verifyBUSCheckoutReservation(String version, String roomClassName, String channel, String message,
			...
			String isInncenterFeeCreate, String feeName, String feeType, String feeCategoryValue, String categoryFee, String Void) {
		...
		try {
			...
			String getbalance=reservation.getTripSummaryBalance(driver,  testSteps);
			app_logs.info(getbalance);
			reservation.clickPolicyCollapseIcon(driver, testSteps, "Check In", "");
			String policyName=reservation.getPolicyfromDetailTab(driver, testSteps);
			...
			if(!policyName.equalsIgnoreCase("No Policy")) {
				...
				reservation.clickCloseButtonOfSuccessModelPopupAfterCheckIn(driver, testSteps);
				Wait.waitforPageLoad(50, driver);
				}
			}else{
				reservation.clickOnConfirmCheckInButton(driver, testSteps);
			...
			boolean isExist= Utility.isElementPresent(driver, By.xpath(OR_Reservation.CP_Reservation_RolleBackButton));
			if(isExist) {
			reservation.clickCheckOutButton(driver, testSteps);
			reservation.generatGuestReportToggle(driver, testSteps, "No");
			reservation.clickCheckoutProceedButtons(driver, testSteps, "Confirm Check-out");
			Wait.waitforPageLoad(30, driver);
			}
			...
			roomCharges=Utility.convertDecimalFormat(roomCharges);
			app_logs.info(roomCharges);
			totalAmount= reservation.getBalance_TripSummary(driver);
			roomCharges=Utility.convertDecimalFormat(totalAmount);
			app_logs.info(totalAmount);
			Wait.wait15Second();
			...
			System.out.println("got tax" + calcInTaxVal);
			System.out.println("calcValFee" + calcInFeeVal);
			HashMap<String, Response> responses1 = vrboObject.getBookingUpdateRequest(vrvoBookingUpdateToken,
					advertiserId, confirmationNo);
			File responseBodyForBus = bookingRequest.getResponseXmlFile(responses1.get("BODY").toString(), schemaFilesPath, testName,"BusUpdate");
            testSteps.add("Response Body-" + "<a href='" + responseBodyForBus + "' target='_blank'>"
                    + "Click here to open : URL</a><br>");
			...
			vrboObject.validateBookingUpdateXML(response, advertiserId, classId, classId, confirmationNo, "en_US",
					calcInTaxVal, calcInFeeVal, roomchargeAmounts, getExternalIdOfTax, getExternalIdOfFee, setTaxRates,
					setFeeRates, payStatus, title, nameONCard, email, phoneCode, phoneNumberVrbo, getAddress1,
					getAddress2, getAddress3, countryName, getAaddress4, postalCode, adult, child, SDate, EDate,

					startDate,  resStatus, tripTotal, isVoidCharges,dueDates,testSteps);

		}catch (Exception e) {
			e.printStackTrace();
			Utility.catchException(driver,e, test_description, test_catagory, "Checkout Reservation and Verify Bus Update", testName, test_description,
					test_catagory, testSteps);

		} catch (Error e) {
			e.printStackTrace();
			Utility.catchError(driver,e, test_description, test_catagory, "Checkout Reservation and Verify Bus Update", testName, test_description,
					test_catagory, testSteps);
		}
		...
		try {
			...
			Utility.catchException(driver, e, "Delete Tax", "Tax", "Tax", testName, test_description, test_catagory, testSteps);
		} catch (Error e) {
			Utility.catchError(driver, e, "Delete Tax", "Tax", "Tax", testName, test_description, test_catagory, testSteps);			
		}
	...
	public Object[][] getDataOne() {
		...
		return Utility.getData("BusCheckoutReservation", otaexcel);
