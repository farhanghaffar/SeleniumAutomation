...
public class VerifyRefundForGroupPickupAndAccountV2 extends TestCore
{
	...
	{
		return Utility.getData("VerifyRefundForGroupPickupnAcco", excelRiddhi);
	}
	...
	public void verifyRefundForGroupPickupAndAccountV2(String TestCaseId, String TestCaseName, 
			...
			String paymentMethod, String refundAmount, String refundAccType) throws Exception
	{
		...
		String taskAssignee = "";
		String taskStatus = "";
		String accountName = "Riddhi_Group"+Utility.generateRandomNumber();
		String accountFirstName = "grpPickup_"+Utility.GenerateRandomNumber();
		String accountLastName = "reservation";
		...
		try
		{
			...
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(test_name, test_description, test_catagory, test_steps);
				Utility.updateReport(e, "Failed to navigate and create group account", test_name, "ReservationV2_Groups", driver);
			} else {
				Assert.assertTrue(false);
			}
		}
		...
		try
		{
			...
			test_steps.add("==================Creating Room Block==================");
			app_logs.info("==================Creating Room Block==================");
			group.navigateRoomBlock(driver, test);
			app_logs.info("==================Navigate to Room Block Tab==================");
			test_steps.add("==================Navigate to Room Block Tab==================");	
			String BlockName = "block_R"+Utility.fourDigitgenerateRandomString();
			
			/*
			group.createNewBlockQuote(driver, BlockName, CheckInDate, CheckOutDate, RatePlanName, "1", roomClassName, test_steps);
			test_steps.addAll(group.roomingListClick(driver));
			advgrp.enter_RoomPickupdetails(driver, test_steps,"5454545454545454","12/2024");
			pickupResNumber = group.pickUp_getResNo(driver);
			app_logs.info("Group pick Reservation number: "+pickupResNumber);			
			group.pickUpCloseClick(driver);*/
			
			
			group.createNewBlock(driver, BlockName, CheckInDate, CheckOutDate, 
					RatePlanName, "2", roomClassName);
			group.navigateRoomBlock(driver);
			pickupResNumber = advgrp.PickupFromRoomingList(driver, accountFirstName, accountLastName, roomClassName, refundAmount, "Cash");
			//test_steps.addAll(getTest_Steps);
			app_logs.info("Pickup Reservation Number - "+pickupResNumber);
			test_steps.add("Pickup Reservatioon Number - "+pickupResNumber);
		}
		catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(test_name, test_description, test_catagory, test_steps);
				Utility.updateReport(e, "Failed to Create group Block", test_name, "ReservationV2_Groups", driver);
			} else {
				Assert.assertTrue(false);
			}
		}
		...
		try
		{
			//Navigate to Reservation Dashboard and Search For pickup reservation and perform refund by Group Account
			test_steps.add("========== Navigate to Reservationn DashBoard==========");
			app_logs.info("========== Navigate to Reservationn DashBoard==========");
			
			navigation.ReservationV2_Backward(driver);
			resV2.basicSearch_WithReservationNo(driver, pickupResNumber, true);
			
			//reservationPage.Search_ResNumber_And_Click(driver, pickupResNumber);
			resV2.click_Folio(driver, test_steps);
			folioNewobj.clickRefundButtonFromFolioTab(driver, test_steps);			
			//(//span[contains(text(),'Account - Riddhi_Group8683 (954839226942634)')])[last()]
			String payByGroupAcc="Account - "+accountName+" ("+groupAccountNo+")";
			folioNewobj.refundPaymentinGuestFolio(driver, availableMarketSegments, refundAmount, 
					paymentMethod, accountName+" ("+groupAccountNo+")", "", "", "");
			String amount = Utility.convertDecimalFormat(refundAmount);
			amount = "-$" + amount + "";
			folioNewobj.verifyFolioLineItem4AccountPaymentRefund(driver, test_steps, Utility.getCurrentDate("dd/MM/yyyy"), 
					paymentMethod, refundAccType, payByGroupAcc, amount);
			test_steps.add("========================Navigate to History Log & Verify History Log For Refund========================");
			app_logs.info("========================Navigate to History Log & Verify History Log For Refund========================");
			resV2.switchHistoryTab(driver, test_steps);
			resV2.verifyHistoryWithRefundedPayment(driver, test_steps, refundAmount, "", payByGroupAcc, "", "");
		}
		//td[@class='lineitem-date']/span[(text()='Feb 11, 2022')]/../following-sibling::td[@class='lineitem-category']/span[(text()='Account (Group)')]//../following-sibling::td[@class='lineitem-description pr']//a[normalize-space(text()='Account - Riddhi_Group8672 (954839226942635)')]
		//../following-sibling::td[@class='lineitem-description pr']//a[normalize-space(text()='Account - Riddhi_Group8672 (954839226942635)')]
		catch (Exception e) {
			e.printStackTrace();
			if (Utility.reTry.get(test_name) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
