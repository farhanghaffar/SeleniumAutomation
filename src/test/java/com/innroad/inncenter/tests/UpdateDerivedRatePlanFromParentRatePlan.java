...
public class UpdateDerivedRatePlanFromParentRatePlan extends TestCore {
	...
	String createdDerivedRatePlanName = null;
	...
	public void verifyDerivedRatePlanExist(String clientName, String delim, String partOftestName,
		...
		test_name = "verifyDerivedRatePlanExist";
		...
		createdDerivedRatePlanName = derivedRatePlanName;
		...
		for (String s : parentRoomClass.split(Utility.DELIM)) {
		...
		try {
			...
			if (ratesGrid.searchAndVerifyRatePlanExist(driver, derivedRatePlanName, true, testSteps)) {
				...
				testSteps.add("Successfully verified Derived Rate Plan(" + derivedRatePlanName + ") not exist");
				app_logs.info("Successfully verified Derived Rate Plan(" + derivedRatePlanName + ") not exist");
			}
		...
		if (!ratePlanExist) {
			...
			try {
				...
				if (Utility.compareDates(seasonStartDate.split(Utility.SEASONDELIM)[0], todayDate,
						seasonDateFormat) >= 0) {
				...
				if (dateRange.equals(customDateRange)) {
					if (Utility.compareDates(customStartDate.split(Utility.DELIM)[0], todayDate,
							seasonDateFormat) >= 0) {
			...
			try {
				...
				if (parentRatePlanType.equalsIgnoreCase("Package rate plan")) {
					...
					getTestSteps.clear();
					getTestSteps = ratePackage.addProducts(driver, packageRatePlanProductName,
							packageRatePlanProductAmount, packageratePlanProductFirstCalculationMethod,
			...
			try {
				...
				nightlyRate.createUpdateSingleOrMultipleSeason(driver, testSteps, false, seasonStartDate, seasonEndDate,
						...
						isDefaultProrateChecked, isProRateStayInSeason, isProRateInRoomClass,
						roomClassesWithProRateUnchecked, isCustomPerNight, customRoomClasses, customRatePerNight,
						customRateAdultdPerNight, customRateChildPerNight, isCustomRatePerNightAdultandChild, false,
						intervalRatePlanIntervalValue, isIntervalRateplan);
			...
			try {
				...
				if (Utility.reTry.get(testName) == Utility.count) {
					RetryFailedTestCases.count = Utility.reset_count;
					Utility.AddTest_IntoReport(testName, testDescription, testCategory, testSteps);
					Utility.updateReport(e, "Failed to enter rate plan name", testName, "DerivedRatePlanCreation",
							driver);
				} else {
					Assert.assertTrue(false);
				}
			...
			try {
				...
				nightlyRate.selectRestrictions(driver, Boolean.parseBoolean(derivedRateIsRatePlanRistrictionReq),
						...
						derivedRatePromoCode, testSteps);
			...
			try {
				testSteps.add("=================== "
						+ "Verify the newly created Derived Rate Plan in the Rates Grid's Rate Plan drop down"
								.toUpperCase()
						+ " ======================");
				app_logs.info("=================== "
						+ "Verify the newly created Derived Rate Plan in the Rates Grid's Rate Plan drop down"
	...
	public void updateDerivedRatePlanFromParentRatePlan(String delim, String isDefaultRatePlan, String isActive,
		...
		test_name = "UpdateDerivedRatePlanFromParentRatePlan";
		testDescription = "Rates V2 - Create Derived Rate Plan From <br>"
				+ "<a href='https://innroad.atlassian.net/browse/AUTOMATION-1801' target='_blank'>"
				+ "Click here to open Jira: AUTOMATION-1801</a>";
		testCategory = "RateV2";
		...
		try {
			...
			derivedRate.derivedratePlanExist(driver, createdDerivedRatePlanName, true, testSteps);
			derivedRate.clickEditIconOfDerivedRatePlan(driver, createdDerivedRatePlanName, testSteps);

			testSteps.add("=================== VERIFY EDIT RATE PLAN INITIAL STATE AND UPDATE ======================");
			app_logs.info("=================== VERIFY EDIT RATE PLAN INITIAL STATE AND UPDATE ======================");

			nightlyRate.verifyRatePlanNameEditPage(driver, createdDerivedRatePlanName, testSteps);
	...
	public Object[][] getData() {
		...
		return Utility.getData("CreateOneDerivedRateFromParent", envLoginExcel);
	}

	@DataProvider
	public Object[][] getUpdateData() {
		// return test data from the sheetname provided
		return Utility.getData("UpdateDerivedRateFromParent", envLoginExcel);
	}
