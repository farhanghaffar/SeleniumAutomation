...
public class VerifyPackageRatePlanBaseOnIntervalInncenterV2 extends TestCore {
	...
	int productVal = 0;
	HashMap<String, String> getProductValue = new HashMap<String, String>();
	String packageAmount = "";
	String RuleMinStayValue = "";
	...
	public void verifyPackageRatePlanBaseOnIntervalInncenterV2(String delim, String ReservationType, String AccountType,
			String productAmnt, String productFirstCalcMethod, String productSecondCalcMethod, String CheckInDate,
			String CheckOutDate, String ReservationRoomClasses, String RulesUpdateType, String RulesUpdateStartDate,
			String RulesUpdateEndDate, String RulesUpdateDays, String Type_RulesUpdate,
			...
			String FeesGuestMustPay, String PercentOfFee, String FeeChargesType, String Verification) throws Exception {
		...
		RuleMinStayValue = RuleMinStayValue_RulesUpdate;
		HashMap<String, String> ratePlanData = Utility.getExcelData(
				...
				"CreatePackageUsingInterval");

		HashMap<String, String> productCreationData = Utility.getExcelData(
				System.getProperty("user.dir") + "\\test-data\\CentralparkSanityTestData.xlsx", "CreateNewProduct");

		String productName = productCreationData.get("ProductName");
		String productCategory = productCreationData.get("ProductCategory");
		String productAmount = productCreationData.get("ProductAmount");
		String productFirstCalculationMethod = productCreationData.get("ProductFirstCalculationMethod");
		String productSecondCalculationMethod = productCreationData.get("productSecondCalculationMethod");
		boolean isSellOnBookingEngine = Boolean.parseBoolean(productCreationData.get("IsSellOnBookingEngine"));
		String productDescription = productCreationData.get("Description");
...
//		String[] startDate = SeasonStartDate.split("\\"+Utility.DELIM);
//		String[] endDate = SeasonEndDate.split("\\"+Utility.DELIM);
//		
//		String temStartDate = "";
//		String temEndDate = "";
		String timeZone = "America/New_York";
//		for (int i = 0; i < startDate.length; i++) {
//			int days = ESTTimeZone.numberOfDaysBetweenDates(startDate[i], endDate[i],"dd/MM/yyyy");
//			app_logs.info("days: "+days);
//			String getCurrentDate = Utility.getNextDate(0, "dd/MM/yyyy", timeZone);
//			if (ESTTimeZone.CompareDates(startDate[i], "dd/MM/yyyy", timeZone)) {
//				temStartDate = getCurrentDate;
//				getCurrentDate = Utility.getNextDate(days, "dd/MM/yyyy", timeZone);
//				temEndDate = getCurrentDate;
//
//			}
//			if(i!=0) {
//				SeasonStartDate = SeasonStartDate+Utility.DELIM+temStartDate;
//				SeasonEndDate = SeasonEndDate+Utility.DELIM+temEndDate;
//			}
//			else {
//				SeasonStartDate = temStartDate;
//				SeasonEndDate = temEndDate;			
//			}
//
//		}

		app_logs.info("SeasonStartDate: " + SeasonStartDate);
		app_logs.info("SeasonEndDate: " + SeasonEndDate);
		...
		// end here
		int days = ESTTimeZone.numberOfDaysBetweenDates(RulesUpdateStartDate, RulesUpdateEndDate, "dd/MM/yyyy");
		app_logs.info("days: " + days);
		String getCurrentDate = Utility.getNextDate(0, "dd/MM/yyyy", timeZone);
		if (ESTTimeZone.CompareDates(RulesUpdateStartDate, "dd/MM/yyyy", timeZone)) {
			RulesUpdateStartDate = getCurrentDate;
			getCurrentDate = Utility.getNextDate(days, "dd/MM/yyyy", timeZone);
			RulesUpdateEndDate = getCurrentDate;
		...
		if (ReservationType.equalsIgnoreCase("MRB")) {
			...
			String[] checkInDates = CheckInDate.split("\\" + Utility.DELIM);
			String[] checkOutDates = CheckOutDate.split("\\" + Utility.DELIM);
			String newCheckIn = "";
			String newCheckOut = "";
			for (int i = 0; i < checkInDates.length; i++) {
				days = ESTTimeZone.numberOfDaysBetweenDates(checkInDates[i], checkOutDates[i], "dd/MM/yyyy");
				app_logs.info("days: " + days);
				getCurrentDate = Utility.getNextDate(0, "dd/MM/yyyy", timeZone);
				if (ESTTimeZone.CompareDates(checkInDates[i], "dd/MM/yyyy", timeZone)) {
					checkInDates[i] = getCurrentDate;
					checkOutDates[i] = Utility.getNextDate(days, "dd/MM/yyyy", timeZone);

				}
				if (i != 0) {
					newCheckIn = newCheckIn + "|" + checkInDates[i];
					newCheckOut = newCheckOut + "|" + checkOutDates[i];
				} else {
					newCheckIn = checkInDates[i];
					newCheckOut = checkOutDates[i];
				}
			...
			days = ESTTimeZone.numberOfDaysBetweenDates(CheckInDate, CheckOutDate, "dd/MM/yyyy");
			app_logs.info("days: " + days);
			getCurrentDate = Utility.getNextDate(0, "dd/MM/yyyy", timeZone);
		...
		try {
			...
			try {
				...
				// login.login(driver, "https://app.qa3innroad.com/", "autoota", "autouser",
				// "Letmein18@");
		...
		try {
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);
				Utility.updateReport(e, "Failed to click on inventory", testName, "RatesV2", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		try {
			...
			test_steps.add("=================== VERIFY PRODUCT EXIST OR NOT ======================");
			app_logs.info("=================== VERIFY PRODUCT EXIST OR NOT ======================");
			navigation.clickOnProductAndBundle(driver);
			test_steps.add("Navigated to product and bundle");
			app_logs.info("Navigated to product and bundle");

			ratePackage.SearchProduct(driver, productName);
			app_logs.info("Search product");
			boolean isProductExist = ratePackage.verifySearchedProductExist(driver, productName);
			app_logs.info("isProductExist: " + isProductExist);
			if (!isProductExist) {
				test_steps.add("Create a new product");
				ratePackage.createProduct(driver, productName, isSellOnBookingEngine, "Always", RoomClasses,
						productDescription, productAmount, "", productCategory, productFirstCalculationMethod,
						productSecondCalculationMethod, "", "");

			} else {
				test_steps.add("Product with name " + productName + " already exist");
			}
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);
				Utility.updateReport(e, "Failed search product and create product", testName, "RatesV2", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		if (!israteplanExist) {
			try {
				...
				ratePackage.selectParentRatePlan(driver, "Interval rates");
				test_steps.add("Select rate type as: Interval rates");
				...
				test_steps.add("=================== SELECT PRODUCT ======================");
				app_logs.info("=================== SELECT PRODUCT ======================");

				// ratesGrid.addProduct(driver, productName);
				test_steps.add("Add product: " + productName);
				getTest_Steps.clear();
				...
				if (Utility.reTry.get(testName) == Utility.count) {
					RetryFailedTestCases.count = Utility.reset_count;
					Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);
					Utility.updateReport(e, "Failed to enter rate plan level values", testName, "RatesV2", driver);
				} else {
					Assert.assertTrue(false);
				}
			} catch (Error e) {
			...
			try {
				...
				nightlyRate.createSeasonForIntervalRatePlan(driver, test_steps, SeasonStartDate, SeasonEndDate,
						...
						isSeasonPolicies, IsProRateStayInRate, isProRateStayInSeason, isProRateInRoomClass,
						ProRateRoomClassName, IsCustomPerNight, CustomeRoomClass, CustomRatePerNight,
						isAssignPolicyByRoomClass, CustomRateAdultPerNight, CustomRateChildPerNight,
						isCustomRatePerNightAdultandChild, RoomClassInPolicy);
		...
		try {
			...
			if (RulesUpdateType.equalsIgnoreCase("BulkUpdate")) {
				days = ESTTimeZone.numberOfDaysBetweenDates(RulesUpdateStartDate, RulesUpdateEndDate, "dd/MM/yyyy");
				...
				int numberOfDays = ESTTimeZone.numberOfDaysBetweenDates(RulesUpdateStartDate, RulesUpdateEndDate,
						"dd/MM/yyyy") + 1;
				...
				// here verify update in rates grid
				if (!ReservationType.equalsIgnoreCase("MRB")) {
					rateGrid.clickForRateGridCalender(driver, test_steps);
				...
				String[] reservationRoomClasses = ReservationRoomClasses.split("\\" + Utility.DELIM);
				for (int i = 0; i < reservationRoomClasses.length; i++) {
					test_steps.add("==========Getting the rules for room class " + reservationRoomClasses[i]
							+ " in Rates Grid  for " + channelName + " ==========");
					RatesGrid ratesGrid = new RatesGrid();
					ratesGrid.expandRoomClass1(driver, test_steps, reservationRoomClasses[i].trim());
					ratesGrid.expandChannel(driver, test_steps, reservationRoomClasses[i], channelName);

					minStayRule = ratesGrid.getRuleDataValuesForMinStay(driver, test_steps, reservationRoomClasses[i],
							channelName, numberOfDays);
					app_logs.info("minStayRule: " + minStayRule);

					noCheckInRule = ratesGrid.getRuleDataValuesForNoCheckIn(driver, test_steps,
							reservationRoomClasses[i], channelName, (numberOfDays - 1));
					app_logs.info("noCheckInRule: " + noCheckInRule);

					noCheckOutRule = ratesGrid.getRuleDataValuesForNoCheckOut(driver, test_steps,
							reservationRoomClasses[i], channelName, (numberOfDays - 1));
					app_logs.info("noCheckOutRule: " + noCheckOutRule);

					ratesGrid.collapseRoomClass(driver, test_steps, reservationRoomClasses[i]);
					app_logs.info("collapse roomClass");
					String isMinimumStayOn = "No";
					String isNoCheckInChecked = "No";
					String isNoCheckOutChecked = "No";
					if (Type_RulesUpdate.contains("Min stay")) {
						isMinimumStayOn = "Yes";
					}
					if (Type_RulesUpdate.contains("No Check In")) {
						isNoCheckInChecked = "Yes";
					}
					if (Type_RulesUpdate.contains("No Check out")) {
						isNoCheckInChecked = "Yes";
					}

					HashMap<String, String> getRuleBaseOnDays = new HashMap<String, String>();
					for (int k = 0; k < numberOfDays; k++) {
						String getDate = ESTTimeZone.getNextDateBaseOnPreviouseDate(RulesUpdateStartDate, "MM/dd/yyyy",
								"MM/dd/yyyy", k, timeZone);
						test_steps.add(
								"===== Verifying min stay, no check in and no checkout rule after bulk rule update for date: "
										+ getDate + " =====");
						app_logs.info("getRuleBaseOnDays: " + getRuleBaseOnDays);
						if (isMinimumStayOn.equalsIgnoreCase("yes")) {
							assertEquals(minStayRule.get(k).trim(), RuleMinStayValue_RulesUpdate,
									"Failed: min stay value is mismathing after change in min stay value!");
						}
						if (isNoCheckInChecked.equalsIgnoreCase("yes")) {
							assertEquals(noCheckInRule.get(k).trim(), "Yes",
									"Failed: no checkin is mismathing after bulk update!");
						}

						if (isNoCheckOutChecked.equalsIgnoreCase("yes")) {
							assertEquals(noCheckOutRule.get(k).trim(), "Yes",
									"Failed: no checkout is mismathing after bulk update!");
						}
					}
				}
				...
				String toasterMesssage = "";
				RatesGrid ratesGrid = new RatesGrid();
				for (String roomClassName : roomClassesList) {

					ratesGrid.expandRoomClass1(driver, test_steps, roomClassName.trim());
					ratesGrid.expandChannel(driver, test_steps, roomClassName.trim(), channelName);

					for (int i = 0; i < getDates.size(); i++) {
						//String newDateDate = Utility.getCustomDate(RulesUpdateStartDate, "dd/MM/yyyy", "dd/MM/yyyy", i);
						rateGrid.clickForRateGridCalender(driver, test_steps);
						...
						for (String type : type_rulesUpdateList) {
							if (type.equalsIgnoreCase("Min stay")) {
								...
								ratesGrid.updateNoCheckInRule(driver, roomClassName, channelName, true, i);
							} else if (type.equalsIgnoreCase("No Check out")) {
								ratesGrid.updateNoCheckOutRule(driver, roomClassName, channelName, true, i);
							}
						}
					}
				}

				// here to verify override rules value
				rateGrid.clickForRateGridCalender(driver, test_steps);
				Utility.selectDateFromDatePicker(driver,CheckInDate , "dd/MM/yyyy");
		...
		try {
			...
			if (ReservationType.equalsIgnoreCase("MRB")) {
				...
				rateGrid.clickOnProductTab(driver);
				app_logs.info("Click on products tab");
				getTest_Steps.clear();
				productName = productName.trim();
				getTest_Steps = rateGrid.updateProductRateWithDifferentCombination(driver, productAmnt,
						productFirstCalcMethod, productSecondCalcMethod, false);
				test_steps.addAll(getTest_Steps);

				getProductValue = rateGrid.getProductValue(driver, productName);
				...
				for (int i = 0; i < chkIn.length; i++) {
					...
					getRoomClassWithRates.put(splitRoomClass[i],
							rateGrid.getRoomClassRateWithAdditionalAdultAndChildBasedOnPackage(driver,
									splitRoomClass[i], test_steps));
				...
				if (isSeasonExist) {
					if (!ReservationType.equalsIgnoreCase("MRB")) {
						...
						getRoomClassWithRates.put(ReservationRoomClasses,
								rateGrid.getRoomClassRateWithAdditionalAdultAndChildBasedOnPackage(driver,
										ReservationRoomClasses, test_steps));
						nightlyRate.closeSeason(driver, test_steps);
...
//		getRates.add("33.33");
//		
//		RatePlanName = "Package_Farhan";
//		folioName = "Package_Farhan";
//		interval = "3";
//		getProductValue.put("amount", "100");
//		getProductValue.put("firstCalcuationMethod", "room");
//		getProductValue.put("secondCalcuationMethod", "night");

		// getRoomClassWithRates.put(roomClass[0], getRates);
		// getRatesWithAdultsAndChild =
		// getRoomClassWithRates.get(ReservationRoomClasses);
				...
				if (driver.findElements(By.xpath("(//span[text()='" + ReservationRoomClasses + "'])[2]")).size() > 0
					...
					if (restricrionsLengthOfStay && restricrionsBookingWindow && isSeasonExist) {
							...
							if (!isMinStayRuleBrokenPopComeOrNot) {
								...
								try {
									assertTrue(minStayColor.equalsIgnoreCase("Red"),
											"Red color label for minstay rule is not found");
									app_logs.info("Successfully verified the red color label for min stay rule");
								...
								try {
									assertTrue(minStayColor.equalsIgnoreCase("green"),
											"green color label for minstay rule is not found");
									app_logs.info("Successfully verified the green color label for min stay rule");
									test_steps.add("Successfully verified the green color label for min stay rule");
								} catch (Error e) {
									test_steps.add(e.toString());
								} catch (Exception e) {
									test_steps.add(e.toString());
						...
						if (checkInColor.equalsIgnoreCase("Red")) {
							...
							try {
								...
								assertTrue(noCheckinColor.equalsIgnoreCase("green"),
										"green color label for no check in rule is not found");
								app_logs.info("Successfully verified the green color label for no check in rule");
								test_steps.add("Successfully verified the green color label for no check in rule");
							} catch (Error e) {
								test_steps.add(e.toString());
							} catch (Exception e) {
								test_steps.add(e.toString());
						...
						try {
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {

								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerRoomPerNight(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
												adultRate.get(0), childRate1.get(0), isProStayRate.get(0),
												customRatePerNight.get(0), adultRatePerNight.get(0),
												childRatePerNight.get(0), folioName, getProductValue, test_steps);

							}

							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {

								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerRoomPerStay(driver,
												ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate,
												adult, children, ReservationRoomClasses, interval, baseRate.get(0),
												adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
												adultRate.get(0), childRate1.get(0), isProStayRate.get(0),
												customRatePerNight.get(0), adultRatePerNight.get(0),
												childRatePerNight.get(0), folioName, getProductValue, test_steps);
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {

								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerAdultPerNight(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerChildPerStay(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
								...
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerChildPerNight(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerPersonPerNight(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerPersonPerStay(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPerRoomPerInterval(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForChildPerInterval(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForAdultPerInterval(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
									&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
								rateListInReservation = reservationPage
										.intervalRateVerificationInChangeStayDetailsWithPackageForPersonPerInterval(
												driver, ratesConfig.getProperty("defaultDateFormat"), CheckInDate,
												CheckOutDate, adult, children, ReservationRoomClasses, interval,
												baseRate.get(0), adultCapacity.get(0), personCapacity.get(0),
												isAdditonalAdultChild.get(0), maxAdult1.get(0), maxPerson.get(0),
							...
							if (rateListInReservation.get(rateListInReservation.size() - 2).get(0)
									.equalsIgnoreCase("no")) {
...
//							test_steps.add("bug id: " + bugId);
						...
						app_logs.info(rateListInReservation);
						...
						ArrayList<String> productInFolio = rateListInReservation.get(3);
						for (int i = 0; i < intervalDatesSingle.size(); i++) {

							if (i == 1) {
								String folioDates = "";

								String date = folio.itemDate(driver, "Package", 1);
								if (date.contains(",")) {
									folioDates = Utility.parseDate(intervalDatesSingle.get(0),
											ratesConfig.getProperty("defaultDateFormat"), "EE MMM dd, yyyy");
								} else {
									folioDates = Utility.parseDate(intervalDatesSingle.get(0),
											ratesConfig.getProperty("defaultDateFormat"), "EE dd MMM yyyy");
								}

								app_logs.info("expecetd: " + folioDates);
								app_logs.info("found: " + date);

								String description = folio.itemDetailsDescroption(driver, "Package", 1);

								String getamount = folio.itemAmount(driver, "Package", 1);
								getamount = folio.splitStringBySign(getamount, "$");
								String strAmount = productInFolio.get(0);
								app_logs.info("expected amount: " + strAmount);
								app_logs.info("found: " + getamount);
								String expProdDes = "";
								if (getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {
									expProdDes = folioName + "- Per " + getProductValue.get("secondCalcuationMethod")
											+ " Component(s)";
								} else {
									expProdDes = folioName + " - Per " + getProductValue.get("secondCalcuationMethod")
											+ " Component(s)";
								}
								test_steps.add(folioDates + "         " + expProdDes + "           " + strAmount);
								test_steps.add(date + "          " + description + "            " + getamount);
								app_logs.info("expected description name: " + expProdDes);
								...
								if (!description.equalsIgnoreCase(expProdDes)) {
								...
								test_steps.add("Verified product amount in folio");
							} else {
								String folioDates = "";
								...
								if (date.contains(",")) {
									folioDates = Utility.parseDate(intervalDatesSingle.get(i),
											ratesConfig.getProperty("defaultDateFormat"), "EE MMM dd, yyyy");
								} else {
									folioDates = Utility.parseDate(intervalDatesSingle.get(i),
											ratesConfig.getProperty("defaultDateFormat"), "EE dd MMM yyyy");
			...
			try {
				...
				if (mrbFlag) {
					...
					HashMap<String, ArrayList<ArrayList<String>>> getRate = new HashMap<>();
					...
					app_logs.info(
							"isMinStayRuleBrokenPopComeOrNotMRB: " + isMinStayRuleBrokenPopComeOrNotMRB.toString());
					...
					String[] adults = adult.split("\\" + Utility.DELIM);
					String[] child = children.split("\\" + Utility.DELIM);
					String[] checkin = CheckInDate.split("\\" + Utility.DELIM);
					String[] checkout = CheckOutDate.split("\\" + Utility.DELIM);

					for (int j = 0; j < roomClass.length; j++) {

						String getRoomClass = roomClass[j].trim();
						String getAdult = adults[j].trim();
						String getChild = child[j].trim();
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {

							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerRoomPerNight(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
											isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
											childRatePerNight.get(0), folioName, getProductValue, test_steps);
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
							...
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerAdultPerNight(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerAdultPerStay(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerChildPerStay(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
							...
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerChildPerNight(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerPersonPerNight(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerPersonPerStay(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPerRoomPerInterval(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForChildPerInterval(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForAdultPerInterval(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
								&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
							rateListInReservation = reservationPage
									.intervalRateVerificationInChangeStayDetailsWithPackageForPersonPerInterval(driver,
											ratesConfig.getProperty("defaultDateFormat"), checkin[j], checkout[j],
											getAdult, getChild, getRoomClass, interval, baseRate.get(0),
											adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
											maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
						...
						getRate.put(roomClass[j], rateListInReservation);
						rateListInReservation = getRate.get(roomClass[j]);
						for (int i = 0; i < rateListInReservation.size(); i++) {
							app_logs.info(rateListInReservation.get(i));
						}

						if (rateListInReservation.get(rateListInReservation.size() - 2).get(0).equalsIgnoreCase("no")) {
							app_logs.info("in else part of interval");
							String bugId = "<a href='https://innroad.atlassian.net/browse/RATES-2170' target='_blank'>"
									+ "Failed: Interval rate when pro rated amount is displaying wrong from new reservation and on selecting the room class rate is updating correctly on trip summary.<br>Click here for more info: RATES-2170</a>";
							test_steps.add("bug id: " + bugId);
						...
						reservationPage.verifyMRBForIntervalRatePlan(driver, test_steps, roomClass[j], "Yes", Account,
								adults[j], isMinStayRuleMRB.get(j), isMinStayRuleBrokenPopComeOrNotMRB.get(j),
								minStayRuleValueMRB.get(j), checkInColorMRB.get(j), checkOutColorMRB.get(j), child[j],
								checkin[j], checkout[j], j + 1, childRate1.size());
					}

//					reservationPage.verifyMRBForIntervalRatePlan(driver,
// 							test_steps,
...
// 							"dd/MM/yyyy",folioName);
					...
					reservationPage.click_Folio(driver, test_steps);
					String category = "Room Charge";
					app_logs.info("rates.get(RoomClass[i]: " + getRate.get(RoomClass[0]));
					app_logs.info("rates.get(RoomClass[i]: " + getRate.get(RoomClass[1]));

					for (int i = 0; i < RoomClass.length; i++) {
						ArrayList<String> folioRate = new ArrayList<String>();
						ArrayList<String> intervalDates = new ArrayList<String>();
						app_logs.info("Room Class : " + RoomClass[i] + " at i " + ": " + i);
						app_logs.info(getRate.get(RoomClass[i]));
						rateListInReservation = getRate.get(RoomClass[i]);
						...
						for (int j = 0; j < intervalDates.size(); j++) {
							if (i == 1) {
								String folioDates = Utility.parseDate(intervalDates.get(0),
										ratesConfig.getProperty("defaultDateFormat"), "EE dd MMM yyyy");
				...
				if (restricrionsLengthOfStay && restricrionsBookingWindow && isSeasonExist) {
					if (driver.findElements(By.xpath("(//span[text()='" + ReservationRoomClasses + "'])[2]")).size() > 0
							...
							if (!isMinStayRuleBrokenPopComeOrNot) {
								...
								try {
									assertTrue(minStayColor.equalsIgnoreCase("Red"),
											"Red color label for minstay rule is not found");
									app_logs.info("Successfully verified the red color label for min stay rule");
									test_steps.add("Successfully verified the red color label for min stay rule");
								} catch (Error e) {
									test_steps.add(e.toString());
								} catch (Exception e) {
									test_steps.add(e.toString());
						...
						try {
							 ...
							 * customRatePerNight.get(0), adultRatePerNight.get(0),
							 * childRatePerNight.get(0), folioName, test_steps);
							 */
							...
							if (getPayAbleAmount.equals(getRoomChargesFromTripSummary)) {
								...
								test_steps.add(
										"Verified room charges in select room room class sectiona and in trip summary are matching");
						...
						for (int i = 0; i < intervalDatesSingle.size(); i++) {
							...
							if (i == 1) {
								String folioDates = Utility.parseDate(intervalDatesSingle.get(0),
										ratesConfig.getProperty("defaultDateFormat"), "EE dd MMM yyyy");
...
//			
			...
			try {
				...
				if (ActionOnReservation.equalsIgnoreCase("No Show")) {
				...
				else if (ActionOnReservation.equalsIgnoreCase("Change Stay Details: recalculate")) {
					Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);
					...
					changeStaydetails(RatePlanName, CheckInDate, CheckOutDate, ReservationChangeClass, channelName,
							...
							getProductValue.get("amount"));

				} else if (ActionOnReservation.equalsIgnoreCase("Change Stay Details: no rate change")) {
					Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);

					ChangeStayDetailsForNoRateChange(RatePlanName, CheckInDate, CheckOutDate, ReservationChangeClass,
							channelName, ReservationRoomClasses, ReservationType, adult, children, PromoCode, interval,
							getProductValue.get("amount"));

				} else if (ActionOnReservation.equalsIgnoreCase("Change Stay Details: Add and Remove Dates")) {
					Utility.AddTest_IntoReport(testName, test_description, test_catagory, test_steps);
					ChangeStayDetailsForAddAndRemoveDates(RatePlanName, CheckInDate, CheckOutDate,
							ReservationChangeClass, channelName, ReservationRoomClasses, ReservationType, adult,
							children, PromoCode, interval);
				}
	...
	public void changeStaydetails(String RatePlanName, String CheckInDate, String CheckOutDate,
			...
			String adult, String children, String PromoCode, String interval, String productValue) throws Exception {
		...
		try {
			...
			// here first check rules are override or not if override then remove overridden
			// then update it
			...
			rateGrid.selectRatePlan(driver, RatePlanName, test_steps);
			RatesGrid ratesGrid = new RatesGrid();
			ratesGrid.expandRoomClass1(driver, test_steps, ReservationChangeClass.trim());
			ratesGrid.expandChannel(driver, test_steps, ReservationChangeClass.trim(), channelName);
			String updateValueForMinStayRule = String.valueOf((Integer.parseInt(RuleMinStayValue) + 1));
			for (int i = 0; i < daysChange; i++) {

				rateGrid.overrideMinStayValue(driver, test_steps, ReservationChangeClass, channelName, "");
				ratesGrid.updateNoCheckInRule(driver, ReservationChangeClass.trim(), channelName, false, i);
				ratesGrid.updateNoCheckOutRule(driver, ReservationChangeClass.trim(), channelName, false, i);
			}
			...
			rateGrid.clickOnEditRatePlan(driver);
			folioName = rateGrid.getFolioNameOfRatePlan(driver, test_steps);
//			productVal = Integer.parseInt(productValue) + 50;
//			
//			getProductValue.put("amount", Integer.toString(productVal));
//			
//			rateGrid.clickOnProductTab(driver);
//			getTest_Steps = rateGrid.updateProductRateWithDifferentCombination(driver,Integer.toString(productVal),"","",true);
//			
			nightlyRate.switchCalendarTab(driver, test_steps);
			...
			if (isSeasonExistChange) {
				...
				rateGrid.enterRoomClassRatesBasedOnPackage(driver, ReservationChangeClass, "" + newRate,
				...
				rateGrid.enterRoomClassRatesBasedOnPackage(driver, ReservationChangeClass, "" + newRate, false, "", "",
						test_steps);
				
				// update rules at season level
				rateGrid.updateSeasonLevelRules(driver, updateValueForMinStayRule,test_steps);				
				...
				getRoomClassWithRates.put(ReservationRoomClasses,
						rateGrid.getRoomClassRateWithAdditionalAdultAndChildBasedOnPackage(driver,
								ReservationChangeClass, test_steps));
				nightlyRate.closeSeason(driver, test_steps);
			...
			ratesGrid.expandChannel(driver, test_steps, ReservationChangeClass.trim(), channelName);
			String[] reservationRoomClasses = ReservationRoomClasses.split("\\" + Utility.DELIM);
			HashMap<String, ArrayList<String>> getRatesAfterUpdateAtSeasonLevel = new HashMap<String, ArrayList<String>>();
			test_steps.add(
					"===== Verifying min stay, no check in and no checkout rule after update rule at season level =====");
			for (int i = 0; i < 1; i++) {
				ratesGrid.expandRoomClass1(driver, test_steps, reservationRoomClasses[i].trim());
				ratesGrid.expandChannel(driver, test_steps, reservationRoomClasses[i], channelName);

				getRatesAfterUpdateAtSeasonLevel = rateGrid.getRateExAdExChForChannel(driver, reservationRoomClasses[i], channelName, daysChange);
				minStayRule = ratesGrid.getRuleDataValuesForMinStay(driver, test_steps, reservationRoomClasses[i],
						channelName, daysChange);
				app_logs.info("minStayRule: " + minStayRule);

				noCheckInRule = ratesGrid.getRuleDataValuesForNoCheckIn(driver, test_steps, reservationRoomClasses[i],
						channelName, (daysChange - 1));
				app_logs.info("noCheckInRule: " + noCheckInRule);

				noCheckOutRule = ratesGrid.getRuleDataValuesForNoCheckOut(driver, test_steps, reservationRoomClasses[i],
						channelName, (daysChange - 1));
				app_logs.info("noCheckOutRule: " + noCheckOutRule);
				...
				HashMap<String, String> getRuleBaseOnDays = new HashMap<String, String>();
				for (int k = 0; k < daysChange; k++) {
					app_logs.info("getRuleBaseOnDays: " + getRuleBaseOnDays);
						assertEquals(minStayRule.get(k).trim(), updateValueForMinStayRule,
								"Failed: min stay value is mismathing after change in min stay value!");
						assertEquals(noCheckInRule.get(k).trim(), "Yes",
								"Failed: no checkin is mismathing after bulk update!");
			...
			// here to verify season level rates with rates gird
			ArrayList<String> getBaseRateFromRatesGrid = getRatesAfterUpdateAtSeasonLevel.get("rates");
			ArrayList<String> getAdultRateFromRatesGrid = getRatesAfterUpdateAtSeasonLevel.get("adults");
			ArrayList<String> getChildRateFromRatesGrid = getRatesAfterUpdateAtSeasonLevel.get("childs");
			for (int outterLoopForDays = 0; outterLoopForDays < daysChange; outterLoopForDays++) {
				for (int innerLoopForEachCell = 0; innerLoopForEachCell < getBaseRateFromRatesGrid.size(); innerLoopForEachCell++) {
					assertEquals(getBaseRateFromRatesGrid.get(innerLoopForEachCell), baserate,"Failed: Base rate is mismatching in rates grid");
					assertEquals(getAdultRateFromRatesGrid.get(innerLoopForEachCell), adultsRate,"Failed: adult rate is mismatching in rates grid");
					assertEquals(getChildRateFromRatesGrid.get(innerLoopForEachCell), childRate,"Failed: child rate is mismatching in rates grid");
				}
...
//						test_steps.add("bug id: " + bugId);
				...
				if (isButtonEnabled) {
					...
					for (int i = 0; i < intervalDates.size(); i++) {
						...
						if (i == 1) {
							...
							if (date.contains(",")) {
								folioDates = Utility.parseDate(intervalDates.get(0),
										ratesConfig.getProperty("defaultDateFormat"), "EE MMM dd, yyyy");
							} else {
								folioDates = Utility.parseDate(intervalDates.get(0),
										ratesConfig.getProperty("defaultDateFormat"), "EE dd MMM yyyy");
							...
							String date = folio.itemDate(driver, category, i + 1);
							if (date.contains(",")) {
								folioDates = Utility.parseDate(intervalDates.get(i),
										ratesConfig.getProperty("defaultDateFormat"), "EE MMM dd, yyyy");
							} else {
								folioDates = Utility.parseDate(intervalDates.get(i),
										ratesConfig.getProperty("defaultDateFormat"), "EE dd MMM yyyy");
					 ...
					 * 
					 * String date = folio.itemDate(driver, category, i + 1);
					 * app_logs.info("ecpected: " + folioDates); app_logs.info("found: " + date);
					 * 
					 * assertEquals(date, folioDates,
					 * "Failed: Interval date is mismatching in folio line item!");
					 * 
					 * String description = folio.itemDetailsDescroption(driver, category, i + 1);
					 * app_logs.info("ecpected: " + folioName); app_logs.info("found: " +
					 * description);
					 * 
					 * assertEquals(description, folioName,
					 * "Failed: Description is mismatching in folio line item!");
					 * 
					 * String amount = folio.itemAmount(driver, category, i + 1); amount =
					 * folio.splitStringBySign(amount, "$"); double dbAmount =
					 * Double.parseDouble(folioRate.get(i)); String strAmount =
					 * String.format("%.2f", dbAmount); app_logs.info("expected: " + strAmount);
					 * app_logs.info("found: " + amount); assertEquals(amount, "" + strAmount,
					 * "Failed: Amount is mismatching in folio line item!");
					 * test_steps.add(folioDates + "         " + folioName + "           " +
					 * strAmount);
					 * 
					 * }
					 */
			...
			if (driver.findElements(By.xpath("(//span[text()='" + ReservationChangeClass + "'])[2]")).size() > 0
				...
				if (isSeasonExistChange) {
					...
					/*
					 * rateListInReservation.clear(); rateListInReservation =
					 * reservationPage.intervalRateVerificationInChangeStayDetails(driver,
					 * ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate,
					 * maxAdults, "0", ReservationRoomClasses, interval, baseRate.get(0),
					 * adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
					 * maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					 * isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
					 * childRatePerNight.get(0), folioName, test_steps);
...
//					test_steps.add("bug id: " + bugId);
				...
				for (int i = 0; i < intervalDates.size(); i++) {
					...
					if (i == 1) {
						 ...
						 * 
						 * String amount = folio.itemAmount(driver, category, i + 1); amount =
						 * folio.splitStringBySign(amount, "$");
						 * 
						 * double expecetdAmount = Double.parseDouble(folioRate.get(i)); String
						 * getAmount = String.format("%.2f", expecetdAmount); app_logs.info("expected: "
						 * + getAmount); app_logs.info("found: " + amount);
						 * 
						 * assertEquals(amount, getAmount,
						 * "Failed: Amount is mismatching in folio line item!");
						 * test_steps.add(folioDates + "         " + folioName + "           " +
						 * getAmount);
						 */
				...
				// test_steps.add("Verified interval rate along with dates and description after
				// change adult");
...
//					test_steps.add("bug id: " + bugId);
				...
				for (int i = 0; i < intervalDates.size(); i++) {
					...
					if (i == 1) {
						 ...
						 * 
						 * String date = folio.itemDate(driver, category, i + 1);
						 * app_logs.info("expecetd: " + folioDates); app_logs.info("found: " + date);
						 * 
						 * assertEquals(date, folioDates,
						 * "Failed: Interval date is mismatching in folio line item!");
						 * 
						 ...
						 * folio.splitStringBySign(amount, "$"); double dbAmount =
						 * Double.parseDouble(folioRate.get(i)); String strAmount =
						 * String.format("%.2f", dbAmount); app_logs.info("expected amount: " +
						 * strAmount); app_logs.info("found: " + amount);
						 * 
						 * assertEquals(amount, strAmount,
						 * "Failed: Amount is mismatching in folio line item!");
						 * test_steps.add(folioDates + "         " + folioName + "           " +
						 * strAmount);
						 */
...
//				test_steps.add("bug id: " + bugId);
	...
	void ChangeStayDetailsForNoRateChange(String RatePlanName, String CheckInDate, String CheckOutDate,
			String ReservationChangeClass, String channelName, String ReservationRoomClasses, String ReservationType,
			String adult, String children, String PromoCode, String interval, String productValue) throws Exception {
		ArrayList<String> folioRate = rateListInReservation.get(0);
		ArrayList<String> intervalDates = rateListInReservation.get(1);
		...
		if (minStayRuleValueChange > 0) {
			...
			isMinStayRuleChnage = true;
			isMinStayRuleBrokenPopComeOrNotChange = reservationPage.verifyMinStayPopupComeOrNot(driver, minruleChange,
					minStayRuleValueChange, daysChange);
		}
		...
		checkOutColorChange = reservationPage.verifyNoCheckOutPopupComeOrNot(driver, test_steps, noCheckOutRuleChnage,
				CheckInDate, CheckOutDate);
		Utility.app_logs.info("checkOutColorChange : " + checkOutColorChange);
		...
		isSeasonExistChange = nightlyRate.verifySeasonExistInbetweencheckinAndCheckout(driver, test_steps, CheckInDate,
				CheckOutDate);
		...
		try {
			...
			if (isSeasonExistChange) {
				...
				rateGrid.updateProRateWithPackageAtClassLevel(driver, ReservationChangeClass,
						isAdditonalAdultChild.get(0), true, String.valueOf(customRatePN), String.valueOf(adultPN),
						String.valueOf(childPN), test_steps);
		...
		if (driver.findElements(By.xpath("(//span[text()='" + ReservationChangeClass + "'])[2]")).size() > 0
			...
			if (isSeasonExistChange) {
				 ...
				 * maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
				 * isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
				 * childRatePerNight.get(0), folioName, folioRate, totalRoomCharges,
				 * test_steps);
				 */
				...
				try {
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerRoomPerNight(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
										isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
										childRatePerNight.get(0), folioName, folioRate, totalRoomCharges,
										getProductValue, test_steps);
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerRoomPerStay(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerAdultPerNight(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerAdultPerStay(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerChildPerStay(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerChildPerNight(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerPersonPerNight(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerPersonPerStay(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerRoomPerInterval(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerChildPerInterval(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerAdultPerInterval(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsForNoRateChangePerPersonPerInterval(driver,
										ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, adult,
										children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
...
//					test_steps.add("bug id: " + bugId);
			...
			/*
			 * test_steps.add("=====Verification of interval rate in folio====="); for (int
			 * i = 0; i < intervalDates.size(); i++) {
			 * 
			 * String folioDates = Utility.parseDate(intervalDates.get(i),
			 * ratesConfig.getProperty("defaultDateFormat"), "EE MMM dd, yyyy");
			 * 
			 ...
			 * app_logs.info("expecetd: " + folioDates); app_logs.info("found: " + date);
			 * 
			 * // assertEquals(date, folioDates, "Failed: Interval date is // mismatching in
			 * folio line item!");
			 * 
			 * String description = folio.itemDetailsDescroption(driver, category, i + 1);
			 * app_logs.info("expected description name: " + folioName);
			 * app_logs.info("found: " + description);
			 * 
			 * // assertEquals(description, folioName, "Failed: Description // is
			 * mismatching in folio line item!");
			 * 
			 * String getamount = folio.itemAmount(driver, category, i + 1); getamount =
			 * folio.splitStringBySign(getamount, "$"); double dbAmount =
			 * Double.parseDouble(folioRate.get(i)); String strAmount =
			 * String.format("%.2f", dbAmount); app_logs.info("expected amount: " +
			 * strAmount); app_logs.info("found: " + getamount);
			 * 
			 * // assertEquals(getamount, strAmount, "Failed: Amount is // mismatching in
			 * folio line item!"); test_steps.add(folioDates + "         " + folioName +
			 * "           " + strAmount);
			 * 
			 * if (!date.equalsIgnoreCase(folioDates)) {
			 * test_steps.add("Failed: Folio date is mismatching!"); break; }
			 * 
			 * if (!description.equalsIgnoreCase(folioName)) {
			 * test_steps.add("Failed: Folio description is mismatching!"); break;
			 * 
			 * }
			 * 
			 * if (!getamount.equalsIgnoreCase(strAmount)) {
			 * test_steps.add("Failed: Folio amount is mismatching!"); break;
			 * 
			 * }
			 * 
			 * }
		...
		test_steps.add("=================== VERIFICATION OF NO RATE CHANGE AFTER UPDATED ADULT ======================");
		app_logs.info("=================== VERIFICATION OF NO RATE CHANGE AFTER UPDATED ADULT ======================");
		...
		if (driver.findElements(By.xpath("(//span[text()='" + ReservationChangeClass + "'])[2]")).size() > 0
			...
			if (isSeasonExistChange) {
				 ...
				 * reservationPage.intervalRateVerificationInChangeStayDetails(driver,
				 * ratesConfig.getProperty("defaultDateFormat"), CheckInDate, CheckOutDate, "" +
				 * max_adult, "0", ReservationRoomClasses, interval, baseRate.get(0),
				 * adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
				 * maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
				 * isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
				 * childRatePerNight.get(0), folioName, oldRates1, totalRoomCharges,
				 * test_steps);
				 */
...
//					test_steps.add("bug id: " + bugId);
			 ...
			 * strAmount); app_logs.info("found: " + getamount);
			 * 
			 * assertEquals(getamount, strAmount,
			 * "Failed: Amount is mismatching in folio line item!");
			 * test_steps.add(folioDates + "         " + folioName + "           " +
			 * strAmount);
			 * 
	...
	void ChangeStayDetailsForAddAndRemoveDates(String RatePlanName, String CheckInDate, String CheckOutDate,
		...
		ArrayList<String> productInFolio = rateListInReservation.get(3);
		ArrayList<String> oldRates1 = new ArrayList<>();
		ArrayList<String> oldPackageRates = new ArrayList<>();
		String category = "Room Charge";
		int numberOfDys = 0;
		navigation.Inventory(driver);
		test_steps.add("Navigate Inventory");
		...
		ArrayList<String> getOldInterval = reservationPage.getIntervalDuration("dd/MM/yyyy", CheckInDate, CheckOutDate,
				"MMM dd yyyy", "US/Eastern", interlLength, isProStayRate.get(0));

		int addedDays = interlLength + 1;
		CheckInDate = CheckOutDate;
		String oldcheckout = CheckOutDate;
		CheckOutDate = ESTTimeZone.getNextDateBaseOnPreviouseDate(CheckInDate, "dd/MM/yyyy", "dd/MM/yyyy", +addedDays,
				"US/Eastern");
		app_logs.info("CheckOutDate: " + CheckOutDate);
		...
		if (isSeasonExistChange) {
			...
			rateGrid.updateProRateWithPackageAtClassLevel(driver, ReservationChangeClass, isAdditonalAdultChild.get(0),
					true, String.valueOf(customRatePN), String.valueOf(adultPN), String.valueOf(childPN), test_steps);
			...
			getRoomClassWithRates.put(ReservationRoomClasses,
					rateGrid.getRoomClassRateWithAdditionalAdultAndChildBasedOnPackage(driver, ReservationChangeClass,
							test_steps));
			nightlyRate.closeSeason(driver, test_steps);
		...
		if (driver.findElements(By.xpath("(//span[text()='" + ReservationChangeClass + "'])[2]")).size() > 0
			...
			if (isSeasonExistChange) {
				...
				double oldPackageAmount = 0.00;

				oldPackageRates.clear();
				for (int i = 0; i < productInFolio.size(); i++) {

					oldPackageAmount = oldPackageAmount + (Double.parseDouble(productInFolio.get(i)));
					oldPackageRates.add(productInFolio.get(i));

				}

				packageAmount = String.format("%.2f", oldPackageAmount);
				app_logs.info("Old Package Amount : " + oldPackageAmount);
...
//							newdaysChange, test_steps);
				...
				try {
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerRoomPerNight(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
										isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
										childRatePerNight.get(0), folioName, oldRates1, totalRoomCharges,
										getOldInterval, oldRates1, getNewInterval, getProductValue, oldPackageRates,
										packageAmount, test_steps);
					}
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerRoomPerStay(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerRoomPerInterval(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerAdultPerNight(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerAdultPerStay(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("adult")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerAdultPerInterval(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
						...
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerChildPerNight(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerChildPerStay(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("child")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerChildPerInterval(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("night")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerPersonPerNight(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("stay")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerPersonPerStay(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
					...
					if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("person")
							&& getProductValue.get("secondCalcuationMethod").equalsIgnoreCase("interval")) {
						rateListInReservation = reservationPage
								.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerPersonPerInterval(
										driver, ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
										adult, children, ReservationRoomClasses, interval, baseRate.get(0),
										adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
										maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
...
//						test_steps.add("bug id: " + bugId);
			...
			oldPackageRates.clear();
			productInFolio = rateListInReservation.get(3);

			for (int i = 0; i < productInFolio.size(); i++) {
				app_logs.info("productAmount :  " + productInFolio.get(i));
				oldPackageRates.add(productInFolio.get(i));
			}
			 ...
			 * 
			 * String date = folio.itemDate(driver, category, i + 1);
			 * app_logs.info("expected: " + folioDates); app_logs.info("found: " + date);
			 * 
			 * // assertEquals(date, folioDates, "Failed: Interval date is // mismatching in
			 * folio line item!");
			 * 
			 * String description = folio.itemDetailsDescroption(driver, category, i + 1);
			 * app_logs.info("expected: " + folioName); app_logs.info("found: " +
			 * description);
			 * 
			 * // assertEquals(description, folioName, "Failed: Description // is
			 * mismatching in folio line item!");
			 * 
			 * String getamount = folio.itemAmount(driver, category, i + 1); getamount =
			 * folio.splitStringBySign(getamount, "$");
			 * 
			 * String expectedAmount = String.format("%.2f",
			 * Double.parseDouble(folioRate.get(i))); app_logs.info("expected: " +
			 * expectedAmount); app_logs.info("found: " + expectedAmount); //
			 * assertEquals(expectedAmount, expectedAmount, "Failed: // Amount is
			 * mismatching in folio line item!"); test_steps.add(folioDates + "         " +
			 * folioName + "           " + expectedAmount);
			 * 
			 * if (!date.equalsIgnoreCase(folioDates)) {
			 * test_steps.add("Failed: Folio date is mismatching!"); break; } if
			 * (!description.equalsIgnoreCase(folioName)) {
			 * test_steps.add("Failed: Folio description is mismatching!"); break; } if
			 * (!getamount.equalsIgnoreCase(expectedAmount)) {
			 * test_steps.add("Failed: Folio Amount is mismatching!"); break; }
			 * 
			 * }
			 */
		...
		if (driver.findElements(By.xpath("(//span[text()='" + ReservationChangeClass + "'])[2]")).size() > 0
			...
			if (isSeasonExistChange) {
				...
				/*
				 * rateListInReservation = reservationPage.
				 * intervalRateVerificationInChangeStayDetailsBaseOnNewAndOldDates(driver,
				 * ratesConfig.getProperty("defaultDateFormat"), oldcheckout, CheckOutDate,
				 * adult, "0", ReservationRoomClasses, interval, baseRate.get(0),
				 * adultCapacity.get(0), personCapacity.get(0), isAdditonalAdultChild.get(0),
				 * maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
				 * isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
				 * childRatePerNight.get(0), folioName, oldRates1, totalRoomCharges,
...
//						test_steps.add("bug id: " + bugId);
		...
		test_steps.clear();
		testName = "Change stay details-Change only for added/removed dates, update child and verify rate in reservation";
		test_description = "Change stay details-Change only for added/removed dates, update child and verify rate in reservation";
...
//			test_steps.add("New checkout date: " + CheckOutDate);
		...
		if (driver.findElements(By.xpath("(//span[text()='" + ReservationChangeClass + "'])[2]")).size() > 0
			...
			if (isSeasonExistChange) {
				...
				oldPackageRates.clear();
				oldPackageRates = productInFolio;

				double packageAmounts = 0.00;
				for (int i = 0; i < productInFolio.size(); i++) {

					packageAmounts = packageAmounts + (Double.parseDouble(productInFolio.get(i)));
				}

				packageAmount = String.format("%.2f", packageAmounts);
				app_logs.info("total Package Charges: " + packageAmount);
...
//						test_steps.add("bug id: " + bugId);
		...
		oldcheckout = CheckOutDate;
		CheckOutDate = ESTTimeZone.getNextDateBaseOnPreviouseDate(CheckOutDate, "dd/MM/yyyy", "dd/MM/yyyy", +addedDays,
				"US/Eastern");
		app_logs.info("CheckOutDate: " + CheckOutDate);
		daysChange = Utility.getNumberofDays(CheckInDate, CheckOutDate);
		...
		if (driver.findElements(By.xpath("(//span[text()='" + ReservationChangeClass + "'])[2]")).size() > 0
			...
			if (isMinStayRuleChnage) {
				if (!isMinStayRuleBrokenPopComeOrNotChange) {
					minStayColorChange = reservationPage.verifyMinStayLabel(driver, test_steps, ReservationChangeClass,
							minStayRuleValueChange);
					Utility.app_logs.info("minStayColor : " + minStayColorChange);
					try {
						assertTrue(minStayColorChange.equalsIgnoreCase("Red"),
				...
				if (getProductValue.get("firstCalcuationMethod").equalsIgnoreCase("room")
					...
					rateListInReservation = reservationPage
							.intervalRateVerificationInChangeStayDetailsBasedOnPackageBaseOnNewAndOldDatesPerRoomPerNight(
									...
									maxAdult1.get(0), maxPerson.get(0), adultRate.get(0), childRate1.get(0),
									isProStayRate.get(0), customRatePerNight.get(0), adultRatePerNight.get(0),
									childRatePerNight.get(0), folioName, oldRates1, totalRoomCharges, getOldInterval,
									oldRates1, getNewInterval, getProductValue, oldPackageRates, packageAmount,
									test_steps);
				}
...
//					test_steps.add("bug id: " + bugId);
		...
		try {
			...
			if (isSeasonExistChange) {
				...
				rateGrid.updateProRateWithPackageAtClassLevel(driver, ReservationChangeClass,
						isAdditonalAdultChild.get(0), false, String.valueOf(customRatePN), String.valueOf(adultPN),
						String.valueOf(childPN), test_steps);
...
//					test_steps.add("bug id: " + bugId);
	...
	public void ChangeStayDetailsRecalculateAtSeasonLevelAndVerifySaveButtonEnabled(String RatePlanName,
			String CheckInDate, String CheckOutDate, String ReservationChangeClass, String channelName,
			String ReservationRoomClasses, String ReservationType, String adult, String children, String PromoCode,
		...
		if (isSeasonExistChange) {
			...
			double newChildRate = Double.parseDouble(oldchildRate1.get(0));
			double newAdultRate = Double.parseDouble(oldadultRate.get(0));
			rateGrid.enterRoomClassRates(driver, ReservationChangeClass, "" + newRate, oldisAdditonalAdultChild.get(0),
					"" + newAdultRate, "" + newChildRate, test_steps);
			...
			getRoomClassWithRates.put(ReservationRoomClasses,
					rateGrid.getRoomClassRateWithAdditionalAdultAndChild(driver, ReservationChangeClass, test_steps));
			nightlyRate.closeSeason(driver, test_steps);
	...
	public Object[][] getData() {
		...
		return Utility.getData("VerifyPackageRatePlan", envLoginExcel);
	}
