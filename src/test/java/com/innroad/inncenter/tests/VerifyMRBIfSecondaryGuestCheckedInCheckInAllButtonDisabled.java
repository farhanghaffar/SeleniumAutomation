...
public class VerifyMRBIfSecondaryGuestCheckedInCheckInAllButtonDisabled  extends TestCore{
	...
	public void verifyMRBIfSecondaryGuestCheckedInCheckInAllButtonDisabled(String TestCaseId, String TestCaseName,String checkInDate,
		...
		String guestFirstName, String guestLastName,String marketSegment, String referral) throws Exception {
			...
			//checkin secondary guest and verifying checkin all button using checkin policy
			try {
				 res.click_CheckInMrbSecondary(driver, test_steps);
				 res.clickOnProceedToCheckInPaymentButton(driver,test_steps);
				 res.selectCashOnPaymentPopup(driver, test_steps);
				 res.clickOnPayButtonOnPaymentPopup(driver, test_steps);
				 res.checkInPaymentSuccessPopupClose(driver, test_steps);
				 errorMessage="CheckIn All Button is Disabled";
				 res.verifyCheckInAllButtonDisability(driver, test_steps,errorMessage);
			...
			try {
				for(int i=0;i<Utility.testId.size();i++) {
					Utility.testCasePass(Utility.statusCode, i, Utility.comments, "Verify Checkin All Button Disabled");
				}	
	...
	@DataProvider
	   public Object[][] getData() {
		return Utility.getData("VerifyMRBUsingCheckinPolicy", SanityExcel);
	}
