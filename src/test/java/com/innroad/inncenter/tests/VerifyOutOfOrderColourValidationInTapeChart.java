...
public class VerifyOutOfOrderColourValidationInTapeChart extends TestCore{
...
// Create Room Class		
		...
		try {
			...
			driver.findElement(By.xpath(OR.Guest_Services_1)).click();
		...
		try {
			...
			driver.findElement(By.xpath("(//a[text()='Reservations'])[2]")).click();
			...
			test_steps.add("Successfully Verified Out of Order Room colour in Tape Chart");
		...
		try {
			...
			nav.inventory(driver);
			//nav.Rates2(driver);
			driver.findElement(By.xpath("//a[text()='Rates']")).click();
		...
		try {
			...
			driver.findElement(By.xpath("(//a[text()='Setup'])[2]")).click();
			//nav.Setup(driver);
			test_steps.add("Navigated to Setup");
			nav.RoomClass(driver);
	...
	public Object[][] getData() {
		...
		return Utility.getData("verifyOutOfOrderColorInTapeChar", envLoginExcel);
