...
public class CopyButtonInCancelledReservation extends TestCore {
	...
	public void copyButtonInCancelledReservation(String roomClassName, String roomClassAbb, String maxAdults, 
		...
		String testcaseId="848477";
		
		Utility.initializeTestCase(testcaseId, caseId, statusCode, comments, "848477");
		
		testDescription = testName + "<br>"
				+ "<a href='https://innroad.testrail.io/index.php?/cases/view/848477' target='_blank'>"
				+ "Click here to open TestRail: C848477</a><br>";
				
		testCatagory = "Copy Reservation";
		app_logs.info("##################################################################################");
		...
		try {			 
			test_steps.add("========== Verifying copy button status on a cancelled reservation ==========");
			reservationPage.cancellationReservation(driver, test_steps, true);
		//	reservationPage.cancelReservation(driver, test_steps, "CancelReason");
			getTest_Steps=reservationPage.verifyCopyButtonState(driver,"cancel");
		...
		try {
			...
			comments.add(0,  "Notes from UnitOwner Account  is copied to  reservation");
			statusCode.add(0, "1");
			RetryFailedTestCases.count = Utility.reset_count;
			Utility.AddTest_IntoReport(testName, testDescription, testCatagory, test_steps);
		}catch (Exception e) {
	...
	public Object[][] getData() {
		return Utility.getData("CopyButtonInCancelledRes", excel_Swarna);
	}
