...
public class VerifyAdjustChargeVrbo extends TestCore{
	...
	public void verifyBUSChangeStayDetails(String version, String roomClassName, String channel, String message,
		...
		String testName = "Verify Adjustment Charge ON Bus";
		...
		try {
			...
			reservation.click_Folio(driver, testSteps);
			reservation.adjust_LineItem(driver, testSteps, ledgerAccount, "23","HomeAway Rate","Adjust");
			Wait.waitforPageLoad(50, driver);
		}catch (Exception e) {
			e.printStackTrace();
			Utility.catchException(driver,e, test_description, test_catagory, "Update Room Charge", testName, test_description,
					test_catagory, testSteps);

		} catch (Error e) {
			e.printStackTrace();
			Utility.catchError(driver,e, test_description, test_catagory, "Update Room Charge", testName, test_description,
					test_catagory, testSteps);
		}
		...
		try {
			...
			HashMap<String, Double> calcInTaxVal = vrboObject.getVrboTaxesInRoomChargeFolioAFterAddedRoomCharges(driver, chargeName,
					updatedTaxName);
			HashMap<String, Double> calcInFeeVal = reservation.getVrboFeeInFolio(driver, updatedFeeName, days);
			app_logs.info("got tax" + calcInTaxVal);
			app_logs.info("calcValFee" + calcInFeeVal);
