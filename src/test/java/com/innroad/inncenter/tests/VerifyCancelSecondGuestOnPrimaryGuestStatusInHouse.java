	...
	public void verifyStatusOnSecondGuestForInHouseStatus(Map<Object, Object> map)
	{
		...
		//Get Abbreviations
				try {
							...
							RoomAbbri=newRoomclass.getAbbrivation(driver, "|", map.get("RoomClass").toString(), test_steps);
...
//Check IN Primary Room Room
		...
		try
		{
			...
			//	res.clickCloseButtonOfSuccessModelPopup(driver, test_steps); 
				res.clickCloseButtonOfCancelSuccessfully(driver, test_steps);
		...
		try
		{
		    ...
		    System.out.println(" Final Abb No: "+FinalAbb);
			...
			if(res.isRoomReserved||res.isRoomUnAssigned)
			{
				 ...
				 Finalabb=res.stayInfoActualAbb;
				 System.out.println(" Final Abb No: "+Finalabb);
	...
	public Object[][] getData() {
		...
		return Utility.getDataMap("VerifyCancelSGuestStatusInHouse", envLoginExcel);
	}
